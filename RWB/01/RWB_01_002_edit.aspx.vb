Partial Class RWB_01_002_edit
    ...
    Sub sCreate1()
        ...
        If TIMS.ClearSQM(Request("A")) = "E" Then
            ...
            If rSEQNO_E <> "" AndAlso rSEQNO_E = rSEQNO Then
                hid_V.Value = rSEQNO
            End If
    ...
    Private Sub LoadData1(ByVal iSEQNO As Integer)
        Dim sql As String = ""
        sql = "" & vbCrLf
        sql &= " SELECT ROW_NUMBER() OVER(ORDER BY a.SEQNO ASC) ROWNUM " & vbCrLf
        ...
        If dt.Rows.Count > 0 Then
            ...
            txtEDATE1.Text = IIf(flag_ROC, TIMS.Cdate17(Convert.ToString(dr("CEDATED"))), Convert.ToString(dr("CEDATED")))  'edti，by:20181019
            ...
            txtURL.Text = Convert.ToString(dr("C_URL"))
            Common.SetListItem(rblType, Convert.ToString(dr("SUB_FUNID")))
        Else
            Exit Sub
        End If
    End Sub
    ...
    Function CheckData1(ByRef Errmsg As String) As Boolean
        ...
        txtURL.Text = TIMS.ClearSQM(txtURL.Text)
        ...
        If txtTitle.Text = "" Then Errmsg &= "連結標題 不可為空" & vbCrLf
        If txtURL.Text = "" Then Errmsg &= "連結網址 不可為空" & vbCrLf
    ...
    Sub SaveData1()
        ...
        If flagSaveOK1 Then
            '儲存成功
            Dim url1 As String = "RWB_01_002.aspx?id1=" & TIMS.Get_MRqID(Me)
    ...
    Function SaveData2() As Boolean
        ...
        Dim sql As String = ""
        sql = "" & vbCrLf
        sql &= " INSERT INTO TB_CONTENT(SEQNO, FUNID, C_SDATE, C_EDATE, C_TITLE, C_CDATE, C_CACCT, C_UDATE, C_UACCT, C_STATUS, SUB_FUNID, C_URL) " & vbCrLf
        sql &= " VALUES (@SEQNO, @FUNID, @C_SDATE, @C_EDATE, @C_TITLE, GETDATE(), @C_CACCT, GETDATE(), @C_UACCT, 'A', @SUB_FUNID, @C_URL) " & vbCrLf
        ...
        sql &= "     ,C_TITLE = @C_TITLE " & vbCrLf
        sql &= "     ,C_UDATE = GETDATE() " & vbCrLf
        sql &= "     ,C_UACCT = @C_UACCT " & vbCrLf
        sql &= "     ,C_STATUS = 'M' " & vbCrLf
        sql &= "     ,SUB_FUNID = @SUB_FUNID " & vbCrLf
        sql &= "     ,C_URL = @C_URL " & vbCrLf
        ...
        Dim mySDATE1 As String = IIf(flag_ROC, TIMS.Cdate18(txtSDATE1.Text), txtSDATE1.Text)  'edit，by:20181019
        Dim myEDATE1 As String = IIf(flag_ROC, TIMS.Cdate18(txtEDATE1.Text), txtEDATE1.Text)  'edit，by:20181019
        txtTitle.Text = TIMS.ClearSQM(txtTitle.Text)
        txtURL.Text = TIMS.ClearSQM(txtURL.Text)
        Dim oC_SDATE As Object = mySDATE1 + " " + ddlC_SDATE_hh1.SelectedValue + ":" + ddlC_SDATE_mm1.SelectedValue + ":00.000"  'edit，by:20181019
        Dim oC_EDATE As Object = myEDATE1 + " " + ddlC_EDATE_hh1.SelectedValue + ":" + ddlC_EDATE_mm1.SelectedValue + ":00.000"  'edit，by:20181019
        ...
        If hid_V.Value = "" Then
            ...
            With iCmd
                ...
                parms.Add("SUB_FUNID", vITEM)
                parms.Add("C_URL", txtURL.Text)
    ...
    Protected Sub bt_cancle_Click(sender As Object, e As EventArgs) Handles bt_cancle.Click
        Dim url1 As String = ""
        url1 = "RWB_01_002.aspx?id1=" & TIMS.Get_MRqID(Me)
        TIMS.Utl_Redirect(Me, objconn, url1)
    End Sub
End Class