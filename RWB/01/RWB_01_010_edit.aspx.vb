Partial Class RWB_01_010_edit
    ...
    '頁面初始化
    Sub sCreate1()
        txC_CDATE.Text = IIf(flag_ROC, TIMS.Cdate17(DateTime.Now.ToString("yyyy/MM/dd")), DateTime.Now.ToString("yyyy/MM/dd"))  'edit，by:20181019
    ...
    Private Sub LoadData1(ByVal iSEQNO As Integer)
        ...
        sql &= "        ,a.C_CTRLITEM " & vbCrLf
        sql &= "        ,CASE WHEN a.C_CTRLITEM = '1' THEN '按鈕反灰' " & vbCrLf
        sql &= "              WHEN a.C_CTRLITEM = '2' THEN '按鈕消失' " & vbCrLf
        sql &= "              ELSE '' END CCTRLITEM " & vbCrLf
        sql &= " FROM TB_CONTENT a " & vbCrLf
        sql &= " WHERE 1=1 " & vbCrLf
        sql &= "       AND a.FUNID = '009' " & vbCrLf
        ...
        If dt.Rows.Count > 0 Then
            ...
            txC_CDATE.Text = IIf(flag_ROC, TIMS.Cdate17(Convert.ToString(dr("CCDATED"))), Convert.ToString(dr("CCDATED")))  'edit，by:20181019
            txC_SDATE.Text = IIf(flag_ROC, TIMS.Cdate17(Convert.ToString(dr("CSDATED"))), Convert.ToString(dr("CSDATED")))  'edit，by:20181019
            ...
            txC_EDATE.Text = IIf(flag_ROC, TIMS.Cdate17(Convert.ToString(dr("CEDATED"))), Convert.ToString(dr("CEDATED")))  'edit，by:20181019
            ...
            Common.SetListItem(rblCTRLITEM1, Convert.ToString(dr("C_CTRLITEM")))
            txC_CONTENT1.Text = Convert.ToString(dr("C_CONTENT1"))
        Else
            Exit Sub
        End If
    End Sub
    ...
    Function CheckData1(ByRef Errmsg As String) As Boolean
        ...
        txC_SDATE.Text = TIMS.ClearSQM(txC_SDATE.Text)  'edit，by:20181019
        txC_EDATE.Text = TIMS.ClearSQM(txC_EDATE.Text)  'edit，by:20181019
        Dim mySDATE1 As String = IIf(flag_ROC, TIMS.Cdate18(txC_SDATE.Text), txC_SDATE.Text).Replace("/", "-")  'edit，by:20181019
        Dim myEDATE1 As String = IIf(flag_ROC, TIMS.Cdate18(txC_EDATE.Text), txC_EDATE.Text).Replace("/", "-")  'edit，by:20181019
        ...
        Dim vCTRLITEM1 As String = TIMS.ClearSQM(rblCTRLITEM1.SelectedValue)
        txC_CONTENT1.Text = TIMS.ClearSQM(txC_CONTENT1.Text)

        If txC_SDATE.Text = "" OrElse TIMS.CStr1(oC_SDATE) = "" Then Errmsg &= "控制起始日期 不可為空" & vbCrLf
        If vCTRLITEM1 = "" Then Errmsg &= "控制方式 不可為空" & vbCrLf
        If txC_EDATE.Text = "" OrElse TIMS.CStr1(oC_EDATE) = "" Then Errmsg &= "控制結束日期 不可為空" & vbCrLf
        If Errmsg <> "" Then Return False

        If DateDiff(DateInterval.Minute, CDate(oC_SDATE), CDate(oC_EDATE)) = 0 Then Errmsg &= "控制起始日期與控制結束日期 不可相等!!" & vbCrLf
        If DateDiff(DateInterval.Minute, CDate(oC_SDATE), CDate(oC_EDATE)) < 0 Then Errmsg &= "控制起始日期與控制結束日期 順序異常!!" & vbCrLf
    ...
    Sub SaveData1()
        ...
        If flagSaveOK1 Then
            '儲存成功
            Dim url1 As String = "RWB_01_010.aspx?id1=" & TIMS.Get_MRqID(Me)
    ...
    Function SaveData2() As Boolean
        ...
        Dim sql As String = ""
        sql = "" & vbCrLf
        sql &= " INSERT INTO TB_CONTENT(SEQNO, FUNID, C_SDATE, C_EDATE, C_CONTENT1, C_CTRLITEM, C_CDATE, C_CACCT, C_UDATE, C_UACCT, C_STATUS) " & vbCrLf
        sql &= " VALUES (@SEQNO, @FUNID, @C_SDATE, @C_EDATE, @C_CONTENT1, @C_CTRLITEM, GETDATE(), @C_CACCT, GETDATE(), @C_UACCT, 'A') " & vbCrLf
        ...
        sql &= "     ,C_CTRLITEM = @C_CTRLITEM " & vbCrLf
        ...
        txC_SDATE.Text = TIMS.ClearSQM(txC_SDATE.Text)
        txC_EDATE.Text = TIMS.ClearSQM(txC_EDATE.Text)
        Dim mySDATE1 As String = IIf(flag_ROC, TIMS.Cdate18(txC_SDATE.Text), txC_SDATE.Text)  'edit，by:20181019
        Dim myEDATE1 As String = IIf(flag_ROC, TIMS.Cdate18(txC_EDATE.Text), txC_EDATE.Text)  'edit，by:20181019
        txC_CONTENT1.Text = TIMS.ClearSQM(txC_CONTENT1.Text)
        Dim vCTRLITEM1 As String = TIMS.ClearSQM(rblCTRLITEM1.SelectedValue)
        Dim oC_SDATE As Object = mySDATE1 + " " + ddlC_SDATE_hh1.SelectedValue + ":" + ddlC_SDATE_mm1.SelectedValue + ":00.000"  'edit，by:20181019
        Dim oC_EDATE As Object = myEDATE1 + " " + ddlC_EDATE_hh1.SelectedValue + ":" + ddlC_EDATE_mm1.SelectedValue + ":00.000"  'edit，by:20181019
        ...
        If hid_V.Value = "" Then
            ...
            With iCmd
                ...
                parms.Add("C_CONTENT1", txC_CONTENT1.Text)
                parms.Add("C_CTRLITEM", vCTRLITEM1)
    ...
    Protected Sub bt_cancle_Click(sender As Object, e As EventArgs) Handles bt_cancle.Click
        Dim url1 As String = ""
        url1 = "RWB_01_010.aspx?id1=" & TIMS.Get_MRqID(Me)
        TIMS.Utl_Redirect(Me, objconn, url1)
    End Sub
End Class