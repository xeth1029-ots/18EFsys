Partial Class RWB_01_002
    ...
    Sub sSearch1()
        'BANNER圖示連結
        'Dim vITEM As String = TIMS.ClearSQM(rblType.SelectedValue)
        Dim vITEM As String = TIMS.GetListValue(rblType)
        ...
        Dim schCCDATE1 As String = schC_CDATE1.Text
        Dim schCCDATE2 As String = schC_CDATE2.Text
        Dim schCSDATE1 As String = schC_SDATE1.Text
        Dim schCSDATE2 As String = schC_SDATE2.Text
        Dim schCEDATE1 As String = schC_EDATE1.Text
        Dim schCEDATE2 As String = schC_EDATE2.Text

        Dim sql As String = ""
        sql = "" & vbCrLf
        sql &= " SELECT ROW_NUMBER() OVER(ORDER BY a.SEQNO DESC) ROWNUM " & vbCrLf
        ...
        sql &= " ,CASE WHEN a.SUB_FUNID = '1' THEN '訓練資源連結' " & vbCrLf
        sql &= "  WHEN a.SUB_FUNID = '2' THEN '求職求才資源連結' " & vbCrLf
        sql &= "  WHEN a.SUB_FUNID = '3' THEN '其他資源連結' " & vbCrLf
        sql &= "  ELSE '' END CSUBFUN " & vbCrLf
        sql &= " FROM TB_CONTENT a " & vbCrLf
        sql &= " WHERE 1=1 " & vbCrLf
        sql &= " AND a.C_STATUS <> 'D' " & vbCrLf
        sql &= " AND a.FUNID = '010' " & vbCrLf
        ...
        If vITEM <> "" Then sql &= " AND a.SUB_FUNID = @SUB_FUNID " & vbCrLf

        sql &= " ORDER BY a.C_UDATE DESC " & vbCrLf

        Dim parms As Hashtable = New Hashtable()
        If schCCDATE1 <> "" Then parms.Add("schCCDATE1", If(flag_ROC, TIMS.Cdate18(schCCDATE1), schCCDATE1))  'edit，by:20181019
        If schCCDATE2 <> "" Then parms.Add("schCCDATE2", If(flag_ROC, TIMS.Cdate18(schCCDATE2), schCCDATE2))  'edit，by:20181019
        If schCSDATE1 <> "" Then parms.Add("schCSDATE1", If(flag_ROC, TIMS.Cdate18(schCSDATE1), schCSDATE1))  'edit，by:20181019
        If schCSDATE2 <> "" Then parms.Add("schCSDATE2", If(flag_ROC, TIMS.Cdate18(schCSDATE2), schCSDATE2))  'edit，by:20181019
        If schCEDATE1 <> "" Then parms.Add("schCEDATE1", If(flag_ROC, TIMS.Cdate18(schCEDATE1), schCEDATE1))  'edit，by:20181019
        If schCEDATE2 <> "" Then parms.Add("schCEDATE2", If(flag_ROC, TIMS.Cdate18(schCEDATE2), schCEDATE2))  'edit，by:20181019
        If vITEM <> "" Then parms.Add("SUB_FUNID", vITEM)
    ...
    Protected Sub bt_add_Click(sender As Object, e As EventArgs) Handles bt_add.Click
        Dim url1 As String = "RWB_01_002_edit.aspx?id1=" & TIMS.Get_MRqID(Me)
    ...
    Sub GetSearchStr()
        Session("_SearchStr") = "prg=RWB_01_002"
