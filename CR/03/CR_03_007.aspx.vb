...
Public Class CR_03_007
    ...
    Public Function SEARCH_DATA1_dt() As DataTable
        ...
        'DECLARE @TPLANID NVARCHAR(2)='28';DECLARE @YEARS SMALLINT=CONVERT(SMALLINT,'2024');DECLARE @APPSTAGE NVARCHAR(1)='2';DECLARE @ORGKIND2 NVARCHAR(1)='W';
...
FROM dbo.VIEW2B PP
JOIN dbo.V_GOVCLASSCAST3 IG3 ON IG3.GCID3=PP.GCID3 AND IG3.GCID3=2072
...
"
        If TIMS.sUtl_ChkTest() Then
            ...
            TIMS.WriteLog(Me, String.Concat("--##CR_03_007.aspx , sSql:", vbCrLf, sSql))
        End If
    ...
    Sub ExportXlsGW1()
        ...
        '113年度下半年提升勞工自主學習計畫-推拿整復訓練課程統計表									
        Dim v_rblOrgKind2 As String = TIMS.GetListValue(rblOrgKind2)
        Dim s_PLANNAME As String = TIMS.GetListText(rblOrgKind2) '計畫
        Dim V_SHTNM1 As String = If(v_rblOrgKind2 = "G", "產投", If(v_rblOrgKind2 = "W", "自主", ""))
        Dim V_SHEETNM1 As String = String.Concat(V_SHTNM1, "-推拿整復課程(陳核版)")
        ...
        Dim s_TITLENAME1 As String = String.Concat(s_ROCYEAR1, "年度", s_APPSTAGE_NM2, s_PLANNAME, "-推拿整復訓練課程統計表")
        Dim s_FILENAME1 As String = String.Concat(s_ROCYEAR1, "年", s_APPSTAGE_NM2, s_PLANNAME, "推拿整復課程統計表_", TIMS.GetDateNo())
        ...
        SyncLock print_lock
            ...
            '序號,訓練單位名稱,課程名稱(含期別),訓練時數,訓練人次,開訓日期,結訓日期,"縣市別(辦訓地)",聯絡人,聯絡電話,
            SetCellValue(ws, "A2", "序號")
            SetCellValue(ws, "B2", "訓練單位名稱")
            SetCellValue(ws, "C2", "課程名稱(含期別)")
            SetCellValue(ws, "D2", "訓練時數")
            SetCellValue(ws, "E2", "訓練人次")
            SetCellValue(ws, "F2", "開訓日期")
            SetCellValue(ws, "G2", "結訓日期")
            SetCellValue(ws, "H2", String.Concat("縣市別", vbCrLf, "(辦訓地)"))
            SetCellValue(ws, "I2", "聯絡人")
            SetCellValue(ws, "J2", "聯絡電話")
            ws.Cells("A1:J2").Style.Font.Bold = True
            ...
            For Each dr1 As DataRow In dtXls1.Rows
                ...
                SetCellValue(ws, "F" & idxStr, dr1("STDATE")) '"開訓日期")
                SetCellValue(ws, "G" & idxStr, dr1("FTDATE")) '"開訓日期")
                SetCellValue(ws, "H" & idxStr, dr1("CTNAME")) '"辦訓縣市別")
                SetCellValue(ws, "I" & idxStr, dr1("CONTACTNAME")) '"聯絡人")
                SetCellValue(ws, "J" & idxStr, V_CONTACTPHONE, ExcelHorizontalAlignment.Left) '"聯絡電話")
            ...
            SetCellValue(ws, "F" & idxStr, "") '"結訓日期")
            SetCellValue(ws, "G" & idxStr, "") '"單位屬性")
            SetCellValue(ws, "H" & idxStr, "") '"辦訓縣市別")
            SetCellValue(ws, "I" & idxStr, "") '"課程流水號")
            SetCellValue(ws, "J" & idxStr, "") '"聯絡人")
            ...
            ws.Column(ws.Cells(String.Format("A3:A{0}", idxStr)).Start.Column).Width = 10 '序號
            ...
            ws.Column(ws.Cells(String.Format("F3:F{0}", idxStr)).Start.Column).Width = 15
            ws.Column(ws.Cells(String.Format("G3:G{0}", idxStr)).Start.Column).Width = 15
            ...
            ws.Column(ws.Cells(String.Format("J3:J{0}", idxStr)).Start.Column).Width = 25
