<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="SD_17_002.aspx.vb" Inherits="WDAIIP.SD_17_002" %>
...
<head>
    ...
    <script type="text/javascript" language="javascript">
        ...
        function SelectAll(flag, obj) {
            var cst_statu = 12; //撥款狀況
            var cst_date1 = 13; //撥款日期
            ...
            if (flag == 0) {
                ...
                for (i = 1; i < MyTable.rows.length; i++) {
                    MyTable.rows(i).cells(cst_statu).children(0).selectedIndex = idx;
                }

            } else {
                //撥款日期全選
                var objValue = MyTable.rows(1).cells(cst_date1).children(0).value;

                if (obj.checked && objValue != '') {
                    for (i = 1; i < MyTable.rows.length; i++) {
                        MyTable.rows(i).cells(cst_date1).children(0).value = objValue;
                    }
                } else if (!obj.checked) {
                    for (i = 2; i < MyTable.rows.length; i++) {
                        MyTable.rows(i).cells(cst_date1).children(0).value = '';
                    }
                }
            }
        }
        ...
        function selDate(idx) {
            var cst_date1 = 13; //撥款日期
            var MyTable = document.getElementById('DataGrid1');
            show_calendar(MyTable.rows(idx).cells(cst_date1).children(0).id, '', '', 'CY/MM/DD');
        }
        ...
        function chkSave() {
            ...
            for (i = 1; i < MyTable.rows.length; i++) {
                AllotDate = MyTable.rows(i).cells(cst_date1).children(0).value;
                if (AllotDate != '') {
            ...
            for (i = 1; i < MyTable.rows.length; i++) {
                AppliedStatus = MyTable.rows(i).cells(cst_statu).children(0).selectedIndex;
                AllotDate = MyTable.rows(i).cells(cst_date1).children(0).value;
                ...
                <td>
                        ...
                        <tr>
                            <td>
                                <asp:Label ID="TitleLab1" runat="server"></asp:Label><asp:Label ID="TitleLab2" runat="server">
									首頁&gt;&gt;學員動態管理&gt;&gt;補助金請領&gt;&gt;<FONT color="#990000">補助撥款</FONT>
                                </asp:Label>
                            </td>
                        </tr>
                    </table>
                    <table class="table_nw" id="Table2" width="100%">
                        <tbody>
                            ...
                            <tr>
                                <td class="bluecol">經費審核
                                </td>
                                <td class="whitecol">
                                    <asp:RadioButtonList ID="AuditList" runat="server" RepeatDirection="Horizontal" Font-Size="X-Small">
                                    </asp:RadioButtonList>
                                </td>
                            </tr>
                        </tbody>
                    ...
                    <table width="740">
                        <tr>
                            <td align="center">
                                <asp:Button ID="btnSch" runat="server" Text="查詢" CssClass="asp_button_S"></asp:Button><br />
                                ...
                                <asp:DataGrid ID="DataGrid1" runat="server" Width="100%" AutoGenerateColumns="False" CssClass="font" AllowSorting="True">
                                    ...
                                    <Columns>
                                        ...
                                        <asp:TemplateColumn HeaderText="是否取得結訓資格">
                                            ...
                                            <ItemTemplate>
                                                <input type="hidden" id="hid_SOCID" runat="server" />
