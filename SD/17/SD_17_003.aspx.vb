Partial Class SD_17_003
    ...
    'Const cst_審核狀態 As Integer = 12
    'Const cst_審核備註 As Integer = 13
    'Const cst_保險證號 As Integer = 14
    'Const cst_預算別代碼 As Integer = 15

#Region "Functions"

    'SQL 取得資料 DataTable
    Private Function Get_ClassStudentsOfClass(ByVal OCIDValue As String, _
        ByVal ORDER_BY As String) As DataTable
        Dim rst As DataTable = Nothing

        Dim sqlstr As String = ""
        sqlstr = "" & vbCrLf
        sqlstr += " select" & vbCrLf
        sqlstr += "  a.SOCID, dbo.SUBSTR(a.StudentID,-2) StudentID" & vbCrLf
        sqlstr += " ,b.IDNO,b.Name,d.TotalCost" & vbCrLf
        sqlstr += " ,f.SumOfMoney,f.PayMoney ,a.WorkSuppIdent" & vbCrLf
        sqlstr += " ,f.AppliedStatusM,f.AppliedNote,a.ActNO,f.BudID" & vbCrLf
        ...
        sqlstr += "   where exists (" & vbCrLf
        sqlstr += "     select 'x' from Class_StudentsOfClass x " & vbCrLf
        sqlstr += "     where x.ocid='" & OCIDValue & "' " & vbCrLf
        sqlstr += "     and x.socid=m.socid  )" & vbCrLf
        sqlstr += "   group by m.socid " & vbCrLf
        sqlstr += " ) tt on tt.socid =a.socid " & vbCrLf
        sqlstr += " where 1=1" & vbCrLf
        sqlstr += " AND f.Budid is not null" & vbCrLf
        sqlstr += " AND a.OCID='" & OCIDValue & "'" & vbCrLf
        'If ORDER_BY = "" Then SqlStr += " order by dbo.SUBSTR(a.StudentID,-2) " Else SqlStr += " order by " & ORDER_BY
        If ORDER_BY <> "" Then
    ...
    Private Sub Show_DataGrid(ByVal dg As DataGrid, ByVal dt As DataTable, ByVal PageIndex As Integer)
        ...
        If dt.Rows.Count > 0 Then
            ...
            AuditNum(Me.ViewState("OCIDValue1"))
        End If
    End Sub

    Private Function Get_StudBligateData(ByVal idno As String, _
        ...
        Try
            ...
            '    .SelectCommand.Parameters.Add("@mdate", SqlDbType.DateTime).Value = sdate
            '    cntBli = .SelectCommand.ExecuteScalar()
            'End With
            ...
            If Not rst Then
                ...
                sqlStr += "where b.ActNo is not null and a.MDate<=" & TIMS.To_date(sdate) & " order by a.MDate desc "
    ...
    Sub OpenButtons(ByVal OCIDValue As String)
        ...
        If IsDBNull(dr("AppliedResultM")) OrElse (dr("AppliedResultM").ToString = "R") Then
            ...
            Me.AuditCheck.Enabled = True
            TIMS.Tooltip(AuditCheck, "整班經費審核，尚未確認", True)
        Else
            Me.AuditCheck.Enabled = False
...
#Region "NO USE"
    ...
    'Private Sub Get_YearsPeriod(ByVal idno As String, _
    '    ByRef sdate As String, ByRef edate As String, ByVal applied As String, _
    ...
    '    If applied = "N" Then
    '        Sql += " and dbo.NVL(c.AppliedStatusM,'R')='R'" & vbCrLf
    '    Else
    ...
    'Private Function Get_SubSidyCost(ByVal idno As String, _
    '    ByVal stdate As String, ByVal applied As String, ByRef tmpSDate As String, ByRef tmpEDate As String, _
    '    ByVal tmpConn As SqlConnection) As Integer
    ...
    '    If (dr("AppliedResultM").ToString = "Y") Or (dr("AppliedResultM").ToString) = "N" Then
...
#End Region
    ...
    Sub AuditNum(ByVal OCIDValue As String)
        Dim sql As String = ""
        Dim dr As DataRow = Nothing
        sql = "" & vbCrLf
        sql += " SELECT " & vbCrLf
        sql += "  dbo.NVL( Sum(Case When AppliedStatusM = 'Y' Then 1 Else 0 End),0) as SNum" & vbCrLf '審核成功筆數
        sql += " ,dbo.NVL( Sum(Case When AppliedStatusM = 'N' Then 1 Else 0 End),0) as FNum" & vbCrLf '審核失敗筆數
        sql += " ,dbo.NVL( Sum(Case When AppliedStatusM = 'R' Then 1 Else 0 End),0) as RNum" & vbCrLf '退件修正筆數
        sql += " ,dbo.NVL( Sum(Case When AppliedStatusM is Null Then 1 Else 0 End),0) as ANum " & vbCrLf '未審核(請選擇、還原)筆數
        sql += " FROM Stud_SubsidyCost " & vbCrLf
        sql += " WHERE Budid is not null " & vbCrLf
        sql += " AND SOCID IN (SELECT SOCID FROM Class_StudentsOfClass WHERE OCID='" & OCIDValue & "')" & vbCrLf
        dr = DbAccess.GetOneRow(sql, objconn)
...
#End Region
    ...
    Private Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        If OCIDValue1.Value <> "" Then
            ...
            Show_DataGrid(Datagrid2, Get_ClassStudentsOfClass(OCIDValue1.Value, Me.ViewState("sort")), 0)
    ...
    Private Sub AuditCheck_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles AuditCheck.Click
        ...
        Me.ViewState("OCIDValue1") = ""
        If OCIDValue1.Value.Trim = "" Then
            Common.MessageBox(Me, "請先選擇一職類班別")
            Exit Sub
        Else
            Me.ViewState("OCIDValue1") = OCIDValue1.Value.Trim.Replace("'", "''")
            sql = "select 'x' from class_classinfo where ocid ='" & Me.ViewState("OCIDValue1") & "' "
            dr = DbAccess.GetOneRow(sql, objconn)
            If dr Is Nothing Then
                Common.MessageBox(Me, "該職類班別不存在，請重新選擇")
                Exit Sub
            End If
        End If
        ...
        '    Exit Sub
        'End If
        'AuditCheck.Attributes("onclick") = "return confirm('經費審核確認')"
        ...
        Dim conn As SqlConnection = DbAccess.GetConnection
        sql = "" & vbCrLf
        sql += " SELECT * FROM Stud_SubsidyCost p" & vbCrLf
        sql += " WHERE exists (" & vbCrLf
        sql += " 	SELECT 'x' FROM Class_StudentsOfClass c " & vbCrLf
        sql += " 	WHERE c.OCID='" & Me.ViewState("OCIDValue1") & "'" & vbCrLf
        sql += " 	and p.socid =c.socid" & vbCrLf
        sql += " )" & vbCrLf
        ...
        For Each item As DataGridItem In Datagrid2.Items
            ...
            Dim hSOCID As HtmlInputHidden = item.FindControl("hSOCID")

            If dt.Select("SOCID='" & hSOCID.Value & "'").Length > 0 Then
                dr = dt.Select("SOCID='" & hSOCID.Value & "'")(0)
                Select Case AppliedStatusM.SelectedIndex
                    Case 0
                        dr("AppliedStatusM") = Convert.DBNull
                        dr("AppliedStatus") = Convert.DBNull
                    Case 1
                        dr("AppliedStatusM") = "Y" '通過
                        dr("AppliedStatus") = "1" '撥款 'Convert.DBNull
                    Case 2
                        dr("AppliedStatusM") = "N" '不通過
                        dr("AppliedStatus") = "0" '不撥款
                    Case 3
                        dr("AppliedStatusM") = "R" '退件
                        dr("AppliedStatus") = "0" '不撥款
                End Select
        ...
        sql = "SELECT * FROM Class_StudentsOfClass WHERE OCID='" & Me.ViewState("OCIDValue1") & "'"
        dt = DbAccess.GetDataTable(sql, da, conn)
        For Each item As DataGridItem In Datagrid2.Items
            'Dim AppliedStatusM As DropDownList = item.FindControl("AppliedStatusM")
            ...
            If dt.Select("SOCID='" & hSOCID.Value & "'").Length > 0 Then
                dr = dt.Select("SOCID='" & hSOCID.Value & "'")(0)
                If BudID.SelectedValue <> "" Then
                    dr("BudgetId") = BudID.SelectedValue '預算別
                Else
                    dr("BudgetId") = Convert.DBNull
                End If
                dr("ModifyAcct") = sm.UserInfo.UserID
                dr("ModifyDate") = Now
            End If
    ...
    Private Sub Datagrid2_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles Datagrid2.ItemDataBound
        ...
        Select Case e.Item.ItemType
            Case ListItemType.Header
                ...
                'Dim sqlAdp As New SqlDataAdapter
                'Dim sqlStr As String = String.Empty
                'Dim appRst As String = String.Empty

                'sqlStr = "select AppliedResultM from Class_ClassInfo where OCID=@ocid "
                'With sqlAdp
                '    .SelectCommand = New SqlCommand(sqlStr, objConn)
                '    .SelectCommand.Parameters.Clear()
                '    .SelectCommand.Parameters.Add("@ocid", SqlDbType.Int).Value = OCIDValue1.Value
                '    appRst = Convert.ToString(.SelectCommand.ExecuteScalar())
                'End With
                'listVerifyAll.Attributes.Add("onChange", "SelectAll();")
                'If appRst = "Y" Or appRst = "N" Then listVerifyAll.Enabled = False Else listVerifyAll.Enabled = True
                ...
                Else
                    ...
                    listVerifyAll.Attributes.Add("onChange", "SelectAll();")
                End If
                Select Case UCase(Me.ViewState("sort"))
                    Case UCase("StudentID"), UCase("StudentID DESC")
                        mysort.ImageUrl = "../../images/SortUp.gif"
                        e.Item.Cells(cst_學號).Controls.Add(mysort)
                    Case UCase("IDNO"), UCase("IDNO DESC")
                        mysort.ImageUrl = "../../images/SortUp.gif"
            ...
            Case ListItemType.Item, ListItemType.AlternatingItem, ListItemType.EditItem
                ...
                Dim labTotal As Label = e.Item.FindControl("lab_Total")
                Dim listVerify As DropDownList = e.Item.FindControl("list_Verify")
                Dim btnBackVerify As LinkButton = e.Item.FindControl("btn_BackVerify")
                ...
                Dim labSubSidyCost As Label = e.Item.FindControl("lab_SubSidyCost")
                Dim labPersonalCost As Label = e.Item.FindControl("lab_PersonalCost")
                Dim labBalance As Label = e.Item.FindControl("lab_Balance")
                Dim labOtherGovApply As Label = e.Item.FindControl("lab_OtherGovApply")
                Dim labStar As Label = e.Item.FindControl("lab_Star")
                Dim hSOCID As HtmlInputHidden = e.Item.FindControl("hSOCID")

                'Dim totalSubSidy, sumSubSidy As Integer
                Dim totalSubSidy As Integer = 0
                Dim sDate As String = ""
                Dim eDate As String = ""
                btnBackVerify.CommandArgument = Convert.ToString(drData("SOCID"))

                hSOCID.Value = Convert.ToString(drData("SOCID"))
                ...
                ''餘額、其他補助 'If sm.UserInfo.Years < 2008 Then totalSubSidy = 30000 Else totalSubSidy = 50000
                totalSubSidy = TIMS.Get_3Y_SupplyMoney() '產投 政府補助經費

                'labBalance.Text = totalSubSidy - Get_SubSidyCost(labIDNO.Text, drData("STDate"), "Y", sDate, eDate, objconn)
                'labOtherGovApply.Text = Get_SubSidyCost(labIDNO.Text, drData("STDate"), "N", sDate, eDate, objconn)
                '含職前webservice
                labBalance.Text = totalSubSidy - TIMS.Get_SubsidyCost(labIDNO.Text, drData("STDate").ToString(), drData("SOCID").ToString(), "Y", objconn)
                '含職前webservice
                labOtherGovApply.Text = TIMS.Get_SubsidyCost(labIDNO.Text, drData("STDate").ToString(), drData("SOCID").ToString(), "N", objconn)

                e.Item.Cells(cst_其他申請中金額).ToolTip = Msg_目前申請總額 ' "學員目前已申請未核准補助金總額(超過剩餘可用餘額以紅字表示)"
                'e.Item.Cells(cst_學號).ToolTip = Get_StudSubSidyCostTooltip(labIDNO.Text, sDate, eDate, objconn)
                ...
                If IsDBNull(drData("AppliedStatusM")) = False Then
                    ...
                    listBudID.Enabled = False
                Else
                    btnBackVerify.Visible = False
                End If
                If Not listBudID.Items.FindByValue(Convert.ToString(drData("BudID"))) Is Nothing Then listBudID.SelectedValue = drData("BudID")
                '是否結訓
                If CBool(drData("CreditPoints")) = True Then labEndClass.Text = "是" Else labEndClass.Text = "否"
                '出席達80%
                If drData("TOHours") > 0 Then
                    If drData("TOHours") > (drData("THours") * 2 / 10) Then labOnClassRate.Text = "否" Else labOnClassRate.Text = "是"
                Else
                    labOnClassRate.Text = "是"
                End If
                '是否為在職者
                Select Case Convert.ToString(drData("WorkSuppIdent"))
                    Case "Y"
