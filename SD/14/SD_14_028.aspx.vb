Public Class SD_14_028
    Inherits AuthBasePage 'System.Web.UI.Page

    'OJT-24022101：<系統> 產投 - 學員線上簽到(退)紀錄表：介接line@打卡資料 + 增加「學員線上簽到(退)紀錄表」功能
    Const cst_printFN1 As String = "SD_14_028"
    ...
    Sub Search1()
        ...
        '限制為只有正式儲存之班級 '已轉班
        sql &= " WHERE pp.IsApprPaper='Y' AND pp.TransFlag='Y' AND cc.ISSUCCESS='Y' AND cc.NOTOPEN='N'" & vbCrLf
        ...
        If SCH_STRAINDATE.Text <> "" Then sql &= " AND v2.STRAINDATE=@STRAINDATE" & vbCrLf

        Dim parms As New Hashtable
        If sRelShip <> "" Then parms.Add("RelShip", String.Concat(sRelShip, "%"))
        If Hid_OCID1.Value <> "" Then parms.Add("OCID", Val(Hid_OCID1.Value))
        If SCH_STRAINDATE.Text <> "" Then parms.Add("STRAINDATE", TIMS.Cdate2(SCH_STRAINDATE.Text))
    ...
    Function checkData1(ByRef errMsg As String) As Boolean
        ...
        'SCH_STRAINDATE.Text=TIMS.cdate3(TIMS.ClearSQM(SCH_STRAINDATE.Text))
        'If SCH_STRAINDATE.Text="" Then
        '    errMsg &= "上課日期 不可為空!" & vbCrLf
        'ElseIf SCH_STRAINDATE.Text <> "" AndAlso Not TIMS.IsDate1(SCH_STRAINDATE.Text) Then
        '    errMsg &= "上課日期 格式有誤!" & vbCrLf
        'End If
    ...
    Private Function CHK_STUD_ADP_STUDATTEND(oConn As SqlConnection, v_OCID As String, v_STRAINDATE As String) As Integer
        Dim rst As Integer = 0
        If v_OCID = "" OrElse v_STRAINDATE = "" Then Return rst

        v_STRAINDATE = TIMS.Cdate3(v_STRAINDATE)
        If v_STRAINDATE = "" Then Return rst

        Dim parms As New Hashtable From {{"OCID", Val(v_OCID)}, {"STRAINDATE", v_STRAINDATE}}
        Dim sql As String = ""
        sql &= " SELECT 1 CROW FROM ADP_STUDATTEND a" & vbCrLf
        sql &= " WHERE a.OCID=@OCID AND a.STRAINDATE=@STRAINDATE" '#{OCID}" & vbCrLf
    ...
    Protected Sub btnSearch1_Click(sender As Object, e As EventArgs) Handles btnSearch1.Click
        ...
        If OCIDValue1.Value <> "" AndAlso SCH_STRAINDATE.Text <> "" Then
            Dim rPMS As New Hashtable From {{"HP", 1}, {"OCID", OCIDValue1.Value}, {"STRAINDATE", TIMS.Cdate3(SCH_STRAINDATE.Text)}}

            Call TIMS.UPDATE_ADP_STUDATTEND(sm, objconn, OCIDValue1.Value, SCH_STRAINDATE.Text, rPMS)

            Dim sErrMsg3 As String = ""
            Dim iStd As Integer = CHK_STUD_ADP_STUDATTEND(objconn, OCIDValue1.Value, SCH_STRAINDATE.Text)
            If iStd = 0 Then sErrMsg3 &= "查無 學員線上簽到(退)紀錄表 資料!" & vbCrLf
            If sErrMsg3 <> "" Then
    ...
    Private Sub DataGrid1_ItemCommand(source As Object, e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles DataGrid1.ItemCommand
        ...
        Select Case e.CommandName
            Case cst_dg1eCM_Print1 '"Print1"
                ...
                '取得線上簽到退紀錄使用 LINEAT.Wdaline 依班級/日期
                Dim rPMS As New Hashtable From {{"HP", 1}, {"OCID", s_OCID}, {"STRAINDATE", s_STRAINDATE}}
                Call TIMS.UPDATE_ADP_STUDATTEND(sm, objconn, s_OCID, s_STRAINDATE, rPMS)

                Dim sErrMsg3 As String = ""
                Dim iStd As Integer = CHK_STUD_ADP_STUDATTEND(objconn, s_OCID, s_STRAINDATE)
                If iStd = 0 Then sErrMsg3 &= "查無 學員線上簽到(退)紀錄表 資料!" & vbCrLf
                If sErrMsg3 <> "" Then
                    Common.MessageBox(Me, sErrMsg3)
                    Exit Sub
                ...
                Dim prtstr As String = $"&TPlanID={sm.UserInfo.TPlanID}&OCID={s_OCID}&PCSValue={s_PCSValue}&STRAINDATE={s_STRAINDATE}"
