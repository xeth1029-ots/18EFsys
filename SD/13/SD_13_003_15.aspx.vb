...
#Region "Functions"
    ...
    Private Function Get_ClassStudentsOfClass(ByVal ocid As Integer, ByVal orderCol As String) As DataTable
        ...
        sql &= " ,d.THours" & vbCrLf
        sql &= " ,ISNULL(t.TOHours,0) TOHours" & vbCrLf
        ...
        sql &= " 	select m.SOCID, sum(ISNULL(m.Hours,0)) TOHours" & vbCrLf
        sql &= " 	from Class_StudentsOfClass cs" & vbCrLf
        sql &= " 	join Stud_Turnout m on cs.socid=m.socid" & vbCrLf
        sql &= " 	where cs.ocid =@OCID" & vbCrLf
        sql &= " 	group by m.SOCID" & vbCrLf
        sql &= " ) t on t.SOCID=a.SOCID" & vbCrLf
        ...
        '    slogMsg1 &= "##SD_13_003_15.aspx, sql: " & sql & vbCrLf
        '    'slogMsg1 &= "##SYS_01_001, myParam: " & TIMS.GetMyValue3(myParam) & vbCrLf
        '    TIMS.writeLog(Me, slogMsg1)
        'End If
        rst = DbAccess.GetDataTable(sql, objconn, parms)
    ...
    '檢查是否存在只有加保沒有退保的紀錄，有的話代表加保中。
    Private Function Get_StudBligateData(ByVal idno As String,
        ...
        sqlStr += " from (select ActNo,ChangeMode,Max(MDate) as MDate from Stud_BligateData where ChangeMode=4 and IDNO='" & idno & "' and MDate<= " & TIMS.to_date(sdate) & " group by ActNo,ChangeMode ) a "
        sqlStr += " join (select ActNo,ChangeMode,Max(MDate) as MDate from Stud_BligateData where ChangeMode=2 and IDNO='" & idno & "' group by ActNo,ChangeMode) b on b.ActNo=a.ActNo and b.MDate>=a.MDate "
        'sqlStr += " where b.ActNo is not null "
        ...
        If Not rst Then
            'sqlStr = " select a.ActNo,a.ChangeMode,a.MDate,b.ActNo as ActNo2,b.ChangeMode as ChangeMode2,b.MDate as MDate2 " & vbCrLf
            'sqlStr += " from (select ActNo,ChangeMode,Max(MDate) as MDate from Stud_BligateData where ChangeMode=4 and upper(IDNO)='" & idno & "' group by ActNo,ChangeMode) a "
            'sqlStr += " left join (select ActNo,ChangeMode,Max(MDate) as MDate from Stud_BligateData where ChangeMode=2 and upper(IDNO)='" & idno & "' group by ActNo,ChangeMode) b on b.ActNo=a.ActNo and b.MDate>=a.MDate "
            'sqlStr += " where b.ActNo is not null and a.MDate<= " & TIMS.to_date(sdate) & " order by a.MDate desc "
            'Dim dt As New DataTable
            'Dim cmd As New SqlCommand(sqlStr, tmpConn)
            'With cmd
            '    .Parameters.Clear()
            '    dt.Load(.ExecuteReader())
            'End With
...
#End Region
    ...
    Private Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        If OCIDValue1.Value <> "" Then
            ...
            Show_DataGrid(Datagrid2, odt, "SOCID")
