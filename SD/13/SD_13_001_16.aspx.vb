...
#End Region
    ...
    Sub sUtl_Search1()
        ...
        '除數可能有溢位問題，無條件捨去餘2位數。
        sqlstr &= "        ,FLOOR(ISNULL(b.TotalCost,0)/ISNULL(b.TNum,1)) Total ,b.TotalCost TotalCostX " & vbCrLf
        ...
        sqlstr &= "        ,e.DegreeID ,d.StudStatus ,d.MIdentityID ,a.STDate ,a.AppliedResultM ,d.AppliedResult " & vbCrLf
        sqlstr &= "        ,g.SOCID Exist ,g.SumOfMoney ,g.PayMoney ,g.AppliedStatus ,g.AppliedNote ,g.SupplyID " & vbCrLf
        '其他申請中金額
        ...
        sqlstr &= "  ) f ON f.SOCID = d.SOCID " & vbCrLf
        ...
        Select Case e.Item.ItemType
            ...
            Case ListItemType.Item, ListItemType.AlternatingItem
                ...
                If drv("GovCnt") = 0 Then 'NUMBER
                    star1.Text = "*" '未填寫(意見調查表)
                    Flag3 = 0 '未填寫(意見調查表)
                End If
                If Flag3 = 0 Then TIMS.Tooltip(star1, "未填寫調查表。") 'GovCnt
                stud_1.Text = drv("StudentID").ToString '學號
                BudID = TIMS.Get_Budget(BudID, 2)
                '沒有公務可用。
                If Not BudID.Items.FindByValue("01") Is Nothing Then
    ...
    Sub SaveData1()
        ...
        sql = "" & vbCrLf
        sql &= " insert into STUD_SUBSIDYCOST(SOCID,SUMOFMONEY,PAYMONEY,SUPPLYID,BUDID,MODIFYACCT,MODIFYDATE)" & vbCrLf
        sql &= " values (@SOCID,@SUMOFMONEY,@PAYMONEY,@SUPPLYID,@BUDID,@MODIFYACCT,getdate())" & vbCrLf
        Dim iCmd As New SqlCommand(sql, oConn, oTrans)

        sql = "" & vbCrLf
        sql &= " UPDATE STUD_SUBSIDYCOST" & vbCrLf
        sql &= " SET SUMOFMONEY=@SUMOFMONEY" & vbCrLf
        sql &= " ,PAYMONEY=@PAYMONEY" & vbCrLf
        sql &= " ,SUPPLYID=@SUPPLYID" & vbCrLf
        sql &= " ,BUDID=@BUDID" & vbCrLf
        sql &= " ,MODIFYACCT=@MODIFYACCT" & vbCrLf
        sql &= " ,MODIFYDATE=getdate()" & vbCrLf
        sql &= " WHERE 1=1" & vbCrLf
        sql &= " AND SOCID=@SOCID" & vbCrLf
        Dim uCmd As New SqlCommand(sql, oConn, oTrans)

        sql = "" & vbCrLf
        sql &= " DELETE STUD_SUBSIDYCOST" & vbCrLf
        sql &= " WHERE 1=1" & vbCrLf
        sql &= " AND SOCID=@SOCID" & vbCrLf
        Dim dCmd As New SqlCommand(sql, oConn, oTrans)
