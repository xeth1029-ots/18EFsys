...
#Region "Functions"
    ...
    Private Function Check_IsClosed(ByVal tmpID As Integer) As Boolean
        ...
        With cmd
            .Parameters.Clear()
            .Parameters.Add("OCID", SqlDbType.Int).Value = tmpID
            dt.Load(.ExecuteReader())
            If dt.Rows.Count > 0 Then
                If dt.Rows(0)("IsClosed") = "Y" Then rst = True
            End If
        End With
        Return rst
    End Function
...
#End Region
    ...
    Sub sUtl_Search1()
        ...
        If Check_IsClosed(OCIDValue1.Value) = True Then
            ...
            '判斷是否填寫問卷 (Stud_QuestionFac)
            sqlstr &= "        ,dbo.FN_GET_GOVCNT(d.SOCID) GovCnt ,e.Name ,e.IDNO ,d.CreditPoints " & vbCrLf
            '除數可能有溢位問題，無條件捨去餘2位數。
            ...
            sqlstr &= "        ,b.TotalCost TotalCostX ,c.Total2 TotalX ,b.TNum ,a.THours ,ar.DistID ,ISNULL(f.CountHours,0) CountHours " & vbCrLf
            sqlstr &= "        ,e.DegreeID ,d.StudStatus ,d.MIdentityID ,a.STDate ,a.AppliedResultM ,d.AppliedResult ,g.SOCID Exist " & vbCrLf
            sqlstr &= "        ,g.SumOfMoney ,g.PayMoney ,g.AppliedStatus ,g.AppliedNote ,g.SupplyID " & vbCrLf
            '其他申請中金額
            ...
            sqlstr &= " LEFT JOIN ( " & vbCrLf
            sqlstr &= "    SELECT t.SOCID ,Sum(t.Hours) CountHours " & vbCrLf
            sqlstr &= "    FROM Stud_Turnout2 t" & vbCrLf
            sqlstr &= "    JOIN Class_StudentsOfClass cs ON cs.socid = t.socid " & vbCrLf
            sqlstr &= "    WHERE 1=1 AND cs.ocid = '" & OCIDValue1.Value & "' " & vbCrLf
            sqlstr &= "    GROUP BY t.SOCID " & vbCrLf
            sqlstr &= " ) f ON d.SOCID = f.SOCID " & vbCrLf
            ...
            Try
                dt = DbAccess.GetDataTable(sqlstr, objconn)  'SQL
...
#Region "(No Use)"
            ...
            '        Dim flagCanAdd As Boolean = False '不可增加。
            '        flagCanAdd = False '不可增加1筆資料。
            '        If txtIDNO <> "" Then
            '            If txtIDNO.IndexOf(Convert.ToString(dt_Blacklist.Rows(i)("IDNO"))) = -1 Then
            '                flagCanAdd = True '可增加1筆資料。
            '            End If
            '        Else
            '            flagCanAdd = True '可增加1筆資料。
            '        End If
    ...
    Private Sub DataGrid1_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles DataGrid1.ItemDataBound
        ...
        Select Case e.Item.ItemType
            ...
            Case ListItemType.Item, ListItemType.AlternatingItem
                ...
                Dim Flag3 As Integer = 0  '填寫調查表 0:未填寫調查表 1:有填寫。
                ...
                '-------------
                Dim star1 As TextBox = e.Item.FindControl("star1") '未填寫調查表
                ...
                If drv("ESupplyID").ToString <> "" Then
                    Common.SetListItem(SupplyID, drv("ESupplyID").ToString)
                ...
                star1.Text = "" '已填寫調查表 
                Flag3 = 1 '已填寫調查表
                If drv("GovCnt") = 0 Then 'NUMBER
                    star1.Text = "*" '未填寫調查表
                    Flag3 = 0 '未填寫調查表
                End If
                If Flag3 = 0 Then TIMS.Tooltip(star1, "未填寫調查表。")
                stud_1.Text = drv("StudentID").ToString '學號
                BudID = TIMS.Get_Budget(BudID, 2)
                '沒有公務可用。
                If Not BudID.Items.FindByValue("01") Is Nothing Then If drv("DistID").ToString <> "001" Then BudID.Items.Remove(BudID.Items.FindByValue("01"))
                ...
                Else
                    ...
                    If Not e.Item.Cells(cst_申請狀態).Text = "審核通過" Then
                        ...
                        End If
                        'totalleft = Total  '20091221 andy 
                        If drv("GovAppl2") > Total - CInt(SumOfMoney.Text) Then e.Item.Cells(cst_其他申請中金額).Text = "<font color=Red>" & drv("GovAppl2").ToString & "</font>"
                    Else
                        ...
                        End If
                        'totalleft = Total  '20091221 andy 
                        If drv("GovAppl2") > Total Then e.Item.Cells(cst_其他申請中金額).Text = "<font color=Red>" & drv("GovAppl2").ToString & "</font>"
                    End If
                End If
                ...
                '或 未填寫調查表。201407 AMU
                If Flag = 0 OrElse FlagStudy = 0 OrElse Flag3 = 0 Then
                    Checkbox1.Disabled = True
                    Checkbox1.Checked = False
                    e.Item.Cells(cst_是否補助).Text = "<font color='RED'>否</font>"
                    SumOfMoney.Enabled = False '不可填入補助費用
                ...
                If gsBlackIDNO <> "" AndAlso gsBlackIDNO.IndexOf(Convert.ToString(drv("IDNO"))) > -1 Then
                    ...
                    BudID.Enabled = False
                    hidBlackMsg.Value += "學號" + stud_1.Text + "." + drv("IDNO") + " " + drv("Name") + "已受處分" & vbCrLf '加入單名單暫存(2009/07/28 判斷黑名單)
                End If
                'END 黑名單為不補助鎖定特定選項
                ...
                If Checkbox1.Disabled = False Then
                    If Checkbox1.Checked = True Then
                        ...
                        If dt.Select(ff).Length = 0 Then
                            dr = dt.NewRow()
                            dt.Rows.Add(dr)
                            dr("SOCID") = iSOCID  'DataGrid1.DataKeys(item.ItemIndex)
                        ElseIf dt.Select(ff).Length = 1 Then
                            dr = dt.Select(ff)(0)
                        End If
                    ...
                    Else
                        '執行刪除一筆資料。
                        If dt.Select(ff).Length <> 0 Then
                            dt.Select(ff)(0).Delete()
                            delFlag = True
                        End If
                    End If
                End If
                '20080717  Andy  不補助則刪除
                If Not delFlag AndAlso Checkbox1.Disabled = True AndAlso Checkbox1.Checked = False Then
                    If dt.Select(ff).Length <> 0 Then
                        dt.Select(ff)(0).Delete()
                        delFlag = True
                    End If
