<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="SD_03_002_add.aspx.vb" Inherits="WDAIIP.SD_03_002_add" %>
...
<head>
    ...
    <script type="text/javascript">
        ...
        function open_SD01001sch() {
            ...
            var url1 = "../01/SD_01_001_sch.aspx?ID=" + rqID + "&SPAGE=SD03002&IDNO=" + IDNO.value + "&BIRTH=" + birthday.value + "&CNAME=" + escape(Name.value);
        ...
        function open_SD01001sch2() {
            ...
            /* Dim rqOCID As String = TIMS.ClearSQM(Request("OCID")) Dim rqSTDATE As String = TIMS.ClearSQM(Request("STDATE")) */
            var url1 = "../01/SD_01_001_sch2.aspx?ID=" + rqID + "&SPAGE=SD03002&IDNO=" + IDNO.value + "&BIRTH=" + birthday.value
                + "&CNAME=" + escape(Name.value) + "&OCID=" + Hid_OCID.value + "&STDATE=" + escape(STDateHidden.value);
        ...
        function chgPriorWorkType1_disabled() {
            ...
            //lab2017_2.innerHTML = '工作<br />起迄日期'; //lab2017_4.innerHTML = '工作薪資';
            if (BtnCheckBli) { BtnCheckBli.style.display = cst_none; }
            //spnStartOrg.style.display = cst_inline; //spnStartDate.style.display = cst_inline; //spnStartNo.style.display = cst_inline;
            ...
            switch (getRadioValue(document.form1.PriorWorkType1)) {
                case '1':
                    ...
                    //PriorWorkOrg1.style.display = cst_none; //ActNo.style.display = cst_none;
                    break;
                case '2':
                    //PriorWorkOrg1.disabled = true; //ActNo.disabled = true;
                    //SOfficeYM1.disabled = true; //FOfficeYM1.disabled = true;
                ...
                case '3':
                    ...
                    //PriorWorkOrg1.style.display = cst_none; //PriorWorkOrg2.style.display = cst_none;
                    //spnPriorWorkOrg1.style.display = cst_none; //spnPriorWorkOrg2.style.display = cst_none;
                    ActNo2.style.display = cst_none;
                    //ActNo.disabled = true;
                    break;
                case '4':
        ...
        function EnterChannelChange() {
            // alert("EnterChannelChange。"); //debugger;
            ...
            //有現場報名資料，網路可改現場報名 //'0.沒有有現場報名'1.有現場報名
        ...
        /* //TIMSC-272 //推介種類 function TRNDModeChange() { //debugger;// alert("TRNDModeChange。");
		var TRNDType = document.form1.TRNDType; for (var i = 0; i < TRNDType.length; i++) { TRNDType[i].checked = false; }
		if (document.form1.TRNDMode.selectedIndex != 0) { //if(document.form1.TRNDMode.value=='1' || document.form1.TRNDMode.value=='3'){
		if (document.form1.TRNDMode.value == '1') { for (var i = 0; i < document.form1.TRNDType.length; i++) { document.form1.TRNDType[i].disabled = false; }  }
		else { for (var i = 0; i < document.form1.TRNDType.length; i++) { document.form1.TRNDType[i].disabled = true; } } } } */

        // update MenuTable->UMenuTable //('在職者顯示 MenuTable / BackTable) 改變id 讓 chkdata 找到 BackTable,MenuTable
        ...
        //就職狀況改變是否為在職者補助 46 / 47
        function ChangeWorkSuppIdent(num) {
            var msg = '';
            var Item = '';
            var Page = 1;
            var int_3Y_SupplyMoney = 0; //'產投 政府補助經費 (3年)
            var str_3Y_SupplyMoney = ''; //'產投 政府補助經費 (3年)
            var SubsidyCost4X = 0; //是否為在職者補助可補助經費
            //'產投 政府補助經費 (3年)
            int_3Y_SupplyMoney = parseInt(document.form1.hide_3Y_SupplyMoney.value, 10);
            str_3Y_SupplyMoney = document.form1.hide_3Y_SupplyMoney.value.replace("0000", "萬 ");
            //目前 在職者已補助經費s
            //document.getElementById('SubsidyCost').innerhtml
            var SubsidyCostVal = 0;
            var TPlanID = document.getElementById('TPlanID');
            if (document.getElementById("SubsidyCost")) {
                if (document.getElementById('SubsidyCost').innerText != '') {
                    SubsidyCostVal = parseInt(document.getElementById('SubsidyCost').innerText, 10);
                }
            }
            var cst_pt1 = 0;
            var cst_pt2 = 1;
            ...
            if (num != undefined) {
                if (IrblWorkSuppIdent) {
                    if (NrblWorkSuppIdent[cst_pt1].checked == true && getRadioValue(document.form1.JobStateType) == 0) {
                        NrblWorkSuppIdent[cst_pt1].checked = false;
                        NrblWorkSuppIdent[cst_pt2].checked = true;
                        msg += "就職狀況選擇為失業時，(是否為在職者補助)不可為「是」";
                        if (Item == '') Item = 'JobStateType'; Page = 1;
                    }
                    if (document.getElementById("rblWorkSuppIdent").disabled == false) {
                        if (NrblWorkSuppIdent[cst_pt1].checked == true) {
                            //('在職者顯示 MenuTable / BackTable) 改變id 讓 chkdata 找到 BackTable,MenuTable
                            Change_MBTable(1);
                        }
                        if (NrblWorkSuppIdent[cst_pt2].checked == true) {
                            //('失業者不顯示 MenuTable / BackTable)
                            Change_MBTable(2);
                        }
                    }
                ...
                if (NrblWorkSuppIdent) {
                    if (getRadioValue(document.form1.JobStateType) == 1) {
                    ...
                    if (msg == '') {
                        if (document.form1.MIdentityID.value == '01') {
                            if (TPlanID.value == '46') { SubsidyCost4X = 6000; }
                            if (TPlanID.value == '47') { SubsidyCost4X = 5000; }
                        }
                        else {
                            if (TPlanID.value == '46') { SubsidyCost4X = 8000; }
                            if (TPlanID.value == '47') { SubsidyCost4X = 8000; }
                        }
                        SubsidyCost4X = parseInt(SubsidyCost4X, 10) + parseInt(SubsidyCostVal, 10);
                    }
                    //('失業者不顯示 MenuTable / BackTable)
                    Change_MBTable(2);
                    if (msg == '') {
                        //document.getElementById("rblWorkSuppIdent").disabled=false;
                        if (getRadioValue(document.form1.JobStateType) == 1) {
                            if (SubsidyCost4X > int_3Y_SupplyMoney) {
                                //document.getElementById("rblWorkSuppIdent").disabled=true; //document.getElementsByName('rblWorkSuppIdent')[1].checked=false;
                                NrblWorkSuppIdent[cst_pt2].checked = true;
                                msg += '在職者補助合計為' + SubsidyCost4X + '，超過3年' + str_3Y_SupplyMoney + '補助額度\n'; if (Item == '') Item = 'JobStateType'; Page = 1;
                            }
                            else {
                                //('在職者顯示 MenuTable / BackTable) 改變id 讓 chkdata 找到 BackTable,MenuTable
                                Change_MBTable(1);
                                //document.getElementById("rblWorkSuppIdent").disabled=false; //document.getElementsByName('rblWorkSuppIdent')[2].checked=false;
                                NrblWorkSuppIdent[cst_pt1].checked = true;
                            }
                        }
                        if (getRadioValue(document.form1.JobStateType) == 0) {
                            //document.getElementById("rblWorkSuppIdent").disabled=true; //document.getElementsByName('rblWorkSuppIdent')[1].checked=false;
                            NrblWorkSuppIdent[cst_pt2].checked = true;
                        }
                    }
            ...
            if (isChecked(document.getElementsByName('AcctMode'))) {
                switch (getRadioValue(document.getElementsByName('AcctMode'))) {
                    ...
                    case '1':
                        ...
                        break;
                        //case '2': //如果是勞工團體會有value=2的情況，不過因為不需要有輸入框，所以等於可以不用作任何判斷處理                                                                                                                                                                                                                               
                }
            }
            if (document.body) { window.scroll(0, document.body.scrollHeight); }
            if (!_isIE) { if (parent && parent.setMainFrameHeight() != undefined) { parent.setMainFrameHeight(); } }
        ...
        function chkdata() {
            ...
            if (document.form1.LevelNo.disabled == false && document.form1.LevelNo.selectedIndex == 0) { msg += '請選擇報名階段\n'; if (Item == '') Item = 'LevelNo'; Page = 1; }
            ...
            if (document.getElementById('CheckBox3').checked == true) {
                if (document.getElementById('CheckBox1').checked == true) {
                    ...
                    //checkzip23 郵遞區號 msg += checkzip23(true, '戶籍地址', 'ZipCode2_B3');
            ...
            if (!flagTPlan_28) {
                ...
                switch (getRadioValue(document.form1.PriorWorkType1)) {
                    case '1':
						...
						*/
                        ...
                        /*,if (tdRealJobless && tdRealJobless.style.color == "red") {,if (RealJobless && RealJobless.value == '' && JoblessID && JoblessID.selectedIndex == 0) {
                        ,if (WorkSuppIdentVal != "Y") { msg += '請輸入受訓前失業週數\n'; if (Item == '') Item = 'RealJobless'; Page = 1; },},else {,if (WorkSuppIdentVal != "Y") {
                ...
                //if (document.form1.PriorWorkType1[0].checked) {} //投保單位<br />加退保日期 //投保單位<br />加退保日期
                if (SOfficeYM1.value != '' && FOfficeYM1.value != '' && FOfficeYM1.value < SOfficeYM1.value) { msg += '[ 加退保日期 的迄日]必需大於[ 加退保日期 的起日]\n'; if (Item == '') Item = 'FOfficeYM1'; Page = 1; }
                ...
                Item = 'JoblessID'; Page = 1; },if (document.form1.RealJobless.value != '' && !isUnsignedInt(document.form1.RealJobless.value)) { msg += '失業週數必須為數字\n'; 
                if (Item == '') Item = 'RealJobless'; Page = 1; },},},},*/
                ...
                /* if (document.form1.MIdentityID.value == '05') { if (document.form1.NativeID.selectedIndex == 0) { msg += '請選擇民族別\n'; 
                if (Item == '') Item = 'NativeID'; Page = 1; } } */
            }
            else {
                //debugger; //alert(' ZipCode3=['+ document.getElementById('ZipCode3').value +']\n EmergencyAddress=['+document.getElementById('EmergencyAddress').value+']\n EmergencyAddress=['+document.getElementById('EmergencyAddress').value+']');
                ...
                if (document.form1.CheckBox2.checked == false && document.form1.CheckBox3.checked == false) {
                    ...
                    if ($('#ZipCode3').val() != "" || $('#ZipCode3_B3').val() != "" || $('#EmergencyAddress').val() != "") {
                        //checkzip23 郵遞區號 msg += checkzip23(true, '緊急通知人地址', 'ZipCode3_B3');
                        if ($('#ZipCode3_B3').val() == '') { msg += '緊急通知人地址項若有填值時，必須填寫完整緊急通知人地址(郵遞區號後3碼/後2碼)\n'; if (Item == '') Item = 'ZipCode3_B3'; Page = 1; }
                        if ($('#EmergencyAddress').val() == '') { msg += '緊急通知人地址項若有填值時，必須填寫完整緊急通知人地址\n'; if (Item == '') Item = 'EmergencyAddress'; Page = 1; }
                    }
                    if ($('#ZipCode3_B3').val() != '') {
                        //checkzip23 郵遞區號
                        msg += checkzip23(true, '緊急通知人地址', 'ZipCode3_B3');
            ...
            /*end*/
            /* for(i=0,j=0;i<document.form1.PassPortNO.length;i++){ if (!document.form1.PassPortNO[i].checked) j++; } ,所有計畫都應該填寫下列資訊。 */
            ...
            /* for(i=0,j=0;i<document.form1.MaritalStatus.length;i++){ if (!document.form1.MaritalStatus[i].checked) j++;
			...
			*/
            //debugger;
            if (document.getElementById('ZipCode1').value == '') { msg += '請輸入通訊地址前3碼郵遞區號\n'; if (Item == '') Item = 'City1'; Page = 1; }
            else {
                if (trim(document.getElementById('ZipCode1').value).length != 3) { msg += '通訊地址前3碼郵遞區號必須為3碼\n'; if (Item == '') Item = 'ZipCode1'; Page = 1; }
            }
            //if (document.form1.ZipCode1.value=='') {msg+='請輸入通訊地址(區域)\n';if(Item=='') Item='City1';Page=1;}
            //20090520 add --begin
            ...
            if (trim(document.getElementById('ZipCode1_B3').value) == '') {
                msg += '請輸入通訊地址郵遞區號後2碼\n'; if (Item == '') Item = 'ZipCode1_B3'; Page = 1;
            } else {
                //checkzip23 郵遞區號
                msg += checkzip23(true, '通訊地址', 'ZipCode1_B3');
                //if (!isUnsignedInt(trim(document.getElementById('ZipCode1_B3').value))) { msg += '通訊地址郵遞區號後2碼必須為數字，且不得輸入 00\n'; if (Item == '') Item = 'ZipCode1_B3'; Page = 1; }
                //if (parseInt(trim(document.getElementById('ZipCode1_B3').value), 10) < 1) { msg += '通訊地址郵遞區號後2碼必須為數字，得輸入 01~99 \n'; if (Item == '') Item = 'ZipCode1_B3'; Page = 1; }
                //if (parseInt(trim(document.getElementById('ZipCode1_B3').value), 10) > 99) { msg += '通訊地址郵遞區號後2碼必須為數字，得輸入 01~99 \n'; if (Item == '') Item = 'ZipCode1_B3'; Page = 1; }
                //if (trim(document.getElementById('ZipCode1_B3').value).length != 2) { msg += '通訊地址郵遞區號後2碼長度必須為 2 碼(例 01 或 99)\n'; if (Item == '') Item = 'ZipCode1_B3'; Page = 1; }
            }
            //20090520 add --end
            if (document.form1.Address.value == '') { msg += '請輸入通訊地址\n'; if (Item == '') Item = 'Address'; Page = 1; }
            if (document.form1.CheckBox1.checked == false) {
                if (document.form1.ZipCode2.value == '') { msg += '請輸入戶籍地址(區域)\n'; if (Item == '') Item = 'City2'; Page = 1; }
                    ...
                    if (trim(document.form1.ZipCode2.value).length != 3) { msg += '戶籍地址前3碼郵遞區號必須為3碼\n'; if (Item == '') Item = 'ZipCode2'; Page = 1; }
                }
                if (trim(document.getElementById('ZipCode2_B3').value) == '') {
                    msg += '請輸入戶籍地址郵遞區號後2碼\n'; if (Item == '') Item = 'ZipCode2_B3'; Page = 1;
                } else {
                    //checkzip23 郵遞區號
                    msg += checkzip23(true, '戶籍地址', 'ZipCode2_B3');
                    //if (!isUnsignedInt(trim(document.getElementById('ZipCode2_B3').value))) { msg += '戶籍地址郵遞區號後2碼必須為數字，且不得輸入 00\n'; if (Item == '') Item = 'ZipCode2_B3'; Page = 1; }
                    //if (parseInt(trim(document.getElementById('ZipCode2_B3').value), 10) < 1) { msg += '戶籍地址郵遞區號後2碼必須為數字，得輸入 01~99 \n'; if (Item == '') Item = 'ZipCode2_B3'; Page = 1; }
                    //if (parseInt(trim(document.getElementById('ZipCode2_B3').value), 10) > 99) { msg += '戶籍地址郵遞區號後2碼必須為數字，得輸入 01~99 \n'; if (Item == '') Item = 'ZipCode2_B3'; Page = 1; }
                    //if (trim(document.getElementById('ZipCode2_B3').value).length != 2) { msg += '戶籍地址郵遞區號後2碼長度必須為 2 碼(例 01 或 99)\n'; if (Item == '') Item = 'ZipCode2_B3'; Page = 1; }
                }
                if (document.form1.HouseholdAddress.value == '') { msg += '請輸入戶籍地址\n'; if (Item == '') Item = 'HouseholdAddress'; Page = 1; }
            }
            if (document.form1.Email.value == '') { msg += '請輸入Email\n'; if (Item == '') Item = 'Email'; Page = 1; }
            if (document.form1.Email.value != '' && !checkEmail(document.form1.Email.value) && document.form1.Email.value != '無') { msg += '請輸入正確的E-mail格式\n'; if (Item == '') Item = 'Email'; Page = 1; }
            var j = 0;
            ...
            //if (document.form1.SupplyID.disabled==false) //if (document.form1.SupplyID.selectedIndex==0) {msg+='請選擇補助比例\n';if(Item=='') Item='SupplyID';Page=1;}
            if (rdo_HighEduBg) {
            ...
            if (document.getElementById('PMode') && TPlanID.value == '12') {
            ...
            //if (GetID == "") { return false; } //var NrblHandType = document.getElementsByName('rblHandType'); //var IrblHandType = document.getElementById('rblHandType');
            ...
            //企訓專用('產學訓計畫才會顯示 MenuTable / BackTable)
            var chk_BackTable = false;
            if (TPlanID.value == '28') {
                if (document.getElementById('BackTable')) { chk_BackTable = true; }
            }
            else {
                if (document.getElementById('BackTable') && document.getElementById('BackTable').style.display != 'none') { chk_BackTable = true; }
            }

            var ChkbudIdValueNot99 = true;
            if (BudID && isChecked(document.form1.BudID)) {
                var supplyIdValue = $("#SupplyID").val();
                // 取得被選取的 radio button 的值
                var budIdValue = $("input[name='BudID']:checked").val();
                //debugger; //9：補助比例 0% //BudID：99：不補助 //學員資料維護卡控調整
                if (supplyIdValue == "9" && budIdValue == "99") { ChkbudIdValueNot99 = false; }
            }

            if (chk_BackTable) {
                // document.getElementById('BackTable').style.display = 'none'
                if (ChkbudIdValueNot99) {
                    if (!isChecked(document.getElementsByName('AcctMode'))) {
                        msg += '請輸入郵政或銀行帳號\n'; if (Item == '') { Item = 'AcctMode'; Page = 2; }
                    }
                    else {
                        //如果是勞工團體AcctMode=2的話，就不需要有任何輸入值，所以也不需要任何判斷
                        if (getRadioValue(document.getElementsByName('AcctMode')) == '0') {
                            ...
                            //if (document.getElementById('PostNo_2').value != '' && !isUnsignedInt(document.getElementById('PostNo_2').value)) { msg += '局號必須為數字\n'; if (Item == '') Item = 'PostNo_2'; Page = 2; }
                            if (document.getElementById('PostNo_1').value == '') { msg += '請輸入局號\n'; if (Item == '') { Item = 'PostNo_1'; Page = 2; } }
                            if (document.getElementById('PostNo_1').value != '' && !isUnsignedInt(document.getElementById('PostNo_1').value)) { msg += '局號必須為數字\n'; if (Item == '') Item = 'PostNo_1'; Page = 2; }
                            ...
                            //if (document.getElementById('AcctNo1_2').value != '' && !isUnsignedInt(document.getElementById('AcctNo1_2').value)) { msg += '帳號必須為數字\n'; if (Item == '') Item = 'AcctNo1_2'; Page = 2; }
                            if (document.getElementById('AcctNo1_1').value == '') { msg += '請輸入帳號\n'; if (Item == '') { Item = 'AcctNo1_1'; Page = 2; } }
                            if (document.getElementById('AcctNo1_1').value != '' && !isUnsignedInt(document.getElementById('AcctNo1_1').value)) { msg += '帳號必須為數字\n'; if (Item == '') Item = 'AcctNo1_1'; Page = 2; }
                        }
                        else if (getRadioValue(document.getElementsByName('AcctMode')) == '1') {
                            if (document.getElementById('BankName').value == '') { msg += '請輸入總行名稱\n'; if (Item == '') { Item = 'BankName'; Page = 2; } }
                            if (document.getElementById('AcctheadNo').value == '') { msg += '請輸入總行代號\n'; if (Item == '') { Item = 'AcctheadNo'; Page = 2; } }
                            if (document.getElementById('AcctheadNo').value != '' && !isUnsignedInt(document.getElementById('AcctheadNo').value)) { msg += '總行代號必須為數字\n'; if (Item == '') Item = 'AcctheadNo'; Page = 2; }
                            if (document.all("ExBankName") != null) {
                                if (document.getElementById('ExBankName').value == '') { msg += '請輸入分行名稱\n'; if (Item == '') { Item = 'ExBankName'; Page = 2; } } //amu 20061225 kevin同意再次加入
                            }
                            if (document.all("AcctExNo") != null) {
                                if (document.getElementById('AcctExNo').value == '') { msg += '請輸入分行代號\n'; if (Item == '') { Item = 'AcctExNo'; Page = 2; } } //amu 20061225 kevin同意再次加入
                                if (document.getElementById('AcctExNo').value != '' && !isUnsignedInt(document.getElementById('AcctExNo').value)) { msg += '分行代號必須為數字\n'; if (Item == '') Item = 'AcctExNo'; Page = 2; }
                            }
                            if (document.getElementById('AcctNo2').value == '') { msg += '請輸入帳號\n'; if (Item == '') { Item = 'AcctNo2'; Page = 2; } }
                            if (document.getElementById('AcctNo2').value != '' && !isUnsignedInt(document.getElementById('AcctNo2').value)) { msg += '帳號必須為數字\n'; if (Item == '') Item = 'AcctNo2'; Page = 2; }
                        }
                    }
                }

                if (document.getElementById('FirDate').value != '' && !checkDate(document.getElementById('FirDate').value)) {
                    msg += '第一次投保日期不是正確的日期格式\n'; if (Item == '') { Item = 'FirDate'; Page = 2; }
                }
				...
				*/
                ...
                if (!(flagTPlan_28 && parseInt(document.getElementById('hide_Years').value, 10) > 2008 && document.form1.MIdentityID.value == '02')) {
                    if (ChkbudIdValueNot99 && document.getElementById('ActName').value == '') { msg += '請輸入投保單位名稱\n'; if (Item == '') { Item = 'ActName'; Page = 2; } }
                    if (ChkbudIdValueNot99 && document.getElementById('ActNo1').value == '') { msg += '請輸入投保單位保險證號\n'; if (Item == '') { Item = 'ActNo1'; Page = 2; } }
                    ...
                    //debugger;
                    //20090520 add 投保單位地址郵遞區號後2碼 --begin
                    if (trim(document.getElementById('txt_ActZIPB3').value) != '') {
                        //checkzip23 郵遞區號
                        msg += checkzip23(true, '投保單位地址', 'txt_ActZIPB3');
                        //if (!isUnsignedInt(trim(document.getElementById('txt_ActZIPB3').value))) { msg += '投保單位地址郵遞區號後2碼必須為數字，且不得輸入 00\n'; if (Item == '') { Item = 'txt_ActZIPB3'; Page = 2; } }
                        //if (parseInt(trim(document.getElementById('txt_ActZIPB3').value), 10) < 1) { msg += '投保單位地址郵遞區號後2碼必須為數字，得輸入 01~99 \n'; if (Item == '') Item = 'txt_ActZIPB3'; Page = 1; }
                        //if (parseInt(trim(document.getElementById('txt_ActZIPB3').value), 10) > 99) { msg += '投保單位地址郵遞區號後2碼必須為數字，得輸入 01~99 \n'; if (Item == '') Item = 'txt_ActZIPB3'; Page = 1; }
                        //if (trim(document.getElementById('txt_ActZIPB3').value).length != 2) { msg += '投保單位地址郵遞區號後2碼長度必須為 2 碼(例 01 或 99)\n'; if (Item == '') { Item = 'txt_ActZIPB3'; Page = 2; } }
                    }
                    //20090520 add 投保單位地址郵遞區號後2碼 --end
                    ...
                    if (!document.getElementById('Checkbox4').checked) {
                        ...
                        //if(trim(document.getElementById('Zip').value).length!=3) {msg+='服務單位公司地址[前3碼郵遞區號]必須為3碼\n';if(Item=='') {Item='City5';Page=2;}}
                        //20090520 add 服務單位公司郵遞區號後2碼 --begin
                        if (trim(document.getElementById('ZIPB3').value) != '') {
                            //checkzip23 郵遞區號
                            msg += checkzip23(true, '服務單位公司地址', 'ZIPB3');
                            //if (!isUnsignedInt(trim(document.getElementById('ZIPB3').value))) { msg += '服務單位公司地址郵遞區號後2碼必須為數字，且不得輸入 00\n'; if (Item == '') { Item = 'ZIPB3'; Page = 2; } }
                            //if (parseInt(trim(document.getElementById('ZIPB3').value), 10) < 1) { msg += '服務單位公司地址郵遞區號後2碼必須為數字，得輸入 01~99 \n'; if (Item == '') Item = 'ZIPB3'; Page = 1; }
                            //if (parseInt(trim(document.getElementById('ZIPB3').value), 10) > 99) { msg += '服務單位公司地址郵遞區號後2碼必須為數字，得輸入 01~99 \n'; if (Item == '') Item = 'ZIPB3'; Page = 1; }
                            //if (trim(document.getElementById('ZIPB3').value).length != 2) { msg += '服務單位公司地址郵遞區號後2碼長度必須為 2 碼(例 01 或 99)\n'; if (Item == '') { Item = 'ZIPB3'; Page = 2; } }
                        }
                        //20090520 add 服務單位公司郵遞區號後2碼 --end
                        //if (document.getElementById('Addr').value == '') { msg += '請輸入服務單位公司地址\n'; if (Item == '') { Item = 'Addr'; Page = 2; } }
                    }
                    if (ChkbudIdValueNot99 && document.getElementById('Uname').value == '') { msg += '請輸入目前任職公司名稱\n'; if (Item == '') { Item = 'Uname'; Page = 2; } }
                    ...
                    if (parseInt(getCheckBoxListValue('Q2'), 10) == 0) { msg += '請選擇參訊動機\n'; if (Item == '') { Page = 2; } }
                    //if (! (document.getElementById('TPlanID').value=='28'  && parseInt(document.getElementById('hide_Years').value)>2008 && document.form1.MIdentityID.value=='02'))
                    if (!(flagTPlan_28 && parseInt(document.getElementById('hide_Years').value, 10) > 2008 && document.form1.MIdentityID.value == '02')) {
                        if (document.getElementById('Q4').selectedIndex == 0) { msg += '請選擇服務單位行業別\n'; if (Item == '') { Item = 'Q4'; Page = 2; } }
                    }
                    if (document.getElementById('Q61').value == '') { msg += '個人工作年資必須為數字\n'; if (Item == '') { Item = 'Q61'; Page = 2; } }
                    if (document.getElementById('Q62').value == '') { msg += '在這家公司的年資必須為數字\n'; if (Item == '') { Item = 'Q62'; Page = 2; } }
                    if (document.getElementById('Q63').value == '') { msg += '在這職位的年資必須為數字\n'; if (Item == '') { Item = 'Q63'; Page = 2; } }
                    if (document.getElementById('Q64').value == '') { msg += '最近升遷離本職幾年必須為數字\n'; if (Item == '') { Item = 'Q64'; Page = 2; } }
                    // if (document.getElementById('Q61').value != '' && !isUnsignedInt(document.getElementById('Q61').value)) { msg += '個人工作年資必須為數字\n'; if (Item == '') { Item = 'Q61'; Page = 2; } }
                    // if (document.getElementById('Q62').value != '' && !isUnsignedInt(document.getElementById('Q62').value)) { msg += '在這家公司的年資必須為數字\n'; if (Item == '') { Item = 'Q62'; Page = 2; } }
                    // if (document.getElementById('Q63').value != '' && !isUnsignedInt(document.getElementById('Q63').value)) { msg += '在這職位的年資必須為數字\n'; if (Item == '') { Item = 'Q63'; Page = 2; } }
                    // if (document.getElementById('Q64').value != '' && !isUnsignedInt(document.getElementById('Q64').value)) { msg += '最近升遷離本職幾年必須為數字\n'; if (Item == '') { Item = 'Q64'; Page = 2; } }
            ...
            if (PriorWorkOrg1) {
                PriorWorkOrg1.disabled = false;
                if (PriorWorkOrg2) { PriorWorkOrg2.disabled = false; }
                if (ActNo2) { ActNo2.disabled = false; }
                if (PriorWorkPay) { PriorWorkPay.disabled = false; }
            }
        }

        //MIdentityID:參訓身分別
        function MIdentityChg(IdentValue) {
            //console.log('MIdentityChg,IdentValue: ' + IdentValue);
            //var IdentityID = document.getElementById("IdentityID");
            var MIdentityID = document.getElementById("MIdentityID");
        ...
        /* function checkNativeID() {//debugger;var MIdentityID = document.form1.MIdentityID;var myTr1 = document.getElementById("NativeTr1");myTr1.style.display = 'none';
        if (MIdentityID.value == '05') {myTr1.style.display = cst_inline1;}} */
        ...
        function hard() {
            ...
            if (GetID != "" && document.getElementById(GetID).checked) {
                enableSelection();
                //document.form1.HandTypeID.disabled = false; //document.form1.HandLevelID.disabled = false; //document.form1.HandLevelID2.disabled = false;
            }
            else {
                disableSelection();
                //document.form1.HandTypeID.disabled = true; //document.form1.HandLevelID.disabled = true; //document.form1.HandLevelID2.disabled = true;
            }
            if (document.body) { window.scroll(0, document.body.scrollHeight); }
            if (!_isIE) { if (parent && parent.setMainFrameHeight() != undefined) { parent.setMainFrameHeight(); } }
        ...
        function ChangeMode(num) {
            ...
            var MenuTable_td_1 = $("#MenuTable_td_1");//DetailTable
            var MenuTable_td_2 = $("#MenuTable_td_2");//BackTable
            var MenuTable_td_3 = $("#MenuTable_td_3");//HistoryTable
            var MenuTable_td_4 = $("#MenuTable_td_4");//PassPtTable
            if (MenuTable_td_1.length > 0) { MenuTable_td_1.removeClass(); }
            if (MenuTable_td_2.length > 0) { MenuTable_td_2.removeClass(); }
            if (MenuTable_td_3.length > 0) { MenuTable_td_3.removeClass(); }
            if (MenuTable_td_4.length > 0) { MenuTable_td_4.removeClass(); }
            if (document.getElementById('DetailTable') && (document.getElementById('BackTable') || document.getElementById('UBackTable'))) {

                if (document.getElementById('DetailTable')) document.getElementById('DetailTable').style.display = 'none'; //MenuTable_td_1
                if (document.getElementById('BackTable')) document.getElementById('BackTable').style.display = 'none'; //MenuTable_td_2
                if (document.getElementById('HistoryTable')) document.getElementById('HistoryTable').style.display = 'none'; //MenuTable_td_3
                if (document.getElementById('PassPtTable')) document.getElementById('PassPtTable').style.display = 'none'; //MenuTable_td_4
                if (document.getElementById('ButtonTable4')) document.getElementById('ButtonTable4').style.display = 'none';

                switch (num) {
                    case 1:
                        MenuTable_td_1.addClass("active");
                        document.getElementById('DetailTable').style.display = cst_inline1;
                        document.getElementById('ButtonTable4').style.display = cst_inline1;
                        break;
                    case 2:
                        ...
                        if (TPlanID.value == '28' && parseInt(document.getElementById('hide_Years').value, 10) > 2008 && document.getElementById('hide_MIdentityID').value == '02') {
                            for (var i = 14; i <= 21; i++) {
                                switch (i) {
                                    case 14:
                                    case 15:
                    ...
                    case 4:
                        ...
                        if (document.getElementById('PassPtTable')) document.getElementById('PassPtTable').style.display = cst_inline1;
                        break;
                    default:
                        break;
                }
        ...
        //開啟日曆(回傳欄位,開訓日期,結訓日期,目前指定日期[可略],頁面相對位置[可略],回傳要觸發的按鈕[可略]) //在點選「出生日期」後，開啟日期的選窗
        function callCalendar(BirthdayID, ButtonID) {
            //console.log("BirthdayID : " + BirthdayID); //console.log("ButtonID : " + ButtonID);
            var oBirthdayID = document.getElementById(BirthdayID);
            if (!oBirthdayID) return;
            openCalendar(BirthdayID, '1911/01/01', '2099/12/31', oBirthdayID.value, '', ButtonID);
        }
        ...
        /*,function getDiffDate() {,var FOfficeYM1 = document.getElementById('FOfficeYM1');,var OpenDate = document.getElementById('OpenDate');
        ,var LostJobWeek = document.getElementById('LostJobWeek');,var LostJobDay = document.getElementById('LostJobDay');,if (OpenDate != null) {
        ,if (FOfficeYM1.value != '' && OpenDate.value != '') {,var sDT = new Date(FOfficeYM1.value);,var eDT = new Date(OpenDate.value);,var leaveDay = sDT.MydateDiff("d", eDT) % 7;
        ,if (LostJobWeek) { LostJobWeek.innerText = '' + sDT.MydateDiff("w", eDT); },if (LostJobDay) { LostJobDay.innerText = '' + leaveDay; },},},},*/
        ...
        // 同投保單位地址-公司地址
        function ock_CheckBox4() {
            var Checkbox4 = document.getElementById('Checkbox4');
            if (!Checkbox4) return;
            if (Checkbox4.checked == true) {
                $('#Zip').val($('#txt_ActZip').val());
                $('#ZIPB3').val($('#txt_ActZIPB3').val());
                $('#hidZIP6W').val($('#hid_ActZIP6W').val());
                $('#Zip_N').val($('#hidActZip_N').val());
                $('#City5').val($('#txt_ActCity').val());
                $('#Addr').val($('#txt_ActAddress').val());
            }
        }

        $(document).ready(function () {
            // 獲取所有具有 id 開頭為 "MenuTable_td_" 的 TD 元素
            const tdElements = document.querySelectorAll('td[id^="MenuTable_td_"]');
            // 遍歷每個 TD 元素，添加 keydown 事件監聽器
            tdElements.forEach(td => {
                ...
                /*td.addEventListener('keypress', (event) => {,if (event.key === ' ' || event.key === 'Enter') {,如果按下的是空格鍵，則觸發點擊事件 ,console.log('keypress:' + event.key + '.');,td.click();,},});*/
            });
        });
...
<body onload="Change_BudID();">
    <form id="form1" method="post" runat="server">
        <%--<table class="font" id="FrameTable" cellspacing="1" cellpadding="1" width="100%" border="0"><tr><td><asp:Label ID="TitleLab1" runat="server"></asp:Label><asp:Label ID="Titlelab2" runat="server">
           首頁&gt;&gt;學員動態管理&gt;&gt;學員資料管理&gt;&gt;<font color="#990000">學員資料維護</font></asp:Label></td></tr></table>--%>
        <%--<asp:Button ID="btnTest1" runat="server" Text="btnTest1" />--%>
        <table border="0" cellspacing="1" cellpadding="1" width="100%">
            <tr>
                <td>
                    <table class="font" id="MenuTable" cellspacing="0" cellpadding="0" style="width: 50%" runat="server">
                        <tr class="newlink newlink-blue">
                            <td onclick="ChangeMode(1);" id="MenuTable_td_1" tabindex="0" style="width: 18%" runat="server">個人基本資料 </td>
                            <td onclick="ChangeMode(2);" id="MenuTable_td_2" tabindex="0" style="width: 18%" runat="server">參訓背景 </td>
                            <td onclick="ChangeMode(4);" id="MenuTable_td_4" tabindex="0" style="width: 18%" runat="server">身分驗證紀錄 </td>
                            <td onclick="ChangeMode(3);" id="MenuTable_td_3" tabindex="0" style="width: 18%" runat="server">補助費用歷史頁 </td>
                        </tr>
                    </table>
                    <table id="DetailTable" class="table_nw" cellspacing="1" cellpadding="1" width="100%" runat="server">
                        ...
                        <tr>
                            ...
                            <td class="bluecol_need">畢業狀況<br />
                                &nbsp;<span id="fontGraduateY" runat="server" style="color: black">畢業年份</span>
                                ...
                                <table id="SoldierTable" class="table_nw" cellspacing="1" cellpadding="1" width="100%" runat="server">
                                    ...
                                    <tr>
                                        ...
                                        <td class="whitecol" colspan="3" width="82%">
                                            <input id="ZipCode4" onfocus="this.blur()" maxlength="3" runat="server" />－
                        ...
                        <tr>
                            ...
                            <td class="whitecol" colspan="3" width="82%">
                                <%--<asp:checkbox id="CheckBox2" runat="server" cssclass="font" text="同戶籍地址"></asp:checkbox><br>--%>
                                <input id="ZipCode1" onfocus="this.blur()" maxlength="3" runat="server" />－
                        ...
                        <tr>
                            ...
                            <td class="whitecol" colspan="3" width="82%">
                                ...
                                <input id="ZipCode2" onfocus="this.blur()" maxlength="3" runat="server" />－
                        ...
                        <tr id="trSubsidyID" runat="server">
                            <td class="bluecol" width="18%">津貼類別 <font color="red">
                                <asp:Label ID="SubsidyLabel" runat="server">*</asp:Label></font></td>
                            <td class="whitecol" colspan="3" width="82%">
                                <asp:DropDownList ID="SubsidyID" runat="server"></asp:DropDownList>
                                <input id="SubsidyHidden" type="hidden" name="hidden" runat="server" />
                            </td>
                        </tr>
                        <tr id="trSubsidyIdentity" runat="server">
                            <td class="bluecol" width="18%">津貼<br />
                                身分別<asp:Label ID="LabSubsidy" runat="server" ForeColor="red">*</asp:Label></td>
                            <td class="whitecol" colspan="3" width="82%">
                                <asp:DropDownList ID="SubsidyIdentity" runat="server"></asp:DropDownList></td>
                        </tr>
                        <%--
                        <tr id="NativeTr1" runat="server">
							<td class="bluecol_need" width="100">民族別 </td>
							<td class="whitecol" colspan="3"><asp:DropDownList ID="NativeID" runat="server"></asp:DropDownList></td>
						</tr>
                        --%>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td class="whitecol" colspan="3" width="82%">
                                ...
                                <input id="ZipCode3" onfocus="this.blur()" maxlength="3" runat="server" />－
                        ...
                        <tr id="ForeTr5" runat="server">
                            ...
                            <td class="whitecol" colspan="3" width="82%">
                                <input id="ForeZip" onfocus="this.blur()" maxlength="3" runat="server" />－
...
<asp:Literal ID="BudIDMsg" runat="server"></asp:Literal></td>--%>
                            ...
                            <td class="whitecol" colspan="2" id="td_wc_2445" runat="server"></td>
                        </tr>
                        ...
                        <tr id="trPMode" runat="server">
                            <%--<td id="PModeTD" class="bluecol" runat="server">--%>
                        ...
                        <tr id="WSITR" runat="server">
                            <td class="bluecol_need" width="18%">是否為在職者補助身分 </td>
                            <td class="whitecol" width="32%">
                        ...
                        <tr id="WSITR2" runat="server">
                            <td class="bluecol" width="18%">在職者已補助經費 </td>
                            <td class="whitecol" colspan="3" width="82%">
                    ...
                    <table id="Table2" class="font" border="0" cellspacing="1" cellpadding="1" width="100%">
                        ...
                        <tr id="LearnTR1" runat="server">
                            <td class="bluecol" width="40%">學習券課程單元 </td>
                            <td class="bluecol" width="30%">實際上課時數 </td>
                            <td class="bluecol" width="30%">單元成績(0~100分) </td>
                        </tr>
                        <tr id="LearnTR2" runat="server">
                            <td class="whitecol" rowspan="4">
                                <asp:CheckBoxList ID="RelClass_Unit" runat="server" CssClass="font" CellPadding="1" CellSpacing="10"></asp:CheckBoxList></td>
                            <td class="whitecol">
                                <asp:TextBox ID="Unit1Hour" runat="server" Columns="5" MaxLength="2" Width="30%"></asp:TextBox>小時(
                                <asp:Label ID="Label1" runat="server">label</asp:Label>h)
                            </td>
                            <td class="whitecol">&nbsp;<asp:TextBox ID="Unit1Score" runat="server" Width="30%"></asp:TextBox>分 </td>
                        </tr>
                        <tr id="LearnTR3" runat="server">
                            <td class="whitecol">
                                <asp:TextBox ID="Unit2Hour" runat="server" Columns="5" MaxLength="2" Width="30%"></asp:TextBox>小時(
                                <asp:Label ID="Label2" runat="server">label</asp:Label>h)
                            </td>
                            <td class="whitecol">&nbsp;<asp:TextBox ID="Unit2Score" runat="server" Width="30%"></asp:TextBox>分 </td>
                        </tr>
                        <tr id="LearnTR4" runat="server">
                            <td class="whitecol">
                                <asp:TextBox ID="Unit3Hour" runat="server" Columns="5" MaxLength="2" Width="30%"></asp:TextBox>小時(
                                <asp:Label ID="Label3" runat="server">label</asp:Label>h)
                            </td>
                            <td class="whitecol">&nbsp;<asp:TextBox ID="Unit3Score" runat="server" Width="30%"></asp:TextBox>分 </td>
                        </tr>
                        <tr id="LearnTR5" runat="server">
                            <td class="whitecol">
                                <asp:TextBox ID="Unit4Hour" runat="server" Columns="5" MaxLength="2" Width="30%"></asp:TextBox>小時(
                                <asp:Label ID="Label4" runat="server">label</asp:Label>h)
                            </td>
                            <td class="whitecol">&nbsp;<asp:TextBox ID="Unit4Score" runat="server" Width="30%"></asp:TextBox>分 </td>
                        </tr>
                        <tr id="TPlan23TR" runat="server">
                            <td class="bluecol_need">指定投保單位 保險證號 </td>
                            <td class="whitecol" colspan="3">
                                <asp:TextBox ID="ActNo" runat="server" Columns="13" MaxLength="9" Width="45%"></asp:TextBox></td>
                        </tr>
                        <!-- end--->
                    </table>
                </td>
        ...
        <table id="BackTable" cellspacing="1" cellpadding="1" width="100%" runat="server" class="table_nw">
            ...
            <tr>
                <td class="bluecol" width="20%">郵政/銀行帳號<asp:Label ID="star12" runat="server"><font color="#ff0000">*</font></asp:Label></td>
                <td class="whitecol" colspan="3" width="80%">
            ...
            <tr id="PortTR" runat="server">
                <td class="bluecol" width="20%">局號<asp:Label ID="star13" runat="server"><font color="#ff0000">*</font></asp:Label></td>
                <td class="whitecol" width="30%">
                    <asp:TextBox ID="PostNo_1" runat="server" Columns="8" MaxLength="10" Width="80%"></asp:TextBox>
                    <%--－<asp:TextBox ID="PostNo_2" runat="server" Columns="5" MaxLength="25"></asp:TextBox>--%>
                </td>
                <td class="bluecol_need" width="20%">帳號 </td>
                <td class="whitecol" width="30%">
                    <asp:TextBox ID="AcctNo1_1" runat="server" Columns="8" MaxLength="10" Width="80%"></asp:TextBox>
                    <%--－<asp:TextBox ID="AcctNo1_2" runat="server" Columns="5" MaxLength="25"></asp:TextBox>--%>
                </td>
            </tr>
            <tr id="BankTR1" runat="server">
                <td class="bluecol_need">總行名稱 </td>
                <td class="whitecol">
                    <asp:TextBox ID="BankName" runat="server" MaxLength="100" Width="80%"></asp:TextBox></td>
                <td class="bluecol_need">總行代號 </td>
                <td class="whitecol">
                    <asp:TextBox ID="AcctheadNo" runat="server" Columns="8" MaxLength="50" Width="80%"></asp:TextBox></td>
            </tr>
            <tr id="BankTR2" runat="server">
                <td class="bluecol_need">分行名稱 </td>
                <td class="whitecol">
                    <asp:TextBox ID="ExBankName" runat="server" MaxLength="100" Width="80%"></asp:TextBox></td>
                <td class="bluecol_need">分行代號 </td>
                <td class="whitecol">
                    <asp:TextBox ID="AcctExNo" runat="server" Columns="8" MaxLength="50" Width="80%"></asp:TextBox></td>
            </tr>
            <tr id="BankTR3" runat="server">
                <td class="bluecol_need">帳號 </td>
                <td class="whitecol" colspan="3">
                    <asp:TextBox ID="AcctNo2" runat="server" MaxLength="50" Width="30%"></asp:TextBox></td>
            </tr>
            <tr>
                <td class="bluecol">第一次投保日 </td>
                <td class="whitecol" colspan="3">
                    <asp:TextBox ID="FirDate" runat="server" Columns="10" Width="16%"></asp:TextBox>
                    <span runat="server">
                        <img style="cursor: pointer" onclick="javascript:show_calendar('<%= FirDate.ClientID %>','','','CY/MM/DD');" alt="" align="top" src="../../images/show-calendar.gif" width="30" height="30" /></span>
                </td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td class="whitecol">
                    ...
                    <asp:TextBox ID="ActNo1" runat="server" Columns="13" MaxLength="9" Width="60%"></asp:TextBox>
                    <asp:HiddenField ID="hid_ActNo1" runat="server" />
                    <%--
            ...
            <tr>
                ...
                <td class="whitecol" colspan="3">
                    <input id="txt_ActZip" onfocus="this.blur()" maxlength="3" name="txt_ActZip" runat="server" />－
            ...
            --%>
            <tr>
                <td class="bluecol">公司電話<asp:Label ID="star19" runat="server"> </asp:Label></td>
                <td class="whitecol">
                    <asp:TextBox ID="Tel" runat="server" Width="80%"></asp:TextBox></td>
                <td class="bluecol">公司傳真 </td>
                <td class="whitecol">
                    <asp:TextBox ID="Fax" runat="server" Width="80%"></asp:TextBox></td>
            </tr>
            <tr>
                <td class="bluecol">公司地址<asp:Label ID="star20" runat="server"> </asp:Label></td>
                <td class="whitecol" colspan="3">
                    <asp:CheckBox ID="Checkbox4" runat="server" Text="同投保單位地址"></asp:CheckBox><br />
                    <input id="Zip" onfocus="this.blur()" maxlength="3" name="Zip" runat="server" />－
                    ...
                    <input id="hidCityName7" type="hidden" runat="server" />
                    <input id="hidAREA7" type="hidden" runat="server" />
                    <input id="Zip_N" type="hidden" runat="server" /><br />
                    <asp:TextBox ID="City5" runat="server" Width="20%" onfocus="this.blur()"></asp:TextBox>
                    <input id="bt_openZip7" value="..." type="button" name="bt_openZip6" runat="server" class="asp_button_Mini" />
                    <asp:TextBox ID="Addr" runat="server" Width="60%"></asp:TextBox>
                    <asp:HiddenField ID="Hid_JnZip" runat="server" />
                </td>
            </tr>
            <tr>
            ...
            <tr>
                <td class="bluecol">個人到任目前<br />
                    任職公司起日 </td>
                <td class="whitecol">
                    <asp:TextBox ID="SDate" runat="server" Columns="10" Width="40%"></asp:TextBox>
                    <span runat="server">
                        <img style="cursor: pointer" onclick="javascript:show_calendar('<%= SDate.ClientID %>','','','CY/MM/DD');" alt="" align="top" src="../../images/show-calendar.gif" width="30" height="30" /></span>
                </td>
                <td class="bluecol">&nbsp;個人到任目前<br />
                    職務起日 </td>
                <td class="whitecol">
                    <asp:TextBox ID="SJDate" runat="server" Columns="10" Width="40%"></asp:TextBox>
                    <span runat="server">
                        <img style="cursor: pointer" onclick="javascript:show_calendar('<%= SJDate.ClientID %>','','','CY/MM/DD');" alt="" align="top" src="../../images/show-calendar.gif" width="30" height="30" /></span>
                </td>
            </tr>
            <tr>
                <td class="bluecol">最近升遷日期 </td>
                <td class="whitecol" colspan="1">
                    <asp:TextBox ID="SPDate" runat="server" Columns="10" Width="40%"></asp:TextBox>
                    <span runat="server">
                        <img style="cursor: pointer" onclick="javascript:show_calendar('<%= SPDate.ClientID %>','','','CY/MM/DD');" alt="" align="top" src="../../images/show-calendar.gif" width="30" height="30" /></span>
                </td>
            </tr>
            <tr>
                <td class="table_title" colspan="4" align="center" width="100%">參訓背景資料 </td>
            </tr>
            <tr>
                <td class="bluecol_need">是否由公司<br />
                    推薦參訓 </td>
                <td class="whitecol" colspan="3">
            ...
            <tr>
                <td class="bluecol">服務單位行業別<asp:Label ID="star21" runat="server"><font color="#ff0000">*</font></asp:Label></td>
                <td class="whitecol" colspan="3">
                    <asp:DropDownList ID="Q4" runat="server"></asp:DropDownList>
                    <br />
                    <font color="red">非自願離職者須憑公立就業服務機構開立之產業人才投資方案職業訓練推介單參訓</font>
                </td>
            </tr>
            <tr>
                ...
                <td class="whitecol" colspan="3">
                    ...
                    <font color="#ff0000" size="2">
                        <br />
                        （製造業、營造業、礦業及土石採取業常僱用員工數未滿二百人者或農林漁牧業、水電燃氣業、商業、運輸、倉儲及通信業、金融保險不動產、工商服務業、社會服務及個人服務業經常僱用員工數未滿五十人者，屬中小企業。）</font>
                </td>
            </tr>
            <tr>
                <td class="bluecol">個人工作年資 </td>
                <td class="whitecol">
                    <asp:TextBox ID="Q61" runat="server" Columns="5" Width="20%"></asp:TextBox>
                    <font color="#ff0000" size="2">年(年資開放小數點填寫，以0.5為最小單位，請自行累加計算。例：年資1~11個月，請填：0.5)</font>
                </td>
                <td class="bluecol">在這家公司的年資 </td>
                <td class="whitecol">
                    <asp:TextBox ID="Q62" runat="server" Columns="5" Width="20%"></asp:TextBox>
                    <font color="#ff0000" size="2">年(年資開放小數點填寫，以0.5為最小單位，請自行累加計算。例：年資1~11個月，請填：0.5)</font>
                </td>
            </tr>
            <tr>
                ...
                <td class="whitecol">
                    <asp:TextBox ID="Q63" runat="server" Columns="5" Width="20%"></asp:TextBox>
                    <font color="#ff0000" size="2">年(年資開放小數點填寫，以0.5為最小單位，請自行累加計算。例：年資1~11個月，請填：0.5)</font>
                </td>
                <td class="bluecol">最近升遷離本職幾年&nbsp; </td>
                <td class="whitecol">
                    <asp:TextBox ID="Q64" runat="server" Columns="5" Width="20%"></asp:TextBox>
                    <font color="#ff0000" size="2">年(年資開放小數點填寫，以0.5為最小單位，請自行累加計算。例：年資1~11個月，請填：0.5)</font>
                </td>
            </tr>
        </table>
        ...
        <%--/*--*/--%>
        <table id="PassPtTable" class="table_nw" border="0" cellspacing="1" cellpadding="1" width="100%" runat="server">
            <tr>
                <td valign="top" align="center">&nbsp;&nbsp;&nbsp;
				<asp:Label ID="PassPtTableMsg1" runat="server" CssClass="font" ForeColor="red"></asp:Label><br />
                    <br />
                    <asp:Panel ID="Panel_PassPt" runat="server" Width="98%" Visible="false">
                        <asp:DataGrid Style="z-index: 0" ID="DataGrid2" runat="server" CssClass="font" Width="100%" Visible="False" AutoGenerateColumns="false" CellPadding="8">
                            <AlternatingItemStyle BackColor="#EEEEEE" />
                            <HeaderStyle CssClass="head_navy" />
                            <Columns>
                                ...
                                <asp:BoundColumn DataField="CATEGORY1" HeaderText="類別"></asp:BoundColumn>
                                <asp:BoundColumn DataField="MODIFYDATE" HeaderText="日期" DataFormatString="{0:yyyy/MM/dd HH:mm}"></asp:BoundColumn>
                                <asp:BoundColumn DataField="ACTION1" HeaderText="動作"></asp:BoundColumn>
                                <asp:TemplateColumn HeaderText="檔案下載" ItemStyle-HorizontalAlign="Left">
