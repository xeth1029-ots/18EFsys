Public Class FT_01_006
    ...
    '預計完成日期：2023/03/01 (注意：228連假4天)
    '系統：在職系統    '計畫：產投、自辦在職、充飛、區域據點    '功能：首頁>>定版數據統計表>>薪資分析統計表
    '需求說明：    '根據每年跑一次產出的參訓學員薪資分析明細表 (資料：學員參訓時及結訓後3、6、9、12個月與勞保局勾稽投保薪資(勞保)、實際薪資(勞退) )，
    '依照以下計算邏輯，產出一份薪資分析統計表供下載。
    '統計表格式：    '請參考附件格式檔：參訓人員薪資分析表_格式(企訓產出).csv
    '此為企訓目前產出的格式(他們系統去年已經完成)，訓練組要的格式都一樣，我們比照辦理。
    '(計畫名稱換掉、數據重新計算，其餘欄位跟文字都一樣)
    '作業方式：    '基本上一年一次，可在每年勾稽完薪資分析raw data後，
    '由排程計算統計數據，並產出定版的薪資分析統計表Excel/csv檔供下載。
    '系統介面：    '【訓練計畫】：產投、充飛、自辦在職、區域據點 (企委不用)
    '【學員參訓年度】：從110開始往後~今年度-2 (都是抓前年度的，所以年度顯示到今年-2)    'ex:113年，顯示110、111

    Const cst_FTNUM_006 As String = "006"
