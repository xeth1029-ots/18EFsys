Partial Class CP_02_014_R
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        'For i = 0 To Me.TPlan.Items.Count - 1
        '    If Me.TPlan.Items(i).Selected Then
        '        If TPlanID = "" Then
        '            TPlanID = Convert.ToString("\'" & Me.TPlan.Items(i).Value & "\'" & ",")
        '        Else
        '            TPlanID = TPlanID & Convert.ToString("\'" & Me.TPlan.Items(i).Value & "\'" & ",")
        '        End If
        '    End If
        'Next
        'If TPlanID <> "" Then
        ...
        'strScript += "window.open('" & Url & "GUID=" + cGuid + "&SQ_AutoLogout=true&sys=MultiBlock&filename=CP_02_014_Rpt&path=TIMS&start_date=" & start_date.Text & "&end_date=" & end_date.Text & "&OC_TYPE=" & OC_TYPE & "&TPlan=" & newTPlanID & "&SYMD='+escape('" & SYMD & "')+'&EYMD='+escape('" & EYMD & "'));" + vbCrLf
        'strScript += "</script>"
        'Page.RegisterStartupScript("window_onload", strScript)

        Dim sUrl As String = "CP_02_014_Rpt.aspx"
        Dim MyValue As String = ""
        MyValue = ""
        MyValue &= "&start_date=" & start_date.Text
        MyValue &= "&end_date=" & end_date.Text
        MyValue &= "&OC_TYPE=" & OC_TYPE
        MyValue &= "&TPlan=" & newTPlanID
        MyValue &= "&SYMD=" & SYMD
        MyValue &= "&EYMD=" & EYMD
