Partial Class CP_01_006_add9
    Inherits AuthBasePage

#Region "WEBF"
    Sub sUtl_PageInit1()
        Dim dt As DataTable = TIMS.Get_USERTABCOLUMNS("CLASS_UNEXPECTVISITOR", objconn)
        If dt.Rows.Count = 0 Then Exit Sub
        Call TIMS.sUtl_SetMaxLen(dt, "COURSENAME", CourseName)
        'Call TIMS.sUtl_SetMaxLen(dt, "DATA4_NOTE", Data4_Note)
        Call TIMS.sUtl_SetMaxLen(dt, "STUD_NAME", Stud_Name)
        Call TIMS.sUtl_SetMaxLen(dt, "SITEM1_NOTE", SItem1_Note)
        Call TIMS.sUtl_SetMaxLen(dt, "STUD_NAME2", Stud_Name2)
        Call TIMS.sUtl_SetMaxLen(dt, "SITEM1B_NOTE", SItem1b_Note)

        Call TIMS.sUtl_SetMaxLen(dt, "SITEM2_NOTE", SItem2_Note)
        'Call TIMS.sUtl_SetMaxLen(dt, "SITEM3_NOTE", SITEM3_NOTE)
        'Call TIMS.sUtl_SetMaxLen(dt, "LITEM2_2_NOTE", LItem2_2_Note)
        Call TIMS.sUtl_SetMaxLen(100, LItem2_2_Note)
        Call TIMS.sUtl_SetMaxLen(dt, "CURSENAME", CurseName)
        Call TIMS.sUtl_SetMaxLen(dt, "VISITORNAME", VisitorName)
        Call TIMS.sUtl_SetMaxLen(dt, "LITEM2_3_NOTE", LItem2_3_Note)
        'Call TIMS.sUtl_SetMaxLen(600, LItem2_3_Note)
        'Call TIMS.sUtl_SetMaxLen(dt, "SITEM51_NOTE", SITEM51_NOTE)
        'Call TIMS.sUtl_SetMaxLen(dt, "SITEM61_NOTE", SITEM61_NOTE)
        'Call TIMS.sUtl_SetMaxLen(dt, "SITEM71_NOTE", SITEM71_NOTE)
        'Call TIMS.sUtl_SetMaxLen(dt, "SITEM81_NOTE", SITEM81_NOTE)
        Call TIMS.sUtl_SetMaxLen(100, SITEM51_NOTE)
        Call TIMS.sUtl_SetMaxLen(100, SITEM61_NOTE)
        Call TIMS.sUtl_SetMaxLen(100, SITEM71_NOTE)
        Call TIMS.sUtl_SetMaxLen(100, SITEM81_NOTE)

    End Sub
#End Region

    'CLASS_UNEXPECTVISITOR 
    'Const cst_VerDate_20180101 As String = "2018/01/01"
    Const cst_xx26 As String = "%26"
    'Const cst_xx51 As String = "日期^時段^課程單元^師資^助教^場地^人數上限^專職辦訓人員^未依規定收費及退費^出席率為0且歸責於單位^其他"
    'Const cst_xx61 As String = "學員陳情申訴經查屬實，屬訓練單位之缺失^未落實學員簽到退^代簽^未依規定上課^廣宣缺失^招訓違反公平原則^其他"
    'Const cst_xx71 As String = "助教非核定助教^場地住址相同僅內部教室變更^其他"
    'Const cst_xx81 As String = "訓練單位有內部或與外部單位有糾紛導致影響學員上課權益^其他"
    'Dim sdSITEM51 As String() '將變更項目名稱定義到陣列之中
    'Dim sdSITEM61 As String() '將變更項目名稱定義到陣列之中
    'Dim sdSITEM71 As String() '將變更項目名稱定義到陣列之中
    'Dim sdSITEM81 As String() '將變更項目名稱定義到陣列之中
    Const cst_SAVE1_msg1 As String = "儲存成功"
    Const cst_SAVE1_msg2 As String = "訪查當日點名未到課之學員，應於訪查次日起3日內另以電話抽訪，並填寫「不預告(電話)抽訪學員紀錄表」"

    Const cst_CODE_KIND_SITEM51 As String = "CP_01_006_SITEM51"
    Const cst_CODE_KIND_SITEM61 As String = "CP_01_006_SITEM61"
    Const cst_CODE_KIND_SITEM71 As String = "CP_01_006_SITEM71"
    Const cst_CODE_KIND_SITEM81 As String = "CP_01_006_SITEM81"
    'State
    Const cst_State_Add_新增 As String = "Add"
    Const cst_State_View_檢視 As String = "View" '查詢
    Const cst_State_Edit_修改 As String = "Edit"
    'cst_State_Add_新增/cst_State_View_檢視/cst_State_Edit_修改

    'Dim iPYNum As Integer = 1 'iPYNum = TIMS.sUtl_GetPYNum(Me) '1:2017前 2:2017 3:2018
    'Dim au As New cAUTH
    Dim objconn As SqlConnection
    Dim flag_ROC As Boolean = TIMS.CHK_REPLACE2ROC_YEARS()

    Private Sub sUtl_PageUnload(ByVal sender As Object, ByVal e As System.EventArgs)
        Call TIMS.CloseDbConn(objconn)
    End Sub

    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        '檢查Session是否存在 Start
        ' (直接在 AuthBasePage 處理, 不用個別檢查 Session)  TIMS.CheckSession(Me)
        objconn = DbAccess.GetConnection()
        AddHandler MyBase.Unload, AddressOf sUtl_PageUnload
        'Call TIMS.GetAuth(Me, au.blnCanAdds, au.blnCanMod, au.blnCanDel, au.blnCanSech, au.blnCanPrnt) '2011 取得功能按鈕權限值
        Call TIMS.OpenDbConn(objconn) '開啟連線
        Call sUtl_PageInit1()
        '檢查Session是否存在 End

        'sdSITEM51 = cst_xx51.Split("^")
        'sdSITEM61 = cst_xx61.Split("^")
        'sdSITEM71 = cst_xx71.Split("^")
        'sdSITEM81 = cst_xx81.Split("^")
        'Dim iPYNum As Integer = 1 'iPYNum = TIMS.sUtl_GetPYNum(Me) '1:2017前 2:2017 3:2018
        'iPYNum = TIMS.sUtl_GetPYNum(Me)
        'Hid_VerDate.Value = ""
        'If iPYNum = 3 Then Hid_VerDate.Value = cst_VerDate_20180101

        Dim rqOCID As String = TIMS.ClearSQM(Request("OCID"))
        Dim rqSeqNo As String = TIMS.ClearSQM(Request("SeqNo"))
        'Dim rqDOCID As String = TIMS.ClearSQM(Request("DOCID"))
        Dim rqState As String = TIMS.ClearSQM(Request("State"))
        'cst_State_Add_新增/cst_State_View_檢視/cst_State_Edit_修改
        Hid_State1.Value = rqState
        'Dim rqType As String = TIMS.ClearSQM(Request("Type"))

        If Not IsPostBack Then
            'AuthCount.Attributes("onclick") = "javascript:return Calculate1()"
            'AuthCount.Attributes("onblur") = "javascript:return Calculate1()"
            'RejectCount.Attributes("onclick") = "javascript:return Calculate1()"
            'RejectCount.Attributes("onblur") = "javascript:return Calculate1()"

            AtteRate.Attributes("onclick") = "javascript:return Calculate2()"
            TurthCount.Attributes("onclick") = "javascript:return Calculate2()"
            TurnoutCount.Attributes("onclick") = "javascript:return Calculate2()"
            AtteRate.Attributes("onblur") = "javascript:return Calculate2()"
            TurthCount.Attributes("onblur") = "javascript:return Calculate2()"
            TurnoutCount.Attributes("onblur") = "javascript:return Calculate2()"
            'AuthCount.Attributes("onclick") = "javascript:return Calculate2()"

            OLessonTeah1.Attributes.Add("onDblClick", "javascript:OpenLessonTeah1('Add');")
            teacherbtn.Attributes.Add("onClick", "javascript:OpenLessonTeah1('Add');")
            OLessonTeah2.Attributes.Add("onDblClick", "javascript:OpenLessonTeah2('Add');")
            teacherbtn2.Attributes.Add("onClick", "javascript:OpenLessonTeah2('Add');")
            Button1.Attributes("onclick") = "javascript:return chkdata()"
            'Button1.Enabled = True
            Call sCreate1()

            If rqState = "View" Then Button1.Visible = False
            If rqOCID = "" AndAlso OCIDValue1.Value <> "" Then rqOCID = OCIDValue1.Value
            If rqOCID <> "" Then Call sShowData1(rqOCID, rqSeqNo)
            'If rqDOCID <> "" Then Call sCreate1(rqDOCID, "")
        End If

        Const cst_javascript_openOrg_FMT1 As String = "javascript:openOrg('../../Common/LevOrg{0}.aspx');"
        Button2.Attributes("onclick") = String.Format(cst_javascript_openOrg_FMT1, If(sm.UserInfo.RID = "A" OrElse sm.UserInfo.RoleID <= 1, "", "1"))

        TIMS.ShowHistoryRID(Me, HistoryRID, "HistoryList2", "RIDValue", "center")
        If HistoryRID.Rows.Count <> 0 Then
            center.Attributes("onclick") = "showObj('HistoryList2');"
            center.Style("CURSOR") = "hand"
        End If

        TIMS.ShowHistoryClass(Me, HistoryTable, "HistoryList", "OCIDValue1", "OCID1", "RIDValue", "center", "TMIDValue1", "TMID1")
        If HistoryTable.Rows.Count <> 0 Then
            OCID1.Attributes("onclick") = "showObj('HistoryList');"
            OCID1.Style("CURSOR") = "hand"
        End If
    End Sub

    Sub sCreate1()
        'Select * From SYS_SHAREDCODE Where CODE_KIND ='CP_01_006_SITEM51' ORDER BY SORT_ID
        'Select * From SYS_SHAREDCODE Where CODE_KIND ='CP_01_006_SITEM61' ORDER BY SORT_ID
        'Select * From SYS_SHAREDCODE Where CODE_KIND ='CP_01_006_SITEM71' ORDER BY SORT_ID
        'Select * From SYS_SHAREDCODE Where CODE_KIND ='CP_01_006_SITEM81' ORDER BY SORT_ID
        cblSITEM51 = TIMS.GET_CBLCODE1(cst_CODE_KIND_SITEM51, cblSITEM51, objconn)
        cblSITEM61 = TIMS.GET_CBLCODE1(cst_CODE_KIND_SITEM61, cblSITEM61, objconn)
        cblSITEM71 = TIMS.GET_CBLCODE1(cst_CODE_KIND_SITEM71, cblSITEM71, objconn)
        cblSITEM81 = TIMS.GET_CBLCODE1(cst_CODE_KIND_SITEM81, cblSITEM81, objconn)
        Applytime_HH.Value = "09"
        Applytime_MM.Value = "00"
        'LItem_TR.Attributes.Add("Style", "none")
        'LItem_TR2.Attributes.Add("Style", "none")
        LItem_TR.Style.Item("display") = "none"
        LItem_TR2.Style.Item("display") = "none"
        OrgName.Text = sm.UserInfo.OrgName
        OrgID.Value = sm.UserInfo.OrgID
        If Session("SearchStr") IsNot Nothing Then
            Dim str_SearchStr As String = Convert.ToString(Session("SearchStr"))
            Dim MyValue As String = ""
            MyValue = TIMS.GetMyValue(str_SearchStr, "prgid")
            If MyValue = "CP_01_006" Then
                MyValue = TIMS.GetMyValue(str_SearchStr, "center")
                center.Text = Replace(MyValue, cst_xx26, "&")
                RIDValue.Value = TIMS.GetMyValue(str_SearchStr, "RIDValue")
                MyValue = TIMS.GetMyValue(str_SearchStr, "TMID1")
                TMID1.Text = Replace(MyValue, cst_xx26, "&")
                MyValue = TIMS.GetMyValue(str_SearchStr, "OCID1")
                OCID1.Text = Replace(MyValue, cst_xx26, "&")
                TMIDValue1.Value = TIMS.GetMyValue(str_SearchStr, "TMIDValue1")
                OCIDValue1.Value = TIMS.GetMyValue(str_SearchStr, "OCIDValue1")
                labSFDATE_TW.Text = TIMS.GetMyValue(str_SearchStr, "SFDATE_TW")
            End If
            If Session("SearchStr") IsNot Nothing Then Session("SearchStr") = Session("SearchStr")
        End If
    End Sub

    Function GET_STUDCOUNT2(ByVal OCID As String) As Hashtable
        Dim rst As New Hashtable
        Dim parms As New Hashtable From {{"OCID", OCID}}
        Dim sql As String = ""
        sql &= " SELECT ISNULL(COUNT(1),0) STUDCNT"
        sql &= " ,ISNULL(COUNT(CASE WHEN STUDSTATUS in (2,3) then 1 end),0) REJCNT"
        sql &= " FROM dbo.V_STUDENTINFO WHERE OCID=@OCID"
        Dim dt As DataTable = DbAccess.GetDataTable(sql, objconn, parms)
        If TIMS.dtHaveDATA(dt) Then
            Dim dr1 As DataRow = dt.Rows(0)
            'rst.Clear()
            rst.Add("STUDCNT", dr1("STUDCNT"))
            rst.Add("REJCNT", dr1("REJCNT"))
        End If
        Return rst
    End Function

    '取得資料庫資料 CLASS_UNEXPECTVISITOR
    Sub sShowData1(ByVal OCID As String, ByVal SeqNo As String)
        '不可為空
        OCID = TIMS.ClearSQM(OCID)
        SeqNo = TIMS.ClearSQM(SeqNo)
        If OCID = "" OrElse SeqNo = "" Then SeqNo = "0"
        'Class_Visitor

        Dim drC As DataRow = TIMS.GetOCIDDate(OCID, objconn)
        If drC Is Nothing Then Return
        If drC IsNot Nothing Then
            center.Text = $"{drC("OrgName")}"
            RIDValue.Value = $"{drC("RID")}"
            TMID1.Text = $"{drC("TrainName2")}"
            TMIDValue1.Value = $"{drC("TMID")}"
            OCID1.Text = $"{drC("ClassCName2")}"
            OCIDValue1.Value = $"{drC("OCID")}"

            center.Enabled = False
            TMID1.Enabled = False
            OCID1.Enabled = False
            Button2.Disabled = True
            Button3.Disabled = True
        End If

        '在學員動態管理>>教務管理>>不預告實地抽訪紀錄表，由系統自動帶出應到人數及計算出席率
        '公式：
        '應到人數=參訓人數-退訓人數
        'AuthCount=Hid_StudCount-RejectCount
        '出席率=(實到人數+請假人數)/應到人數 "
        'AtteRate=(TurthCount+TurnoutCount) /AuthCount

        Hid_StudCount.Value = ""
        RejectCount.Text = "" 'TIMS.GetMyValue2(getP, "REJCNT") '退訓人數
        AuthCount.Text = "" 'Val(Hid_StudCount.Value) - Val(RejectCount.Text) '應到人數
        If Hid_State1.Value = cst_State_Add_新增 OrElse Hid_State1.Value = "" Then
            Dim getP As Hashtable = GET_STUDCOUNT2(OCID)
            Hid_StudCount.Value = TIMS.GetMyValue2(getP, "STUDCNT") '參訓人數
            RejectCount.Text = TIMS.GetMyValue2(getP, "REJCNT") '退訓人數
            If (Hid_StudCount.Value <> "" AndAlso RejectCount.Text <> "") Then
                AuthCount.Text = Val(Hid_StudCount.Value) - Val(RejectCount.Text) '應到人數
            End If
        End If

        Dim parms As New Hashtable From {{"OCID", OCID}, {"SeqNo", SeqNo}}
        Dim sql As String = "SELECT * FROM dbo.CLASS_UNEXPECTVISITOR WHERE OCID=@OCID AND SeqNo=@SeqNo"
        Dim dt As DataTable = DbAccess.GetDataTable(sql, objconn, parms)
        If TIMS.dtNODATA(dt) Then Return

        If dt.Rows.Count > 0 Then
            Dim dr As DataRow = dt.Rows(0)

            ApplyDate.Text = TIMS.Cdate3(dr("ApplyDate")) 'edit，by:20181018
            If flag_ROC Then ApplyDate.Text = TIMS.Cdate17(dr("ApplyDate"))  'edit，by:20181018

            Applytime_HH.Value = Left(dr("ApplyTime"), 2)
            Applytime_MM.Value = Right(dr("ApplyTime"), 2)
            AuthCount.Text = $"{dr("AuthCount")}"

            TurthCount.Text = $"{dr("TurthCount")}"
            TruancyCount.Text = $"{dr("TruancyCount")}"
            TurnoutCount.Text = $"{dr("TurnoutCount")}"
            RejectCount.Text = $"{dr("RejectCount")}"
            OtherCount.Text = $"{dr("OtherCount")}"
            AtteRate.Text = ""
            If $"{dr("AtteRate")}" <> "" AndAlso TIMS.IsNumeric1(dr("AtteRate")) Then
                AtteRate.Text = TIMS.ROUND(dr("AtteRate") * 100)
            End If
            OLessonTeah1.Text = ""
            OLessonTeah1Value.Value = ""
            If dr("TechID").ToString <> "" Then
                OLessonTeah1.Text = TIMS.Get_TeachCName(dr("TechID").ToString, objconn)
                OLessonTeah1Value.Value = dr("TechID").ToString
            End If
            OLessonTeah2.Text = ""
            OLessonTeah2Value.Value = ""
            If $"{dr("TechID2")}" <> "" Then
                OLessonTeah2.Text = TIMS.Get_TeachCName(dr("TechID2").ToString, objconn)
                OLessonTeah2Value.Value = dr("TechID2").ToString
            End If
            If $"{dr("CourseName")}" <> "" Then CourseName.Text = $"{dr("CourseName")}"
            'VISITWAY: 1:實地訪查  '2:視訊訪查
            Dim v_rblVISITWAY As String = If($"{dr("VISITWAY")}" <> "", $"{dr("VISITWAY")}", "1")
            Common.SetListItem(rblVISITWAY, v_rblVISITWAY)

            Common.SetListItem(DATA81, $"{dr("Data81")}")
            Common.SetListItem(DATA82, $"{dr("Data82")}")
            Common.SetListItem(DATA83, $"{dr("Data83")}")
            Common.SetListItem(DATA84, $"{dr("Data84")}")

            Common.SetListItem(Item1, $"{dr("Item1")}")
            Common.SetListItem(Item2, $"{dr("Item2")}")
            Common.SetListItem(Item3, $"{dr("Item3")}")
            Common.SetListItem(Item4, $"{dr("Item4")}")

            Common.SetListItem(SITEMR3_1, $"{dr("SITEMR3_1")}")
            Common.SetListItem(SITEMR3_2, $"{dr("SITEMR3_2")}")
            Common.SetListItem(SITEMR3_3, $"{dr("SITEMR3_3")}")
            Common.SetListItem(SITEMR3_4, $"{dr("SITEMR3_4")}")

            Stud_Name.Text = TIMS.ClearSQM(dr("Stud_Name"))
            Stud_Name2.Text = TIMS.ClearSQM(dr("Stud_Name2"))
            Common.SetListItem(SItem1, $"{dr("SItem1")}") '5:抽訪時未到
            Common.SetListItem(SItem1b, $"{dr("SItem1b")}") '5:抽訪時未到
            Common.SetListItem(SItem2, $"{dr("SItem2")}")

            LItem1.Checked = False
            LItem2.Checked = False
            If dr("LItem1").ToString = "1" Then
                LItem1.Checked = True
                'LItem_TR.Attributes.Add("Style", "none")
                'LItem_TR2.Attributes.Add("Style", "none")
                LItem_TR.Style.Item("display") = "none"
                LItem_TR2.Style.Item("display") = "none"
            End If
            If dr("LItem2").ToString = "1" Then
                LItem2.Checked = True
                'LItem_TR.Attributes.Add("Style", "inline")
                'LItem_TR2.Attributes.Add("Style", "inline")
                LItem_TR.Style.Item("display") = "inline"
                LItem_TR2.Style.Item("display") = "inline"
            End If

            LItem2_1.Checked = False
            If dr("LItem2_1").ToString = "1" Then
                LItem2_1.Checked = True
            End If
            If dr("LItem2_1_Date").ToString <> "" Then
                LItem2_1_Date.Text = TIMS.Cdate3(dr("LItem2_1_Date"))
                If flag_ROC Then LItem2_1_Date.Text = TIMS.Cdate17(dr("LItem2_1_Date"))  'edit，by:2018108
            End If

            LItem2_2.Checked = ($"{dr("LItem2_2")}" = "1")
            If $"{dr("LItem2_2b")}" <> "" Then
                Call TIMS.SetCblValue(cblLItem2_2b, dr("LItem2_2b"))
            End If

            LItem2_2_Note.Text = TIMS.ClearSQM(dr("LItem2_2_Note"))
            LItem2_3_Note.Text = TIMS.ClearSQM(dr("LItem2_3_Note"))
            SItem1_Note.Text = TIMS.ClearSQM(dr("SItem1_Note"))
            SItem1b_Note.Text = TIMS.ClearSQM(dr("SItem1b_Note"))
            SItem2_Note.Text = TIMS.ClearSQM(dr("SItem2_Note"))

            If dr("OrgID").ToString <> "" Then
                OrgName.Text = TIMS.GET_ORGNAME(dr("OrgID"), objconn)
                OrgID.Value = dr("OrgID").ToString 'TIMS.GET_OrgName(dr("OrgID").ToString)
            End If
            CurseName.Text = $"{dr("CurseName")}"
            VisitorName.Text = $"{dr("VisitorName")}"

            TIMS.SetCblValue(cblSITEM51, $"{dr("SITEM51")}")
            TIMS.SetCblValue(cblSITEM61, $"{dr("SITEM61")}")
            TIMS.SetCblValue(cblSITEM71, $"{dr("SITEM71")}")
            TIMS.SetCblValue(cblSITEM81, $"{dr("SITEM81")}")
            SITEM51_NOTE.Text = $"{dr("SITEM51_NOTE")}"
            SITEM61_NOTE.Text = $"{dr("SITEM61_NOTE")}"
            SITEM71_NOTE.Text = $"{dr("SITEM71_NOTE")}"
            SITEM81_NOTE.Text = $"{dr("SITEM81_NOTE")}"

            chkB_NOINC5.Checked = ($"{dr("NOINC5")}" = TIMS.cst_YES)
        End If

    End Sub

    '檢查有效儲存資料
    Function CheckData1(ByRef Errmsg As String) As Boolean
        Dim Rst As Boolean = True
        Errmsg = ""

        ApplyDate.Text = TIMS.ClearSQM(ApplyDate.Text)
        OCIDValue1.Value = TIMS.ClearSQM(OCIDValue1.Value)

        If OCIDValue1.Value = "" Then
            Errmsg &= "班級尚未選擇，請重新選擇" & vbCrLf
        ElseIf Not IsNumeric(OCIDValue1.Value) Then
            Errmsg &= "班級選擇有誤，請重新選擇!" & vbCrLf
        End If

        If ApplyDate.Text <> "" Then
            If flag_ROC Then
                If Not TIMS.IsDate7(ApplyDate.Text) Then Errmsg &= "訪查時間 日期格式有誤" & vbCrLf  'edit，by:20181018
            Else
                If Not TIMS.IsDate1(ApplyDate.Text) Then Errmsg &= "訪查時間 日期格式有誤!" & vbCrLf  'edit，by:20181018
            End If

            If Errmsg = "" Then
                If flag_ROC Then
                    ApplyDate.Text = TIMS.Cdate7(ApplyDate.Text)  'edit，by:20181018
                Else
                    ApplyDate.Text = CDate(ApplyDate.Text).ToString("yyyy/MM/dd")  'edit，by:20181018
                End If
            End If
        Else
            Errmsg &= "訪查時間 日期 為必填" & vbCrLf
        End If

        If IsNumeric(Applytime_HH.Value) Then
            If CInt(Applytime_HH.Value) > 23 OrElse CInt(Applytime_HH.Value) < 0 Then Errmsg &= "訪查時間 時間(幾點)格式有誤(00~23)，請重新填寫" & vbCrLf
            Applytime_HH.Value = CStr(CInt(Applytime_HH.Value))
        Else
            Errmsg &= "訪查時間 時間(幾點)格式有誤(00~23)，請重新填寫" & vbCrLf
        End If

        If IsNumeric(Applytime_MM.Value) Then
            If CInt(Applytime_MM.Value) > 59 OrElse CInt(Applytime_MM.Value) < 0 Then Errmsg &= "訪查時間 時間(幾分)格式有誤(00~59)，請重新填寫" & vbCrLf
            Applytime_MM.Value = CStr(CInt(Applytime_MM.Value))
        Else
            Errmsg &= "訪查時間 時間(幾分)格式有誤(00~59)，請重新填寫" & vbCrLf
        End If

        If Errmsg = "" Then
            If Len(Applytime_HH.Value) < 2 Then Applytime_HH.Value = "0" & Applytime_HH.Value
            If Len(Applytime_MM.Value) < 2 Then Applytime_MM.Value = "0" & Applytime_MM.Value
        End If

        '當日教師
        OLessonTeah1Value.Value = TIMS.ClearSQM(OLessonTeah1Value.Value)
        If OLessonTeah1Value.Value = "" Then
            '當日教師
            OLessonTeah1.Text = ""
            OLessonTeah1Value.Value = ""
            ODegreeID1.Value = ""
            ODegreeIDValue1.Value = ""
            Errmsg &= "請輸入 當日教師" & vbCrLf
        End If

        CourseName.Text = TIMS.ClearSQM(CourseName.Text)
        Dim int_Len1 As Integer = 0 '當日課程 int_Len1 = 0
        If CourseName.Text = "" Then
            Errmsg &= "請輸入 當日課程" & vbCrLf
        ElseIf CourseName.MaxLength > 0 Then
            int_Len1 = CourseName.MaxLength
            If Len(CourseName.Text) > int_Len1 Then Errmsg &= $"當日課程 長度超過系統範圍({int_Len1}){vbCrLf}"
        End If

        AuthCount.Text = TIMS.ClearSQM(AuthCount.Text)
        TurthCount.Text = TIMS.ClearSQM(TurthCount.Text)
        TruancyCount.Text = TIMS.ClearSQM(TruancyCount.Text)
        TurnoutCount.Text = TIMS.ClearSQM(TurnoutCount.Text)
        RejectCount.Text = TIMS.ClearSQM(RejectCount.Text)
        OtherCount.Text = TIMS.ClearSQM(OtherCount.Text)

        If Not IsNumeric(AuthCount.Text) Then Errmsg &= "應到人數 應為數字格式有誤，請重新填寫" & vbCrLf
        If Not IsNumeric(TurthCount.Text) Then Errmsg &= "實到人數 應為數字格式有誤，請重新填寫" & vbCrLf
        If Not IsNumeric(TruancyCount.Text) Then Errmsg &= "未到人數 應為數字格式有誤，請重新填寫" & vbCrLf
        If Not IsNumeric(TurnoutCount.Text) Then Errmsg &= "請假人數 應為數字格式有誤，請重新填寫" & vbCrLf
        If Not IsNumeric(RejectCount.Text) Then Errmsg &= "退訓人數 應為數字格式有誤，請重新填寫" & vbCrLf
        If Not IsNumeric(OtherCount.Text) Then Errmsg &= "其他人數 應為數字格式有誤，請重新填寫" & vbCrLf

        AtteRate.Text = TIMS.ClearSQM(AtteRate.Text)
        If Not IsNumeric(AtteRate.Text) Then Errmsg &= "出席率 應為數字格式有誤，請重新填寫" & vbCrLf
        If Errmsg = "" AndAlso AtteRate.Text <> "" Then
            'If Not TIMS.IsNumeric2(AtteRate.Text) Then Errmsg &= "出席率 格式有誤，應為正整數數字格式 " & vbCrLf 'int
            If Not TIMS.IsNumeric1(AtteRate.Text) Then
                Errmsg &= "出席率 格式有誤，應為正整數數字格式 " & vbCrLf 'int
            ElseIf CInt(AtteRate.Text) < 0 Then
                Errmsg &= "出席率 格式有誤，應為正整數數字格式 " & vbCrLf 'int
            End If
        End If
        If Errmsg = "" AndAlso AtteRate.Text <> "" Then If Val(AtteRate.Text) > 100 Then Errmsg &= "出席率 數字格式 不可超過100" & vbCrLf 'int

        If Errmsg = "" Then
            AuthCount.Text = CInt(AuthCount.Text)
            TurthCount.Text = CInt(TurthCount.Text)
            TruancyCount.Text = CInt(TruancyCount.Text)
            TurnoutCount.Text = CInt(TurnoutCount.Text)
            RejectCount.Text = CInt(RejectCount.Text)
            OtherCount.Text = CInt(OtherCount.Text)
        End If

        'VISITWAY: 1:實地訪查  '2:視訊訪查
        Dim v_rblVISITWAY As String = TIMS.GetListValue(rblVISITWAY)
        Dim v_SITEMR3_1 As String = TIMS.GetListValue(SITEMR3_1)
        Dim v_SITEMR3_2 As String = TIMS.GetListValue(SITEMR3_2)
        Dim v_SITEMR3_3 As String = TIMS.GetListValue(SITEMR3_3)
        Dim v_SITEMR3_4 As String = TIMS.GetListValue(SITEMR3_4)
        If v_rblVISITWAY = "" Then Errmsg &= "請選擇 訪查方式：1:實地訪查，2:視訊訪查" & vbCrLf
        '增加卡控： 若分署於【訪查方式】選擇「視訊訪查」，則必須填寫此項之內容，若為「實地訪查」，可不選擇此大項。
        'line@系統 有線上視訊查課，不限定授課方式，實體課程 調整為不卡控
        'If v_rblVISITWAY = "2" Then
        '    If v_SITEMR3_1 = "" Then Errmsg &= "(訪查方式：2:視訊訪查) 請選擇 1.是否可順利連線?" & vbCrLf
        '    If v_SITEMR3_2 = "" Then Errmsg &= "(訪查方式：2:視訊訪查) 請選擇 2.是否有學員未使用本名?" & vbCrLf
        '    If v_SITEMR3_3 = "" Then Errmsg &= "(訪查方式：2:視訊訪查) 請選擇 3.學員上課螢幕擷圖?" & vbCrLf
        '    If v_SITEMR3_4 = "" Then Errmsg &= "(訪查方式：2:視訊訪查) 請選擇 4.是否有學員使用虛擬背景?" & vbCrLf
        'End If

        Stud_Name.Text = TIMS.ClearSQM(Stud_Name.Text)
        If Stud_Name.Text <> "" Then
            int_Len1 = Stud_Name.MaxLength
            If Len(Stud_Name.Text) > int_Len1 Then
                Errmsg &= "三、現場訪查實況：抽訪學員之姓名1 長度超過系統範圍(" & CStr(int_Len1) & ")" & vbCrLf
            End If
        Else
            Stud_Name.Text = ""
            Errmsg &= "請輸入 三、現場訪查實況：抽訪學員之姓名1" & vbCrLf
        End If

        Stud_Name2.Text = TIMS.ClearSQM(Stud_Name2.Text)
        If Stud_Name2.Text <> "" Then
            int_Len1 = Stud_Name2.MaxLength
            If Len(Stud_Name2.Text) > int_Len1 Then Errmsg &= "三、現場訪查實況：抽訪學員之姓名2 長度超過系統範圍(" & CStr(int_Len1) & ")" & vbCrLf
        Else
            Stud_Name2.Text = ""
            Errmsg &= "請輸入 三、現場訪查實況：抽訪學員之姓名2" & vbCrLf
        End If

        If LItem2_1_Date.Text <> "" Then
            If Not TIMS.IsDate7(LItem2_1_Date.Text) Then Errmsg &= "四、現場處理說明：2.不預告抽訪結果需修正如下： 日期格式有誤" & vbCrLf
            If Errmsg = "" Then
                If flag_ROC Then
                    LItem2_1_Date.Text = TIMS.Cdate7(LItem2_1_Date.Text)  'edit，by:20181018
                Else
                    LItem2_1_Date.Text = CDate(LItem2_1_Date.Text).ToString("yyyy/MM/dd")  'edit，by:20181018
                End If
            End If
        End If

        LItem2_2_Note.Text = TIMS.ClearSQM(LItem2_2_Note.Text)
        LItem2_3_Note.Text = TIMS.ClearSQM(LItem2_3_Note.Text)
        If LItem2_2_Note.Text <> "" Then
            int_Len1 = LItem2_2_Note.MaxLength
            If Len(LItem2_2_Note.Text) > int_Len1 Then Errmsg &= $"四、現場處理說明：2.不預告抽訪結果需修正如下：(2)其他 長度超過系統範圍({int_Len1}){vbCrLf}"
        End If
        If LItem2_3_Note.Text <> "" Then
            int_Len1 = LItem2_3_Note.MaxLength
            If Len(LItem2_3_Note.Text) > int_Len1 Then Errmsg &= $"四、現場處理說明：2.不預告抽訪結果需修正如下：(3)其他補充說明 長度超過系統範圍({int_Len1}){vbCrLf}"
        End If

        SItem1_Note.Text = TIMS.ClearSQM(SItem1_Note.Text)
        SItem1b_Note.Text = TIMS.ClearSQM(SItem1b_Note.Text)
        SItem2_Note.Text = TIMS.ClearSQM(SItem2_Note.Text)

        If SItem1_Note.Text <> "" Then
            int_Len1 = SItem1_Note.MaxLength
            If Len(SItem1_Note.Text) > int_Len1 Then Errmsg &= "三、現場訪查實況1.抽訪學員之姓名1-其他 長度超過系統範圍(" & CStr(int_Len1) & ")" & vbCrLf
        End If
        If SItem1b_Note.Text <> "" Then
            int_Len1 = SItem1b_Note.MaxLength
            If Len(SItem1b_Note.Text) > int_Len1 Then Errmsg &= "三、現場訪查實況1.抽訪學員之姓名2-其他 長度超過系統範圍(" & CStr(int_Len1) & ")" & vbCrLf
        End If
        If SItem2_Note.Text <> "" Then
            int_Len1 = SItem2_Note.MaxLength
            If Len(SItem2_Note.Text) > int_Len1 Then Errmsg &= "三、現場訪查實況2.其他 長度超過系統範圍(" & CStr(int_Len1) & ")" & vbCrLf
        End If

        'If Trim(SItem3_Note.Text) <> "" Then
        '    SItem3_Note.Text = Trim(SItem3_Note.Text)
        '    int_Len1 = SItem3_Note.MaxLength
        '    If Len(SItem3_Note.Text) > int_Len1 Then
        '        Errmsg &= "三、現場訪查實況3.其他 長度超過系統範圍(" & CStr(int_Len1) & ")" & vbCrLf
        '    End If
        'Else
        '    SItem3_Note.Text = ""
        'End If

        CurseName.Text = TIMS.ClearSQM(CurseName.Text)
        If CurseName.Text = "" Then
            Errmsg &= "培訓單位人員姓名 為必填" & vbCrLf
        Else
            int_Len1 = CurseName.MaxLength
            If Len(CurseName.Text) > int_Len1 Then Errmsg &= "培訓單位人員姓名 長度超過系統範圍(" & CStr(int_Len1) & ")" & vbCrLf
        End If

        VisitorName.Text = TIMS.ClearSQM(VisitorName.Text)
        If VisitorName.Text = "" Then
            Errmsg &= "訪視人員姓名 為必填" & vbCrLf
        Else
            int_Len1 = VisitorName.MaxLength
            If Len(VisitorName.Text) > int_Len1 Then Errmsg &= "訪視人員姓名 長度超過系統範圍(" & CStr(int_Len1) & ")" & vbCrLf
        End If

        If Errmsg <> "" Then Rst = False
        Return Rst
    End Function

    ''' <summary>SELECT MAX(SeqNO) </summary>
    ''' <param name="oConn"></param>
    ''' <param name="OCIDValue1"></param>
    ''' <returns></returns>
    Public Shared Function Get_UEVSeqNoMax(ByRef oConn As SqlConnection, ByVal OCIDValue1 As String) As Integer
        Dim iSeqNo As Integer = 1

        'Dim sql As String = ""
        'Dim dr As DataRow = Nothing
        Dim parms As New Hashtable From {{"OCID", OCIDValue1}}
        Dim sql As String = "SELECT 1 FROM dbo.CLASS_UNEXPECTVISITOR WHERE OCID=@OCID"
        Dim dt As DataTable = DbAccess.GetDataTable(sql, oConn, parms)
        If TIMS.dtNODATA(dt) Then Return iSeqNo

        '先取出最大SeqNo
        Dim parms2 As New Hashtable From {{"OCID", OCIDValue1}}
        Dim sql2 As String = "SELECT MAX(SeqNO) num FROM dbo.CLASS_UNEXPECTVISITOR WHERE OCID=@OCID"
        Dim dr As DataRow = DbAccess.GetOneRow(sql2, oConn, parms2)
        If dr Is Nothing OrElse IsDBNull(dr("num")) Then Return iSeqNo

        iSeqNo = CInt(dr("num")) + 1
        Return iSeqNo
    End Function

    Sub sSaveData1()
        Dim rqOCID As String = TIMS.ClearSQM(Request("OCID"))
        Dim rqSeqNo As String = TIMS.ClearSQM(Request("SeqNo"))
        Dim rqDOCID As String = TIMS.ClearSQM(Request("DOCID"))
        Dim rqState As String = TIMS.ClearSQM(Request("State"))
        Dim rqType As String = TIMS.ClearSQM(Request("Type"))
        OCIDValue1.Value = TIMS.ClearSQM(OCIDValue1.Value)

        Dim s_RID As String = sm.UserInfo.RID
        If sm.UserInfo.RID = "A" Then
            Dim drC As DataRow = TIMS.GetOCIDDate(OCIDValue1.Value, objconn)
            If drC Is Nothing Then Return
            s_RID = drC("RID")
        End If

        Const cst_sTYPE5_FLG_5 As String = "5" '有5特殊狀況
        Dim flg_STR_STYPE5 As String = "" '無特殊狀況
        If SItem1.SelectedValue = "5" Then flg_STR_STYPE5 = cst_sTYPE5_FLG_5
        If SItem1b.SelectedValue = "5" Then flg_STR_STYPE5 = cst_sTYPE5_FLG_5

        Dim sql As String = ""
        Dim da As SqlDataAdapter = Nothing
        Dim dr As DataRow = Nothing
        Dim dt As DataTable = Nothing
        Dim iSeqNo As Integer = 0

        '(10秒內不可重複存取)
        Dim i_SECX As Integer = 10
        Dim pms1 As New Hashtable From {{"OCID", OCIDValue1.Value}}
        Dim sql_1 As String = "SELECT 1 FROM dbo.CLASS_UNEXPECTVISITOR WHERE OCID=@OCID"
        Dim dt_1 As DataTable = DbAccess.GetDataTable(sql_1, objconn, pms1)
        If TIMS.dtHaveDATA(dt_1) Then
            '(10秒內不可重複存取)
            Dim pms2 As New Hashtable From {{"OCID", OCIDValue1.Value}, {"SECX", i_SECX}}
            Dim sql_2 As String = "SELECT 1 FROM dbo.CLASS_UNEXPECTVISITOR WHERE OCID=@OCID AND DATEDIFF(SECOND,modifydate,GETDATE())<@SECX AND DATEDIFF(SECOND,modifydate,GETDATE())>=0"
            Dim dt_2 As DataTable = DbAccess.GetDataTable(sql_2, objconn, pms2)
            If TIMS.dtHaveDATA(dt_2) Then
                Common.MessageBox(Me, String.Concat("(", i_SECX, "秒內不可重複存取)"))
                Return
            End If
        End If

        'dbo.CLASS_UNEXPECTVISITOR
        If UCase(rqState) = "ADD" Then '表示新增狀態
            '先取出最大SeqNo
            iSeqNo = Get_UEVSeqNoMax(objconn, OCIDValue1.Value)
            sql = "SELECT * FROM dbo.CLASS_UNEXPECTVISITOR WHERE 1<>1"
            dt = DbAccess.GetDataTable(sql, da, objconn)
            dr = dt.NewRow
            dt.Rows.Add(dr)
            dr("OCID") = OCIDValue1.Value
            dr("SeqNo") = iSeqNo
            dr("OrgID") = sm.UserInfo.OrgID
            dr("RID") = s_RID 'sm.UserInfo.RID
        Else
            iSeqNo = rqSeqNo
            sql = $"SELECT * FROM dbo.CLASS_UNEXPECTVISITOR WHERE OCID={rqOCID} and SeqNo={iSeqNo}"
            dt = DbAccess.GetDataTable(sql, da, objconn)
            dr = dt.Rows(0)
        End If

        'edit，by:20181018
        dr("ApplyDate") = If(flag_ROC, TIMS.Cdate18(ApplyDate.Text), TIMS.Cdate3(ApplyDate.Text))

        If Len(Applytime_HH.Value) < 2 Then '補0
            Applytime_HH.Value = "0" & Applytime_HH.Value
        End If
        If Len(Applytime_MM.Value) < 2 Then '補0
            Applytime_MM.Value = "0" & Applytime_MM.Value
        End If
        dr("ApplyTime") = $"{Applytime_HH.Value}:{Applytime_MM.Value}"

        dr("AuthCount") = If(AuthCount.Text <> "", CInt(AuthCount.Text), 0)
        dr("TurthCount") = If(TurthCount.Text <> "", CInt(TurthCount.Text), 0) 'TurthCount.Text
        dr("TruancyCount") = If(TruancyCount.Text <> "", CInt(TruancyCount.Text), 0) 'TruancyCount.Text
        dr("TurnoutCount") = If(TurnoutCount.Text <> "", CInt(TurnoutCount.Text), 0) 'TurnoutCount.Text
        dr("RejectCount") = If(RejectCount.Text <> "", CInt(RejectCount.Text), 0) 'RejectCount.Text
        dr("OtherCount") = If(OtherCount.Text <> "", CInt(OtherCount.Text), 0) 'OtherCount.Text

        dr("TechID") = If(OLessonTeah1Value.Value <> "", OLessonTeah1Value.Value, Convert.DBNull) '不可為空白
        dr("TechID2") = If(OLessonTeah2Value.Value <> "", OLessonTeah2Value.Value, Convert.DBNull) ' OLessonTeah2Value.Value

        CourseName.Text = TIMS.ClearSQM(CourseName.Text)
        dr("CourseName") = CourseName.Text
        'VISITWAY: 1:實地訪查  '2:視訊訪查
        Dim v_rblVISITWAY As String = TIMS.GetListValue(rblVISITWAY)
        dr("VISITWAY") = If(v_rblVISITWAY <> "", v_rblVISITWAY, "1")
        'AtteRate.Text = ""
        'If $"{dr("AtteRate")) <> "" Then AtteRate.Text = dr("AtteRate") * 100
        AtteRate.Text = TIMS.ClearSQM(AtteRate.Text)
        Dim v_AtteRate As String = ""
        If AtteRate.Text <> "" AndAlso TIMS.IsNumeric1(AtteRate.Text) Then
            v_AtteRate = TIMS.ROUND(Val(AtteRate.Text) / Val(100), 3)
        End If
        dr("AtteRate") = If(v_AtteRate <> "", Val(v_AtteRate), Convert.DBNull)

        dr("Data81") = If(DATA81.SelectedValue <> "", DATA81.SelectedValue, Convert.DBNull)
        dr("Data82") = If(DATA82.SelectedValue <> "", DATA82.SelectedValue, Convert.DBNull)
        dr("Data83") = If(DATA83.SelectedValue <> "", DATA83.SelectedValue, Convert.DBNull)
        dr("Data84") = If(DATA84.SelectedValue <> "", DATA84.SelectedValue, Convert.DBNull)

        dr("Item1") = If(Item1.SelectedValue <> "", Item1.SelectedValue, Convert.DBNull)
        dr("Item2") = If(Item2.SelectedValue <> "", Item2.SelectedValue, Convert.DBNull)
        dr("Item3") = If(Item3.SelectedValue <> "", Item3.SelectedValue, Convert.DBNull)
        dr("Item4") = If(Item4.SelectedValue <> "", Item4.SelectedValue, Convert.DBNull)

        dr("SITEMR3_1") = If(SITEMR3_1.SelectedValue <> "", SITEMR3_1.SelectedValue, Convert.DBNull)
        dr("SITEMR3_2") = If(SITEMR3_2.SelectedValue <> "", SITEMR3_2.SelectedValue, Convert.DBNull)
        dr("SITEMR3_3") = If(SITEMR3_3.SelectedValue <> "", SITEMR3_3.SelectedValue, Convert.DBNull)
        dr("SITEMR3_4") = If(SITEMR3_4.SelectedValue <> "", SITEMR3_4.SelectedValue, Convert.DBNull)

        dr("Stud_Name") = Stud_Name.Text
        dr("Stud_Name2") = Stud_Name2.Text
        dr("SItem1") = SItem1.SelectedValue
        dr("SItem1b") = SItem1b.SelectedValue

        dr("SItem2") = SItem2.SelectedValue
        'dr("SItem3") = SItem3.SelectedValue
        '1:是 2:否 3:其他 9:停用
        dr("SItem3") = "9"

        '四、現場處理說明
        dr("LItem1") = If(LItem1.Checked, "1", "2")
        dr("LItem2") = If(LItem2.Checked, "1", "2")
        dr("LItem2_1") = If(LItem2_1.Checked, "1", "2")

        Dim v_LItem2_1_Date As String = ""
        If LItem2_1_Date.Text <> "" Then
            v_LItem2_1_Date = If(flag_ROC, TIMS.Cdate18(LItem2_1_Date.Text), TIMS.Cdate3(LItem2_1_Date.Text))
        End If
        dr("LItem2_1_Date") = If(v_LItem2_1_Date <> "", v_LItem2_1_Date, Convert.DBNull)

        '1:勾選/2:未勾選
        dr("LItem2_2") = If(LItem2_2.Checked, "1", "2")
        Dim sLItem2_2b_VALUE As String = TIMS.GetCblValue(cblLItem2_2b)
        dr("LItem2_2b") = If(sLItem2_2b_VALUE <> "", sLItem2_2b_VALUE, Convert.DBNull)

        LItem2_2_Note.Text = TIMS.ClearSQM(LItem2_2_Note.Text)
        LItem2_3_Note.Text = TIMS.ClearSQM(LItem2_3_Note.Text)
        SItem1_Note.Text = TIMS.ClearSQM(SItem1_Note.Text)
        SItem1b_Note.Text = TIMS.ClearSQM(SItem1b_Note.Text)
        SItem2_Note.Text = TIMS.ClearSQM(SItem2_Note.Text)

        dr("LItem2_2_Note") = LItem2_2_Note.Text
        dr("LItem2_3_Note") = LItem2_3_Note.Text
        dr("SItem1_Note") = SItem1_Note.Text
        dr("SItem1b_Note") = SItem1b_Note.Text
        dr("SItem2_Note") = SItem2_Note.Text

        dr("CurseName") = CurseName.Text
        dr("VisitorName") = VisitorName.Text

        Dim vSITEM51 As String = TIMS.GetCblValue(cblSITEM51)
        Dim vSITEM61 As String = TIMS.GetCblValue(cblSITEM61)
        Dim vSITEM71 As String = TIMS.GetCblValue(cblSITEM71)
        Dim vSITEM81 As String = TIMS.GetCblValue(cblSITEM81)
        dr("SITEM51") = vSITEM51
        dr("SITEM61") = vSITEM61
        dr("SITEM71") = vSITEM71
        dr("SITEM81") = vSITEM81
        dr("SITEM51_NOTE") = TIMS.ClearSQM(SITEM51_NOTE.Text)
        dr("SITEM61_NOTE") = TIMS.ClearSQM(SITEM61_NOTE.Text)
        dr("SITEM71_NOTE") = TIMS.ClearSQM(SITEM71_NOTE.Text)
        dr("SITEM81_NOTE") = TIMS.ClearSQM(SITEM81_NOTE.Text)

        Dim vNOINC5 As String = TIMS.cst_NO
        If chkB_NOINC5.Checked Then vNOINC5 = TIMS.cst_YES
        dr("NOINC5") = vNOINC5

        dr("ModifyAcct") = sm.UserInfo.UserID
        dr("ModifyDate") = Now
        DbAccess.UpdateDataTable(dt, da)

        Dim pms_U As New Hashtable From {{"OCID", OCIDValue1.Value}, {"SeqNO", iSeqNo}}
        Dim sql_U As String = "UPDATE CLASS_UNEXPECTVISITOR SET MODIFYDATE=GETDATE() FROM CLASS_UNEXPECTVISITOR WHERE OCID=@OCID AND SeqNO=@SeqNO"
        DbAccess.ExecuteNonQuery(sql_U, objconn, pms_U)

        If Session("SearchStr") IsNot Nothing Then Session("SearchStr") = Session("SearchStr")

        Dim str_ALERT_MSG As String = cst_SAVE1_msg1
        Select Case flg_STR_STYPE5
            Case cst_sTYPE5_FLG_5 '有特殊狀況"5
                str_ALERT_MSG = cst_SAVE1_msg2
        End Select

        'Dim sScript1 As String = ""
        'If rqType = "CV" Then
        '    sScript1 = "<script language=javascript>window.alert('" + str_ALERT_MSG + "');"
        '    sScript1 &= "window.location.href='CP_01_008.aspx?ID=" & RqID & "';</script>"
        'Else
        '    sScript1 = "<script language=javascript>window.alert('" & str_ALERT_MSG & "');"
        '    sScript1 &= "window.location.href='CP_01_006.aspx?ID=" & RqID & "';</script>"
        'End If
        'Common.RespWrite(Me, TIMS.sUtl_AntiXss(sScript1))

        Dim RqID As String = TIMS.Get_MRqID(Me)
        Dim uUrl1 As String = "CP/01/CP_01_006.aspx?ID=" & RqID
        If rqType = "CV" Then uUrl1 = "CP/06/CP_01_008.aspx?ID=" & RqID
        Call TIMS.BlockAlert(Me, str_ALERT_MSG, uUrl1)
    End Sub

    '儲存
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        Dim Errmsg As String = ""
        Call CheckData1(Errmsg)
        If Errmsg <> "" Then
            Common.MessageBox(Page, Errmsg)
            Exit Sub
        End If

        Call sSaveData1()
    End Sub

    'Private Sub Button4_ServerClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button4.ServerClick
    '    'Session("_SearchStr") = ViewState("_SearchStr")
    '    If Not Session("SearchStr") Is Nothing Then Session("SearchStr") = Session("SearchStr")
    'End Sub

    ''' <summary>
    ''' 回查詢頁面
    ''' </summary>
    Sub sBack2Prev()
        '訪視計畫表用
        Dim RqID As String = TIMS.Get_MRqID(Me)
        Dim rqType As String = TIMS.sUtl_GetRqValue(Me, "Type")
        'Dim rqType As String = UCase(TIMS.ClearSQM(Request("Type")))
        Select Case rqType
            Case "CV"
                Dim url1 As String = "CP_01_008.aspx?ID=" & RqID 'Request("ID")
                TIMS.Utl_Redirect(Me, objconn, url1)
            Case Else
                Dim url1 As String = "CP_01_006.aspx?ID=" & RqID 'Request("ID")
                TIMS.Utl_Redirect(Me, objconn, url1)
        End Select
    End Sub

    '回查詢頁面
    Private Sub Button5_ServerClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button5.ServerClick
        '訪視計畫表用
        If Session("SearchStr") IsNot Nothing Then Session("SearchStr") = Session("SearchStr")
        Call sBack2Prev()
    End Sub
End Class