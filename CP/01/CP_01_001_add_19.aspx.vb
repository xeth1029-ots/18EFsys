Public Class CP_01_001_add_19
    Inherits AuthBasePage

    'CLASS_VISITOR3
    'CLASS_VISITOR
    'VIEW_VISITOR(CLASS_VISITOR/CLASS_VISITOR3)
    'CP_01_001_19*.jrxml --2019--
    'CP_01_001_add_19*.jrxml --2019--
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            If rqOCID <> "" AndAlso rqSeqNo <> "" Then
                create(rqOCID, rqSeqNo)
            Else
                LOAD_CLSINFO(OCIDValue1.Value)
            End If
...
#End Region
    ...
    Sub create(ByVal OCID As String, ByVal SeqNo As String)
        ...
        Call LOAD_CLSINFO(OCID)
        Call LOAD_DATA_1(OCID, SeqNo)
    End Sub
    ...
    Sub LOAD_CLSINFO(ByVal OCID As String)
        '(檢視或修改)一定會有OCID
        Dim drOC As DataRow = TIMS.GetOCIDDate(OCID)
        If drOC Is Nothing Then Exit Sub

        center.Text = drOC("OrgName").ToString
        RIDValue.Value = drOC("RID").ToString
        TMID1.Text = "[" & drOC("TrainID") & "]" & drOC("TrainName").ToString
        TMIDValue1.Value = drOC("TMID").ToString
        OCID1.Text = drOC("CLASSCNAME2").ToString
        OCIDValue1.Value = drOC("OCID").ToString
        center.Enabled = False
        TMID1.Enabled = False
        OCID1.Enabled = False
        Button2.Disabled = True
        Button3.Disabled = True
        APPROVEDCOUNT.Text = drOC("TNUM").ToString '核定 人數
        AUTHCOUNT.Text = TIMS.GET_OCIDInfo(OCID, "STUDCOUNT", objconn) '開訓 人數
        REJECTCOUNT.Text = TIMS.GET_OCIDInfo(OCID, "STUDCOUNT2", objconn) '離退訓 人數
        '核定：<asp:TextBox ID="APPROVEDCOUNT" runat="server" MaxLength="3" Width="10%"></asp:TextBox>人； 開訓：<asp:TextBox ID="AUTHCOUNT" runat="server" 
    ...
    Sub LOAD_DATA_1(ByVal OCID As String, ByVal SeqNo As String)
        OCID = TIMS.ClearSQM(OCID)
        SeqNo = TIMS.ClearSQM(SeqNo)
        ...
        Dim sql As String = ""
        sql = ""
        sql += " SELECT * FROM CLASS_VISITOR3 WHERE 1=1 AND OCID = '" & OCID & "'  " & vbCrLf
        sql += " AND SEQNO = '" & SeqNo & "' "
        ...
        ITEM31NOTE.Text = Convert.ToString(dr("ITEM31NOTE"))
        'If Convert.ToString(dr("ITEM32")) <> "" Then Common.SetListItem(ITEM32, Convert.ToString(dr("ITEM32")))
        TIMS.SetCblValue(CBL_ITEM32, Convert.ToString(dr("CBL_ITEM32")))
        ITEM32NOTE.Text = Convert.ToString(dr("ITEM32NOTE"))
    ...
    Function CheckData1(ByRef Errmsg As String) As Boolean
        ...
        Else
            ...
            End If
        End If
        If APPROVEDCOUNT.Text <> "" Then APPROVEDCOUNT.Text = Trim(APPROVEDCOUNT.Text)
        If AUTHCOUNT.Text <> "" Then AUTHCOUNT.Text = Trim(AUTHCOUNT.Text)
        If TURTHCOUNT.Text <> "" Then TURTHCOUNT.Text = Trim(TURTHCOUNT.Text)
        If TURNOUTCOUNT.Text <> "" Then TURNOUTCOUNT.Text = Trim(TURNOUTCOUNT.Text)
        If TRUANCYCOUNT.Text <> "" Then TRUANCYCOUNT.Text = Trim(TRUANCYCOUNT.Text)
        If REJECTCOUNT.Text <> "" Then REJECTCOUNT.Text = Trim(REJECTCOUNT.Text)
        ...
        ITEM7NOTE.Text = TIMS.ClearSQM(ITEM7NOTE.Text)
        If ITEM7NOTE.Text.Length > 200 Then
            Errmsg += "參訓學員反映意見及問題 可輸入文字長度為(限) 200 個中文字！ " & vbCrLf
        End If
        ITEM7NOTE2.Text = TIMS.ClearSQM(ITEM7NOTE2.Text)
        If ITEM7NOTE2.Text.Length > 300 Then
            Errmsg += "學員反映意見之委訓單位回應說明 可輸入文字長度(限) 300 個中文字！ " & vbCrLf
        End If
        ITEM31NOTE.Text = TIMS.ClearSQM(ITEM31NOTE.Text)
        If ITEM31NOTE.Text.Length > 200 Then
            Errmsg += "綜合建議 可輸入文字長度(限) 200 個中文字！ " & vbCrLf
        End If
        'ITEM32NOTE
        ITEM32NOTE.Text = TIMS.ClearSQM(ITEM32NOTE.Text)
        If ITEM32NOTE.Text.Length > 100 Then
            Errmsg += "缺失處理 其它(請說明) 可輸入文字長度(限) 100 個中文字！ " & vbCrLf
        End If
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        If rOCID <> "" Then
            ...
            Call SAVE_UPDATE_DATA(iSEQNO)
        End If
        If rOCID = "" Then Call SAVE_INSERT_DATA() '表示新增狀態

        If Session("SearchStr") Is Nothing And ViewState("SearchStr") IsNot Nothing Then Session("SearchStr") = Me.ViewState("SearchStr")
        If Session("_SearchStr") Is Nothing And ViewState("_SearchStr") IsNot Nothing Then Session("_SearchStr") = Me.ViewState("_SearchStr")
    ...
    Protected Sub Button4_Click(sender As Object, e As EventArgs) Handles Button4.Click
        ...
        Dim url1 As String = ""
        url1 = "CP_01_001.aspx?ID=" & rqMID 'Request("ID")
        If rqDOCID <> "" Then url1 = "CP_01_001.aspx?ID=" & rqMID & "&DOCID=" & rqDOCID 'Request("DOCID")
    ...
    Sub SAVE_INSERT_DATA()
        ...
        If D3C2.Checked Then iD3C = 2
        If D3C3.Checked Then iD3C = 3
        Dim vCBL_ITEM32 As String = TIMS.GetCblValue(CBL_ITEM32)
        ...
        With iCMD
            ...
            .Parameters.Add("CBL_ITEM32", SqlDbType.VarChar).Value = If(vCBL_ITEM32 <> "", vCBL_ITEM32, Convert.DBNull)
    ...
    Sub SAVE_UPDATE_DATA(ByVal iSEQNO As Integer)
        'CLASS_VISITOR3
        Call TIMS.OpenDbConn(objconn)
        Dim sql As String = ""
        sql = ""
        ...
        sql &= "   ,CBL_ITEM32=@CBL_ITEM32 " & vbCrLf
        ...
        With uCMD
            ...
            .Parameters.Add("CBL_ITEM32", SqlDbType.VarChar).Value = If(vCBL_ITEM32 = "", Convert.DBNull, vCBL_ITEM32)
            .Parameters.Add("ITEM32NOTE", SqlDbType.VarChar).Value = ITEM32NOTE.Text
            .Parameters.Add("CURSENAME", SqlDbType.VarChar).Value = CURSENAME.Text '培訓單位人員姓名
            .Parameters.Add("VISITORNAME", SqlDbType.VarChar).Value = VISITORNAME.Text '訪視人員姓名
            .Parameters.Add("RID", SqlDbType.VarChar).Value = sm.UserInfo.RID
