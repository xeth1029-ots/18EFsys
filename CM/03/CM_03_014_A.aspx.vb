Partial Class CM_03_014_A
    ...
    '往下一層的搜尋值（'回上一層的搜尋值）
    Sub KeepSearch2(Optional ByVal ArgValue As String = "")
        Const Cst_MySearch As String = "_MySearch"
    ...
    Sub Search1()
        ...
        Dim sql As String = ""
        sql = "" & vbCrLf
        sql &= " select Kp.planname, vp.TPlanID, vp.Years " & vbCrLf
        sql &= " ,vp.DistID ,id1.Name DistName ,vp.PlanID " & vbCrLf
        sql &= " 	,sum (ISNULL(gc.trainClass,0)) trainClass" & vbCrLf
        sql &= " 	,sum (ISNULL(gs.trainNum,0)) trainNum" & vbCrLf
        sql &= " 	,sum (ISNULL(gs.closeNum,0)) closeNum" & vbCrLf
        sql &= " 	,sum (ISNULL(gs.JobNum,0)) JobNum" & vbCrLf
        sql &= " 	,sum (ISNULL(gs.NJobNum,0)) NJobNum" & vbCrLf
        sql &= " 	,sum (ISNULL(gs.xJobNum,0)) xJobNum" & vbCrLf
        ...
        sql &= "  join (" & vbCrLf
        sql &= " 	select ip.PlanID,ip.TPlanID " & vbCrLf
        sql &= " 	,COUNT(1) trainClass " & vbCrLf
        ...
        End If
        sql &= " 	)" & vbCrLf
        sql &= " 	group by ip.PlanID,ip.TPlanID " & vbCrLf
        sql &= " ) gc on vp.PlanID=gc.PlanID " & vbCrLf
        sql &= " left join (" & vbCrLf
        sql &= " 	select ip.PlanID,ip.TPlanID " & vbCrLf
        sql &= " 	,COUNT(1) trainNum " & vbCrLf
        ...
        sql &= " 	,sum(case when cs.studstatus in (5) and ISNULL(sg.IsGetJob,'99') not in ('1','2') then 1 else 0 end) xJobNum --x1x2.未就業" & vbCrLf
        sql &= " 	from class_classinfo cc" & vbCrLf
        sql &= " 	join plan_planinfo pp on pp.planid =cc.planid and pp.comidno =cc.comidno and pp.seqno =cc.seqno " & vbCrLf
        sql &= " 	join id_plan ip on ip.planid=cc.planid" & vbCrLf
        ...
        sql &= " 	)" & vbCrLf
        sql &= " 	group by ip.PlanID,ip.TPlanID " & vbCrLf
        sql &= " ) gs on vp.PlanID=gs.PlanID " & vbCrLf
        sql &= " where 1=1" & vbCrLf
        ...
        sql &= " GROUP BY " & vbCrLf
        sql &= "  Kp.planname, vp.TPlanID, vp.Years " & vbCrLf
        sql &= " ,vp.DistID ,id1.Name ,vp.PlanID" & vbCrLf
        sql &= " ORDER BY " & vbCrLf
        sql &= "  Kp.planname, vp.TPlanID, vp.Years " & vbCrLf
        sql &= " ,vp.DistID ,id1.Name ,vp.PlanID" & vbCrLf
    ...
    Private Sub Button3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button3.Click
        Call KeepSearch2()
        TIMS.Utl_Redirect1(Me, "CM_03_014.aspx?ID=" & Request("ID"))
    End Sub
    ...
    Private Sub DataGrid1_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles DataGrid1.ItemCommand
        ...
        Select Case e.CommandName
            Case "searchB"
                Call KeepSearch2(e.CommandArgument)
                TIMS.Utl_Redirect1(Me, "CM_03_014_B.aspx?ID=" & Request("ID"))
        End Select
    ...
    Private Sub DataGrid1_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles DataGrid1.ItemDataBound
        Select Case e.Item.ItemType
            ...
            Case ListItemType.Item, ListItemType.AlternatingItem
                ...
                LinkButton1.Text &= drv("DistName").ToString
                ...
                ArgValue += "&Yearsb=" & drv("Years")
                ArgValue += "&DistIDb=" & drv("DistID")
                ArgValue += "&TPlanIDb=" & drv("TPlanID")
                ArgValue += "&PlanIDb=" & drv("PlanID")
