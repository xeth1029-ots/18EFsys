...
Partial Class TC_11_001
    ...
    'KEY_BIDCASE/ORG_BIDCASE/ORG_BIDCASEFL/ORG_BIDCASEPI/ORG_BIDCASEFL_PI
    'ORG_BIDCASEFL_PI3/ORG_BIDCASEFL_TT /ORG_BIDCASEFL_TT2 /ORG_BIDCASEFL_RT
    ...
    'AND pp.TPLANID='28' AND pp.DISTID='001' AND pp.YEARS='2023' AND pp.APPSTAGE='1'
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If TIMS.Cst_TPlanID54.IndexOf(sm.UserInfo.TPlanID) > -1 Then
            Dim url1 As String = $"TC_11_001_54.aspx?ID={TIMS.Get_MRqID(Me)}"  '(54:充電起飛計畫（在職）)切換程式
        ...
        Dim jsScript2 As String = Common.GetJsString($"{Cst_messages2}{vbCrLf}確定是否要更新班級清單?")
        'BTN_RETRIEVE.Attributes("onclick")=$"return confirm('{jsScript2}');"
        BTN_SAVENEXT2.Attributes("onclick") = $"return confirm('{jsScript2}');"
    ...
    Sub CCreate11()
        '更新班級清單
        'tr_BTN_RETRIEVE.Visible = True
        FrameTableEdt2.Visible = False
        'If fg_test Then FrameTableEdt2.Visible=fg_test '(測試狀態，常態顯示)
    ...
    Protected Sub BTN_ADDNEW1_Click(sender As Object, e As EventArgs) Handles BTN_ADDNEW1.Click
        ...
        Dim fg_CanADDNEW1 As Boolean = Utl_ADDNEW_DATA1(sERRMSG, drRR, -1, "")
        If sERRMSG <> "" Then
    ...
    ''' <returns></returns>
    Private Function Utl_ADDNEW_DATA1(ByRef sERRMSG As String, ByRef drRR As DataRow, iBCID As Integer, vBCASENO_NN As String) As Boolean
        ...
        '(新增檢核使用)
        If iBCID <= 0 AndAlso vBCASENO_NN = "" Then
            ...
            Dim sSql2 As String = "SELECT 1 FROM ORG_BIDCASE WHERE PLANID=@PLANID AND RID=@RID AND APPSTAGE=@APPSTAGE"
        ...
        Hid_KBSID.Value = ""
        Hid_KBID.Value = ""
        'Dim vBCASENO_NN As String="" 'Dim iBCID As Integer=0
        If iBCID <= 0 AndAlso vBCASENO_NN = "" Then
            Try
                ...
                vBCASENO_NN = GET_BCASENO_NN(objconn, irParms)
                ...
                isSql &= " ,PLANID,RID,APPSTAGE,CREATEACCT,CREATEDATE,MODIFYACCT,MODIFYDATE)" & vbCrLf ',BIDACCT,BIDDATE,BISTATUS
                isSql &= " VALUES (@BCID,@BCASENO,@YEARS,@DISTID,@ORGID" & vbCrLf
                isSql &= " ,@PLANID,@RID,@APPSTAGE,@CREATEACCT,GETDATE(),@MODIFYACCT,GETDATE())" & vbCrLf ',@BIDACCT,@BIDDATE,@BISTATUS
                DbAccess.ExecuteNonQuery(isSql, objconn, iParms)
            Catch ex As Exception
                sERRMSG = "資料庫序號有誤，請重新操作!"
                Return False
            End Try
        ElseIf iBCID > 0 AndAlso vBCASENO_NN <> "" Then
            Dim UParms As New Hashtable From {{"MODIFYACCT", sm.UserInfo.UserID}, {"BCID", iBCID}, {"BCASENO", vBCASENO_NN}}
            Dim USql As String = ""
            USql &= " UPDATE ORG_BIDCASE SET MODIFYACCT=@MODIFYACCT,MODIFYDATE=GETDATE()"
            USql &= " WHERE BCID=@BCID AND BCASENO=@BCASENO" & vbCrLf
        ...
        Dim drOB As DataRow = TIMS.GET_ORG_BIDCASE(objconn, v_RID, Hid_BCID.Value, Hid_BCASENO.Value)
        If drOB IsNot Nothing Then
            'If drOB Is Nothing Then Return False
            Dim vYEARS As String = TIMS.ClearSQM(drOB("YEARS"))
            Dim vAPPSTAGE As String = TIMS.ClearSQM(drOB("APPSTAGE")) 'TIMS.GetMyValue2(rPMS, "APPSTAGE")
            '關閉(更新班級清單) Close (Update Class List) CLOSE_RETRIEVE
            Dim drST As DataRow = TIMS.GET_APPLISTAGE_PTYPE01_DR(objconn, vYEARS, vAPPSTAGE)
            If drST IsNot Nothing Then
                If $"{drST("CLOSE_RETRIEVE")}" = "Y" Then tr_BTN_RETRIEVE.Visible = False
            End If
        End If
    ...
    Private Sub SAVE_ORG_BIDCASEPI(ByVal iBCID As Integer)
        If iBCID = 0 OrElse Hid_PCS.Value = "" Then Return
        ...
        For Each sPCS1 As String In saPCSALL1
            ...
            If saPCS1.Length = 3 Then
                ...
                If dt3.Rows.Count = 0 Then
                    iBCPID = DbAccess.GetNewId(objconn, "ORG_BIDCASEPI_BCPID_SEQ,ORG_BIDCASEPI,BCPID")
    ...
    Private Sub SAVE_ORG_BIDCASEFL_PI3(drOB As DataRow, drKB As DataRow, drRR As DataRow, rPMS As Hashtable)
        ...
        If drFLP3 Is Nothing Then
            Dim isSql As String = ""
            isSql &= " INSERT INTO ORG_BIDCASEFL_PI3(BCFP3ID,BCFID,BCID,KBSID,BCPID ,FILENAME1,SRCFILENAME1, MODIFYACCT,MODIFYDATE)" & vbCrLf
            isSql &= " VALUES (@BCFP3ID,@BCFID,@BCID,@KBSID,@BCPID ,@FILENAME1,@SRCFILENAME1, @MODIFYACCT,GETDATE())" & vbCrLf

            Dim iBCFP3ID As Integer = DbAccess.GetNewId(objconn, "ORG_BIDCASEFL_PI3_BCFP3ID_SEQ,ORG_BIDCASEFL_PI3,BCFP3ID")
            'iParmsP.Add("MEMO1", vMEMO1) 'iParmsP.Add("WAIVED", vWAIVED)
...
FROM ORG_BIDCASEFL_PI3 WHERE BCFID=@BCFID AND BCID=@BCID AND KBSID=@KBSID AND BCPID=@BCPID AND BCFP3ID=@BCFP3ID"
    ...
    Sub SAVE_ORG_BIDCASEFL_TT(drOB As DataRow, drKB As DataRow, drRR As DataRow, ByRef rPMS As Hashtable)
        ...
        Dim fPMST As New Hashtable From {{"BCFID", iBCFID}, {"BCID", TIMS.CINT1(vBCID)}, {"KBSID", TIMS.CINT1(vKBSID)}, {"TECHID", TIMS.CINT1(vTECHID)}}
        ...
        If drFLT Is Nothing Then
            ...
            'iParmsT.Add("PATTERN", vPATTERN)
            'iParmsT.Add("MEMO1", vMEMO1)
            'iParmsT.Add("WAIVED", vWAIVED)
...
FROM ORG_BIDCASEFL_TT WHERE BCFID=@BCFID AND BCID=@BCID AND KBSID=@KBSID AND TECHID=@TECHID AND BCFTID=@BCFTID"
    ...
    Sub SAVE_ORG_BIDCASEFL_TT2(drOB As DataRow, drKB As DataRow, drRR As DataRow, ByRef rPMS As Hashtable)
        ...
        Dim fPMST2 As New Hashtable From {{"BCFID", iBCFID}, {"BCID", TIMS.CINT1(vBCID)}, {"KBSID", TIMS.CINT1(vKBSID)}, {"TECHID", TIMS.CINT1(vTECHID)}}
...
FROM ORG_BIDCASEFL_TT2 WHERE BCFID=@BCFID AND BCID=@BCID AND KBSID=@KBSID AND TECHID=@TECHID AND BCFT2ID=@BCFT2ID"
    ...
    Private Sub DataGrid1_ItemCommand(source As Object, e As DataGridCommandEventArgs) Handles DataGrid1.ItemCommand
        ...
        Select Case e.CommandName
            ...
            Case cst_DG1CMDNM_SENDOUT1 'SENDOUT1 送出 
                ...
                'Dim vORGKINDGW As String=$"{drOB("ORGKINDGW")}"
                Dim iProgress As Integer = TIMS.GET_iPROGRESS_BI(sm, objconn, tmpMSG, vBCID, vORGKINDGW)
                Dim EMSG As String = ""
                If iProgress < 100 Then
                    EMSG = String.Concat("線上申辦進度 未達100%，不可送出!", vbCrLf, If(tmpMSG <> "", $"請檢查：({tmpMSG})", ""))
                ...
                If dtPI IsNot Nothing AndAlso dtPI.Rows.Count > 0 Then
                    For Each drPI As DataRow In dtPI.Rows
                        ...
                        If Not fg_PP1_ALL Then
                            EMSG = $"請再次確認所有班級均已送審!於「班級申請」未按送出!_{s_CLASSNAME}"
                            Common.MessageBox(Me, EMSG)
                            Return
                        End If
                    Next
    ...
    Private Sub SHOW_Detail_BIDCASE(ByRef drRR As DataRow, vBCID As String, vCmdName As String)
        '訓練機構有誤
        ...
        'Call Utl_SET_ddlSWITCHTO_VAL(Hid_ORGKINDGW.Value)
        'Dim v_sch_ddlYEARS As String=TIMS.GetListValue(sch_ddlYEARS)
        Dim v_sch_ddlAPPSTAGE As String = TIMS.GetListValue(sch_ddlAPPSTAGE)
        ...
        Dim S1Parms As New Hashtable From {{"TPLANID", sm.UserInfo.TPlanID}, {"RID", vRID}, {"PLANID", vPLANID}, {"APPSTAGE", If(vAPPSTAGE <> "", vAPPSTAGE, v_sch_ddlAPPSTAGE)}}
    ...
    Private Sub RESET_SWITCHTO_VAL(ByRef ddlSwitchTo As DropDownList)
        ...
        Dim sql As String = "SELECT KBSID FROM KEY_BIDCASE WITH(NOLOCK) WHERE KBID='13-1'"
    ...
    Sub SAVEDATA2_BTN_ACTION1(ByVal s_ACTTYPE As String)
        ...
        'Dim vWAIVED As String=If(drFL IsNot Nothing, $"{drFL("WAIVED")}", "")
...
#Region "共用"
    ...
    ''' <returns></returns>
    Public Shared Function GET_BCASENO_NN(ByRef oConn As SqlConnection, ByRef rParms As Hashtable) As String
        ...
        'Dim sSql As String=""
        'sSql &= " SELECT TOP 11 BCASENO,SUBSTRING(BCASENO,1,9) T1" & vbCrLf
        'sSql &= " ,SUBSTRING(BCASENO,9,9) T2,cast(SUBSTRING(BCASENO,9,9) as int) iCASENO" & vbCrLf
        'sSql &= " ,SUBSTRING(BCASENO,10,9) T2,cast(SUBSTRING(BCASENO,10,9) as int) iCASENO" & vbCrLf
        'sSql &= " FROM ORG_BIDCASE" & vbCrLf
...
#Region "Private1"
    ...
    Private Sub SHOW_DATAGRID_14()
        ...
        'ORG_BIDCASEPI/ORG_BIDCASEFL_EV,ORG_BIDCASEFL_PI3
    ...
    Private Sub SHOW_DATAGRID_13()
        ...
        'ORG_BIDCASEPI/ORG_BIDCASEFL_EV
        Dim sParms1 As New Hashtable From {{"BCID", vBCID}, {"RID", vRID}, {"AppStage", vAPPSTAGE}}
        Dim sSql As String = ""
        sSql &= "WITH WPI1 AS ( SELECT a.BCID,a.BCPID,a.PLANID,a.COMIDNO,a.SEQNO" & vbCrLf
        sSql &= " FROM ORG_BIDCASEPI a" & vbCrLf
        sSql &= " WHERE a.BCID=@BCID )" & vbCrLf

        sSql &= ",WFPI2 AS ( SELECT b.BCFEID,a.BCPID,a.PLANID,a.COMIDNO,a.SEQNO" & vbCrLf
    ...
    Private Sub SHOW_DATAGRID_13B()
        ...
        sSql &= ",WFRT2 AS ( SELECT b.BCRTID,a.BCPID" & vbCrLf
    ...
    Private Sub SHOW_DATAGRID_11()
        ...
        Dim dtPTT2 As DataTable = TIMS.GET_PLAN_TEACHER1_TT2(objconn, PMS_S2)

#Region "(資料異常清理)"
        '(資料異常清理)-'11.各授課師資學／經歷證書影本
        Dim dtFLTT2 As DataTable = TIMS.GET_ORG_BIDCASEFL_TT2(objconn, $"{drOB("BCID")}")

        Dim iPTT2 As Integer = If(TIMS.dtHaveDATA(dtPTT2), dtPTT2.Rows.Count, 0)
        Dim iFLTT2 As Integer = If(TIMS.dtHaveDATA(dtFLTT2), dtFLTT2.Rows.Count, 0)
        '(資料異常清理)
        If iFLTT2 > iPTT2 Then
            For Each drFLTT2 As DataRow In dtFLTT2.Rows
                If dtPTT2.Select($"TECHID={drFLTT2("TECHID")}").Length = 0 Then
                    '(查無師資)(刪除經歷證書影本)
                    Dim oYEARS As String = $"{drOB("YEARS")}"
                    Dim oAPPSTAGE As String = $"{drOB("APPSTAGE")}"
                    Dim oPLANID As String = $"{drOB("PLANID")}"
                    ...
                    Dim f_KBSID As String = ""
                    Dim oFILENAME1 As String = "" 'Convert.ToString(drFLTT("FILENAME1"))
                    Dim oUploadPath As String = "" 'TIMS.GET_UPLOADPATH1(oYEARS, oAPPSTAGE, oPLANID, oRID, oBCASENO, vKBSID)
                    Dim s_FilePath1 As String = "" 'MyPage.Server.MapPath(Path.Combine(oUploadPath, oFILENAME1))
                    Try
                        f_KBSID = $"{drFLTT2("KBSID")}"
                        oFILENAME1 = $"{drFLTT2("FILENAME1")}"
                        oUploadPath = TIMS.GET_UPLOADPATH1_BI(oYEARS, oAPPSTAGE, oPLANID, oRID, oBCASENO, f_KBSID)
                        s_FilePath1 = Server.MapPath(Path.Combine(oUploadPath, oFILENAME1))
                        Call TIMS.MyFileDelete(s_FilePath1)
                    Catch ex As Exception
                        Dim strErrmsg As String = String.Concat(New Diagnostics.StackFrame(True).GetMethod().Name, vbCrLf)
                    ...
                    Dim PMS_D1 As New Hashtable From {{"BCFT2ID", drFLTT2("BCFT2ID")}}
                    Dim SQL_D1 As String = "DELETE ORG_BIDCASEFL_TT2 WHERE BCFT2ID=@BCFT2ID"
                    DbAccess.ExecuteNonQuery(SQL_D1, objconn, PMS_D1)
                End If
            Next
        End If
#End Region
        ...
        labmsg2.Text = ""
        If TIMS.dtNODATA(dtPTT2) Then
            labmsg2.Text = TIMS.cst_NODATAMsg1
            Return
        End If
        iDG11_ROWS = dtPTT2.Rows.Count
        ...
        Dim vKBSID As String = $"{drKB("KBSID")}"
        Dim download_Path As String = TIMS.GET_DOWNLOADPATH1_BI(vYEARS, vAPPSTAGE, vPLANID, vRID, vBCASENO, vKBSID)
        Call TIMS.Check_dtBIDCASEFL(Me, dtPTT2, download_Path)
    ...
    Private Sub SHOW_DATAGRID_10()
        ...
        'DataGrid10_ItemDataBound '10.師資助教基本資料表
        Dim PMS_S1 As New Hashtable From {{"BCID", $"{drOB("BCID")}"}, {"RID", $"{drOB("RID")}"}, {"AppStage", $"{drOB("AppStage")}"}}
        Dim dtPTT1 As DataTable = TIMS.GET_PLAN_TEACHER1_TT(objconn, PMS_S1)

#Region "(資料異常清理)"
        '(資料異常清理)-'10.師資助教基本資料表
        Dim dtFLTT As DataTable = TIMS.GET_ORG_BIDCASEFL_TT(objconn, $"{drOB("BCID")}")

        Dim iPTT1 As Integer = If(TIMS.dtHaveDATA(dtPTT1), dtPTT1.Rows.Count, 0)
        Dim iFLTT As Integer = If(TIMS.dtHaveDATA(dtFLTT), dtFLTT.Rows.Count, 0)
        '(資料異常清理)
        If iFLTT > iPTT1 Then
            For Each drFLTT As DataRow In dtFLTT.Rows
                If dtPTT1.Select($"TECHID={drFLTT("TECHID")}").Length = 0 Then
                    '(查無師資)(刪除)(師資助教基本資料表)
                    Dim oYEARS As String = $"{drOB("YEARS")}"
                    Dim oAPPSTAGE As String = $"{drOB("APPSTAGE")}"
                    Dim oPLANID As String = $"{drOB("PLANID")}"
                    ...
                    Dim s_FilePath1 As String = "" 'MyPage.Server.MapPath(Path.Combine(oUploadPath, oFILENAME1))
                    Try
                        f_KBSID = $"{drFLTT("KBSID")}"
                        oFILENAME1 = $"{drFLTT("FILENAME1")}"
                        oUploadPath = TIMS.GET_UPLOADPATH1_BI(oYEARS, oAPPSTAGE, oPLANID, oRID, oBCASENO, f_KBSID)
                        s_FilePath1 = Server.MapPath(Path.Combine(oUploadPath, oFILENAME1))
                        Call TIMS.MyFileDelete(s_FilePath1)
                    Catch ex As Exception
                    ...
                    Dim PMS_D1 As New Hashtable From {{"BCFTID", drFLTT("BCFTID")}}
                    Dim SQL_D1 As String = "DELETE ORG_BIDCASEFL_TT WHERE BCFTID=@BCFTID"
                    DbAccess.ExecuteNonQuery(SQL_D1, objconn, PMS_D1)
                End If
            Next
        End If
#End Region
        ...
        labmsg2.Text = ""
        If TIMS.dtNODATA(dtPTT1) Then
            labmsg2.Text = TIMS.cst_NODATAMsg1
            Return
        End If
        iDG10_ROWS = dtPTT1.Rows.Count
        ...
        Dim vKBSID As String = $"{drKB("KBSID")}"
        Dim download_Path As String = TIMS.GET_DOWNLOADPATH1_BI(vYEARS, vAPPSTAGE, vPLANID, vRID, vBCASENO, vKBSID)
        Call TIMS.Check_dtBIDCASEFL(Me, dtPTT1, download_Path)
    ...
    Private Sub SHOW_DATAGRID_08()
        ...
        sSql1 &= " WHERE a.IsApprPaper='Y' AND a.RESULTBUTTON IS NULL AND a.RID=@RID AND a.AppStage=@AppStage" & vbCrLf
        If sm.UserInfo.RID = "A" Then
            sParms1.Add("TPlanID", sm.UserInfo.TPlanID)
            sParms1.Add("Years", sm.UserInfo.Years)
...
#End Region
    ...
    Function SAVE_ORG_BIDCASEFL_UPLOAD(ByRef rPMS As Hashtable) As Integer
        ...
        Else
            ...
            'iParms.Add("MODIFYDATE", MODIFYDATE)
            Dim isSql As String = ""
            isSql &= " INSERT INTO ORG_BIDCASEFL(BCFID,YEARS,APPSTAGE,RID,BCID,KBSID,WAIVED"
            isSql &= " ,FILENAME1,SRCFILENAME1,PATTERN,MEMO1,MODIFYACCT,MODIFYDATE)" & vbCrLf
            isSql &= " VALUES (@BCFID,@YEARS,@APPSTAGE,@RID,@BCID,@KBSID,@WAIVED"
    ...
    Sub SHOW_BIDCASEFL_DG2(ByRef rPMS As Hashtable)
        ...
        Dim aParms As New Hashtable From {{"YEARS", vYEARS}, {"APPSTAGE", vAPPSTAGE}}
        Dim fg_can_applistage As Boolean = TIMS.CAN_APPLISTAGE_PTYPE01(objconn, aParms)
        '(非)申請階段管理-受理期間-檢核查詢 'Common.MessageBox(Me, cst_stopmsg_11) ' "申請階段受理期間未開放，請確認後再操作!" 'Return
        If Not fg_can_applistage AndAlso tr_BTN_RETRIEVE.Visible Then
            BTN_RETRIEVE.Enabled = False
            TIMS.Tooltip(BTN_RETRIEVE, If(BTN_RETRIEVE.Enabled, "", cst_stopmsg_11), True)
        End If
    ...
    Private Sub SAVE_ORG_BIDCASE_ALL_13B(rPMS As Hashtable)
        ...
        For Each eItem As DataGridItem In oDG1.Items
            ...
            End If
            If fg_RUN_REPORT_1 Then
                Dim rPMS4 As New Hashtable From {{"YEARS", vYEARS}, {"selsqlstr", $"{vHDG_PlanID}-{vHDG_ComIDNO}-{vHDG_SeqNo}"}, {"TPlanID", sm.UserInfo.TPlanID}} 'rPMS4.Clear()
    ...
    Private Sub SAVE_ORG_BIDCASEFL_RT_PDF_FILE(ByRef rPMS As Hashtable, ByRef s_PDF_byte As Byte())
        ...
        Dim sParms3 As New Hashtable From {{"BCFID", vBCFID}, {"BCID", vBCID}, {"KBSID", vKBSID}, {"BCPID", vBCPID}}
        ...
        If dt3.Rows.Count = 0 Then
            ...
            DbAccess.ExecuteNonQuery(isSql, objconn, iParms)
        Else
            'iParms.Add("PATTERN", PATTERN) 'iParms.Add("MEMO1", MEMO1) 'iParms.Add("WAIVED", WAIVED) ',PATTERN=@PATTERN,MEMO1=@MEMO1,WAIVED=@WAIVED
    ...
    Function GET_OLDFILE1(ByRef rPMS As Hashtable) As DataRow
        ...
        'sSql &= " AND CONCAT(kb.ORGKINDGW,kb.KBID)='W01'" & vbCrLf
        'sSql &= " AND a.ORGID=3901" & vbCrLf
        'sSql &= " AND a.DISTID='001'" & vbCrLf
        'sSql &= " AND a.RID!='C7169'" & vbCrLf
...
#End Region
    ...
    Protected Sub But1_Click(sender As Object, e As EventArgs) Handles But1.Click
        ...
        Select Case String.Concat(vORGKINDGW, vKBID)
            ...
            Case Else
                ...
                Dim iBCFID As Integer = If(drFL IsNot Nothing AndAlso $"{drFL("RTUREASON")}" <> "", TIMS.CINT1(drFL("BCFID")), -1)
                '檔案上傳／確定檔案上傳
    ...
    Private Sub DataGrid2_ItemCommand(source As Object, e As DataGridCommandEventArgs) Handles DataGrid2.ItemCommand
        ...
        Select Case e.CommandName
            Case "DELFILE4"
                ...
                '"ORG_BIDCASEFL"
                Dim dParms As New Hashtable From {{"BCFID", vBCFID}}
    ...
    Private Sub DataGrid2_ItemDataBound(sender As Object, e As DataGridItemEventArgs) Handles DataGrid2.ItemDataBound
        Select Case e.Item.ItemType
            Case ListItemType.Item, ListItemType.AlternatingItem
                ...
                End If
                'If Not BTN_DELFILE4.Visible OrElse Not BTN_DELFILE4.Enabled Then BTN_DOWNLOAD4.CommandArgument = ""
        End Select
    End Sub
    ...
    Private Function CHKDEL_ORG_BIDCASEFL(vBCFID As String) As String
        ...
        Dim sParms1 As New Hashtable From {{"BCFID", vBCFID}}
        Dim sSql As String = ""
        ...
        Select Case String.Concat(vORGKINDGW, vKBID)
            Case TIMS.cst_W08_訓練班別計畫表, TIMS.cst_G08_訓練班別計畫表
                ...
                'Return rst
                Dim PMS_1 As New Hashtable From {{"BCID", TIMS.CINT1(Hid_BCID.Value)}, {"BCFID", TIMS.CINT1(vBCFID)}}
                Dim SQL_1 As String = "SELECT * FROM ORG_BIDCASEFL_PI WHERE BCID=@BCID AND BCFID=@BCFID"
                Dim dtFLPI As DataTable = DbAccess.GetDataTable(SQL_1, objconn, PMS_1)
                '刪除檔案
                If TIMS.dtHaveDATA(dtFLPI) Then
                    For Each drFLPI As DataRow In dtFLPI.Rows
                        Dim oFILENAME1 As String = $"{drFLPI("FILENAME1")}"
                        If oFILENAME1 = "" Then Continue For
                        Dim oUploadPath As String = "" 'TIMS.GET_UPLOADPATH1(oYEARS, oAPPSTAGE, oPLANID, oRID, oBCASENO, vKBSID)
                        Dim s_FilePath1 As String = "" 'MyPage.Server.MapPath(Path.Combine(oUploadPath, oFILENAME1))
                        Dim vKBSID As String = $"{drFLPI("KBSID")}"
                        Try
                            oUploadPath = TIMS.GET_UPLOADPATH1_BI(oYEARS, oAPPSTAGE, oPLANID, oRID, oBCASENO, vKBSID)
                            s_FilePath1 = Server.MapPath(Path.Combine(oUploadPath, oFILENAME1))
                            Call TIMS.MyFileDelete(s_FilePath1)
            ...
            Case TIMS.cst_W08_1_iCap課程原始申請資料, TIMS.cst_G08_1_iCap課程原始申請資料
                Dim sParms3 As New Hashtable From {{"BCFID", vBCFID}}
    ...
    Private Sub DataGrid10_ItemCommand(source As Object, e As DataGridCommandEventArgs) Handles DataGrid10.ItemCommand
        ...
        Select Case e.CommandName
            ...
            Case "DELFILE10"
                ...
                Dim dParms As New Hashtable From {{"BCFTID", vBCFTID}}
    ...
    Private Sub DataGrid10_ItemDataBound(sender As Object, e As DataGridItemEventArgs) Handles DataGrid10.ItemDataBound
        Select Case e.Item.ItemType
            Case ListItemType.Item, ListItemType.AlternatingItem
                ...
                chkItem1.Attributes("onclick") = $"selectOnlyThis('{chkItem1.ClientID}',{iDG10_ROWS},'DataGrid10')"
                '0:未轉班,1:已轉班 '未轉班(依計畫查詢) 含列印
                ...
                End If
                'If Not BTN_DELFILE10.Visible OrElse Not BTN_DELFILE10.Enabled Then BTN_DELFILE10.CommandArgument = ""
        End Select
    End Sub
    ...
    Private Sub DataGrid11_ItemCommand(source As Object, e As DataGridCommandEventArgs) Handles DataGrid11.ItemCommand
        ...
        Select Case e.CommandName
            ...
            Case "DELFILE11"
                ...
                End If
                '"ORG_BIDCASEFL"
                Dim dParms As New Hashtable From {{"BCFT2ID", vBCFT2ID}}
    ...
    Private Sub DataGrid11_ItemDataBound(sender As Object, e As DataGridItemEventArgs) Handles DataGrid11.ItemDataBound
        Select Case e.Item.ItemType
            Case ListItemType.Item, ListItemType.AlternatingItem
                ...
                End If
                'If Not BTN_DELFILE11.Visible OrElse Not BTN_DELFILE11.Enabled Then BTN_DELFILE11.CommandArgument = ""
        End Select
    End Sub
    ...
    Public Shared Sub DELETE_Detail_BIDCASE(MyPage As Page, oConn As SqlConnection, drRR As DataRow, drOB As DataRow)
        ...
        If dtFLRT IsNot Nothing AndAlso dtFLRT.Rows.Count > 0 Then
            For Each drFLRT As DataRow In dtFLRT.Rows
                Dim oFILENAME1 As String = $"{drFLRT("FILENAME1")}"
                If oFILENAME1 = "" Then Continue For
                Dim oUploadPath As String = "" 'TIMS.GET_UPLOADPATH1(oYEARS, oAPPSTAGE, oPLANID, oRID, oBCASENO, vKBSID)
                Dim s_FilePath1 As String = "" 'MyPage.Server.MapPath(Path.Combine(oUploadPath, oFILENAME1))
                Dim vKBSID As String = $"{drFLRT("KBSID")}"
                Try
                    oUploadPath = TIMS.GET_UPLOADPATH1_BI(oYEARS, oAPPSTAGE, oPLANID, oRID, oBCASENO, vKBSID)
        ...
        If dtFLEV IsNot Nothing AndAlso dtFLEV.Rows.Count > 0 Then
            For Each drFLEV As DataRow In dtFLEV.Rows
                Dim oFILENAME1 As String = $"{drFLEV("FILENAME1")}"
                If oFILENAME1 = "" Then Continue For
                Dim oUploadPath As String = "" 'TIMS.GET_UPLOADPATH1(oYEARS, oAPPSTAGE, oPLANID, oRID, oBCASENO, vKBSID)
                Dim s_FilePath1 As String = "" 'MyPage.Server.MapPath(Path.Combine(oUploadPath, oFILENAME1))
                Dim vKBSID As String = $"{drFLEV("KBSID")}"
                Try
                    oUploadPath = TIMS.GET_UPLOADPATH1_BI(oYEARS, oAPPSTAGE, oPLANID, oRID, oBCASENO, vKBSID)
                    s_FilePath1 = MyPage.Server.MapPath(Path.Combine(oUploadPath, oFILENAME1))
                    Call TIMS.MyFileDelete(s_FilePath1)
        ...
        If dtFLTT2 IsNot Nothing AndAlso dtFLTT2.Rows.Count > 0 Then
            For Each drFLTT2 As DataRow In dtFLTT2.Rows
                Dim oFILENAME1 As String = $"{drFLTT2("FILENAME1")}"
                If oFILENAME1 = "" Then Continue For
                Dim oUploadPath As String = "" 'TIMS.GET_UPLOADPATH1(oYEARS, oAPPSTAGE, oPLANID, oRID, oBCASENO, vKBSID)
                Dim s_FilePath1 As String = "" 'MyPage.Server.MapPath(Path.Combine(oUploadPath, oFILENAME1))
                Dim vKBSID As String = $"{drFLTT2("KBSID")}"
                Try
                    oUploadPath = TIMS.GET_UPLOADPATH1_BI(oYEARS, oAPPSTAGE, oPLANID, oRID, oBCASENO, vKBSID)
                    s_FilePath1 = MyPage.Server.MapPath(Path.Combine(oUploadPath, oFILENAME1))
                    Call TIMS.MyFileDelete(s_FilePath1)
        ...
        For Each drFL As DataRow In dtFL.Rows
            Dim oFILENAME1 As String = $"{drFL("FILENAME1")}"
            If oFILENAME1 = "" Then Continue For
            Dim oUploadPath As String = ""
            Dim s_FilePath1 As String = ""
            Try
    ...
    Private Sub DataGrid14_ItemDataBound(sender As Object, e As DataGridItemEventArgs) Handles DataGrid14.ItemDataBound
        Select Case e.Item.ItemType
            Case ListItemType.Item, ListItemType.AlternatingItem
                ...
                End If
                'If Not BTN_DELFILE14.Visible OrElse Not BTN_DELFILE14.Enabled Then BTN_DELFILE14.CommandArgument = ""
        End Select
    End Sub
        ...
        Select Case e.CommandName
            ...
            Case "DELFILE14"
                ...
                If dtFLPI3 IsNot Nothing AndAlso dtFLPI3.Rows.Count > 0 AndAlso dtFLPI3.Select(tryFIND).Length > 0 Then
                    For Each drFLP3 As DataRow In dtFLPI3.Select(tryFIND)
                        Dim oFILENAME1 As String = "" 'Convert.ToString(drFLTT("FILENAME1"))
                        Dim oUploadPath As String = "" 'TIMS.GET_UPLOADPATH1(oYEARS, oAPPSTAGE, oPLANID, oRID, oBCASENO, vKBSID)
                        Dim s_FilePath1 As String = "" 'MyPage.Server.MapPath(Path.Combine(oUploadPath, oFILENAME1))
                        Try
                ...
                '"ORG_BIDCASEFL"
                Dim dParms As New Hashtable From {{"BCFP3ID", vBCFP3ID}}
    ...
    ''' <param name="sender"></param>
    ''' <param name="e"></param>
    Protected Sub BTN_RETRIEVE_Click(sender As Object, e As EventArgs) Handles BTN_RETRIEVE.Click
        'Dim v_sch_ddlYEARS As String=TIMS.GetListValue(sch_ddlYEARS)
        'Dim v_sch_ddlAPPSTAGE As String=TIMS.GetListValue(sch_ddlAPPSTAGE)
        Hid_BCID.Value = TIMS.ClearSQM(Hid_BCID.Value)
        Hid_BCASENO.Value = TIMS.ClearSQM(Hid_BCASENO.Value)
        ...
        Dim drRR As DataRow = TIMS.Get_RID_DR(RIDValue.Value, objconn)
        If drRR Is Nothing Then Return
        Dim v_ORGID As String = $"{drRR("ORGID")}"
        'Dim v_ORGLEVEL As String=$"{drRR("ORGLEVEL")}"
        Dim v_PLANID As String = $"{drRR("PLANID")}"
        Dim v_TPLANID As String = $"{drRR("TPLANID")}"
        'Dim v_YEARS As String=$"{drRR("YEARS")}"
        'Dim v_DISTID As String=$"{drRR("DISTID")}"
        Dim v_RID As String = $"{drRR("RID")}"
        ...
        Dim vBCID As String = $"{drOB("BCID")}"
        Dim S2Parms As New Hashtable From {{"TPLANID", v_TPLANID}, {"RID", v_RID}, {"PLANID", v_PLANID}, {"APPSTAGE", vAPPSTAGE}, {"BCID", vBCID}}
        Dim dtPP2 As DataTable = TIMS.GET_CLASS2S_BIdt(objconn, S2Parms)
        Dim strCLASSNAME2_0 As String = TIMS.GET_CLASSNAME2S_BI(dtPP2, TIMS.cst_outTYPE_CLSNM)
        Lab_CLASSNAME2_0.Text = TIMS.GetResponseWrite(strCLASSNAME2_0)

        Dim S1Parms As New Hashtable From {{"TPLANID", v_TPLANID}, {"RID", v_RID}, {"PLANID", v_PLANID}, {"APPSTAGE", vAPPSTAGE}}
        Dim dtPP1 As DataTable = TIMS.GET_CLASS2S_BIdt(objconn, S1Parms)
        If TIMS.dtNODATA(dtPP1) Then Return
        Dim strCLASSNAME2_1 As String = TIMS.GET_CLASSNAME2S_BI(dtPP1, TIMS.cst_outTYPE_CLSNM)
        Lab_CLASSNAME2_1.Text = TIMS.GetResponseWrite(strCLASSNAME2_1)

        Dim strCLASSNAME2_3 As String = ""
        For Each drP2 As DataRow In dtPP2.Rows
            Dim s_ff As String = $"CLASSCNAME2='{drP2("CLASSCNAME2")}'"
            If dtPP1.Select(s_ff).Length = 0 Then
                strCLASSNAME2_3 &= $"{If(strCLASSNAME2_3 <> "", vbCrLf, "")}{drP2("CLASSCNAME2")}({drP2("STDATE_ROC")})"
            End If
        Next
        For Each drP1 As DataRow In dtPP1.Rows
            Dim s_ff As String = $"CLASSCNAME2='{drP1("CLASSCNAME2")}'"
            If dtPP2.Select(s_ff).Length = 0 Then
                strCLASSNAME2_3 &= $"{If(strCLASSNAME2_3 <> "", vbCrLf, "")}{drP1("CLASSCNAME2")}({drP1("STDATE_ROC")})"
            End If
        Next
        Lab_CLASSNAME2_3.Text = TIMS.GetResponseWrite(strCLASSNAME2_3)
        If Lab_CLASSNAME2_3.Text = "" Then Lab_CLASSNAME2_3.Text = TIMS.cst_NODATAMsg1

        FrameTableEdt1.Visible = False
        FrameTableEdt2.Visible = True
    End Sub
    ...
    Protected Sub BTN_SAVENEXT2_Click(sender As Object, e As EventArgs) Handles BTN_SAVENEXT2.Click
        Const cst_DELETEOKMsg3 As String = "已送出-更新班級清單!(已刪除部份資料)"
        ...
        'Dim v_DISTID As String=$"{drRR("DISTID")}"
        Dim v_RID As String = $"{drRR("RID")}"
        'Dim v_ORGKINDGW As String=$"{drRR("ORGKINDGW")}"
        ...
        Dim vYEARS As String = TIMS.ClearSQM(drOB("YEARS"))
        Dim vAPPSTAGE As String = TIMS.ClearSQM(drOB("APPSTAGE")) 'TIMS.GetMyValue2(rPMS, "APPSTAGE")
        Dim v_BCID As String = $"{drOB("BCID")}"
        Dim v_BCASENO As String = $"{drOB("BCASENO")}"
        ...
        '(重新再取一次) 'drRR = TIMS.Get_RID_DR(RIDValue.Value, objconn) 'If drRR Is Nothing Then Return
        If RIDValue.Value = "" OrElse drRR Is Nothing Then
            Common.MessageBox(Me, "資訊有誤(查無業務代碼)，請選擇訓練機構!!")
            Return
        End If

        '原已上傳之班級相關文件從第7項開始均會自動清除，單位須重新上傳
        Call DELETE_Detail_BIDCASE_7(Me, sm, objconn, drRR, drOB)
        'Common.MessageBox(Me, cst_DELETEOKMsg3)
        ...
        'Dim v_sch_ddlAPPSTAGE As String=TIMS.GetListValue(sch_ddlAPPSTAGE)
        Dim sERRMSG As String = ""
        Dim fg_CanADDNEW1 As Boolean = Utl_ADDNEW_DATA1(sERRMSG, drRR, TIMS.CINT1(v_BCID), v_BCASENO)
        If sERRMSG <> "" Then
            Common.MessageBox(Me, sERRMSG)
            Return
        End If
    ...
    ''' <summary>執行部份刪除</summary>
    ''' <param name="MyPage"></param>
    ''' <param name="sm"></param>
    ''' <param name="oConn"></param>
    ''' <param name="drRR"></param>
    ''' <param name="drOB"></param>
    Public Shared Sub DELETE_Detail_BIDCASE_7(MyPage As Page, sm As SessionModel, oConn As SqlConnection, drRR As DataRow, drOB As DataRow)
        '訓練機構有誤
        If drRR Is Nothing OrElse drOB Is Nothing Then Return
        ...
        '刪除檔案 (混成課程教學環境資料表) 
        Dim dtFLRT As DataTable = TIMS.GET_ORG_BIDCASEFL_RT(oConn, vBCID)
        If TIMS.dtHaveDATA(dtFLRT) Then
            For Each drFLRT As DataRow In dtFLRT.Rows
        ...
        '刪除檔案 (教學環境資料表)
        Dim dtFLEV As DataTable = TIMS.GET_ORG_BIDCASEFL_EV(oConn, vBCID)
        If TIMS.dtHaveDATA(dtFLEV) Then
            For Each drFLEV As DataRow In dtFLEV.Rows
        ...
        '刪除檔案 (項目上傳檔案) 'DELETE ORG_BIDCASEFL WHERE
        Dim dtFL As DataTable = TIMS.GET_ORG_BIDCASEFL(oConn, vBCID)
        For Each drFL As DataRow In dtFL.Rows
            Dim oFILENAME1 As String = $"{drFL("FILENAME1")}"
            Dim oBCFID As String = $"{drFL("BCFID")}"

            Dim v_KBSID As String = $"{drFL("KBSID")}"
            Dim drKB As DataRow = TIMS.GET_KEY_BIDCASE(sm, oConn, v_KBSID, v_ORGKINDGW)
            If drKB Is Nothing Then Continue For
            Dim vREDEL7 As String = $"{drKB("REDEL7")}" '執行部份刪除動作
            '有檔案名稱，且 vREDEL7="Y", 執行部份刪除動作
            If oFILENAME1 = "" OrElse Not (vREDEL7 = "Y") Then Continue For
            Dim oUploadPath As String = ""
            Dim s_FilePath1 As String = ""
            Try
                oUploadPath = TIMS.GET_UPLOADPATH1_BI(oYEARS, oAPPSTAGE, oPLANID, oRID, oBCASENO, "")
            ...
            End Try
            Try
                Dim dParms3 As New Hashtable From {{"BCFID", TIMS.CINT1(oBCFID)}, {"BCID", TIMS.CINT1(vBCID)}}
                Dim dsSql3 As String = "DELETE ORG_BIDCASEFL WHERE BCFID=@BCFID AND BCID=@BCID" & vbCrLf
                DbAccess.ExecuteNonQuery(dsSql3, oConn, dParms3)
            Catch ex As Exception
                Dim strErrmsg As String = String.Concat(New Diagnostics.StackFrame(True).GetMethod().Name, vbCrLf)
                strErrmsg &= $",BCID: {vBCID},BCFID: {oBCFID},ex.Message: {ex.Message}{vbCrLf}"
                Call TIMS.WriteTraceLog(strErrmsg, ex) 'Throw ex
            End Try
        Next

        '剩下為 (檔案名稱可能為空),vREDEL7="Y", 執行部份刪除動作
        '08.訓練班別計畫表 ,'10.師資／助教基本資料表 ,'11.各授課師資學／經歷證書影本 ,'13.教學環境資料表 ,
        '16.租借學校場地或政府機關（構）場地者，須檢附校方或機關（構）同意租借證明文件 ,'17.目的事業主管機關核備開課之文件,
        Try
            Dim dParms3B As New Hashtable From {{"BCID", TIMS.CINT1(vBCID)}}
            Dim dsSql3B As String = "DELETE ORG_BIDCASEFL FROM ORG_BIDCASEFL f JOIN KEY_BIDCASE k on k.KBSID=f.KBSID WHERE f.BCID=@BCID AND k.REDEL7='Y'" & vbCrLf
            DbAccess.ExecuteNonQuery(dsSql3B, oConn, dParms3B)
        Catch ex As Exception
            Dim strErrmsg As String = String.Concat(New Diagnostics.StackFrame(True).GetMethod().Name, vbCrLf)
            strErrmsg &= $",BCID: {vBCID}, ex.Message: {ex.Message}{vbCrLf}"
            Call TIMS.WriteTraceLog(strErrmsg, ex) 'Throw ex
        End Try
        ...
        'Dim vPLANID As String=Convert.ToString(drRR("PLANID"))
        'Hid_ORGKINDGW.Value=Convert.ToString(drRR("ORGKINDGW"))
        'dsSql="DELETE ORG_BIDCASE WHERE BCID=@BCID" & vbCrLf 'DbAccess.ExecuteNonQuery(dsSql, oConn, dParms)
        'dsSql="DELETE ORG_BIDCASEFL WHERE BCID=@BCID" & vbCrLf 'DbAccess.ExecuteNonQuery(dsSql, oConn, dParms)
        Dim dParms As New Hashtable From {{"BCID", TIMS.CINT1(vBCID)}}
        Dim dsSql As String = ""
        dsSql = "DELETE ORG_BIDCASEPI WHERE BCID=@BCID"
        DbAccess.ExecuteNonQuery(dsSql, oConn, dParms)
        '混成課程教學環境資料表／遠距
        dsSql = "DELETE ORG_BIDCASEFL_RT WHERE BCID=@BCID"
        DbAccess.ExecuteNonQuery(dsSql, oConn, dParms)
        'iCap課程原始申請資料
        dsSql = "DELETE ORG_BIDCASEFL_EV WHERE BCID=@BCID"
        DbAccess.ExecuteNonQuery(dsSql, oConn, dParms)
        dsSql = "DELETE ORG_BIDCASEFL_PI WHERE BCID=@BCID"
        DbAccess.ExecuteNonQuery(dsSql, oConn, dParms)
        dsSql = "DELETE ORG_BIDCASEFL_TT WHERE BCID=@BCID"
        DbAccess.ExecuteNonQuery(dsSql, oConn, dParms)
        dsSql = "DELETE ORG_BIDCASEFL_TT2 WHERE BCID=@BCID"
        DbAccess.ExecuteNonQuery(dsSql, oConn, dParms)
    End Sub

    ''' <summary>檢核資料異常,異常TRUE,沒事False</summary>
    ''' <param name="drRR"></param>
    ''' <param name="drOB"></param>
    ''' <returns></returns>
    Function CheckPCS_SEQNO(drRR As DataRow, drOB As DataRow) As Boolean
        '訓練機構有誤
        If drRR Is Nothing OrElse drOB Is Nothing Then Return False '(沒資料就算了吧)
        ...
        Dim v_DISTID As String = $"{drRR("DISTID")}"
        Dim v_RID As String = $"{drRR("RID")}"
        Dim v_APPSTAGE As String = $"{drOB("APPSTAGE")}"
        Dim S1Parms As New Hashtable From {{"TPLANID", v_TPLANID}, {"RID", v_RID}, {"PLANID", v_PLANID}, {"APPSTAGE", v_APPSTAGE}}
        Dim dtPP1 As DataTable = TIMS.GET_CLASS2S_BIdt(objconn, S1Parms)
        If TIMS.dtNODATA(dtPP1) Then Return False '(沒資料就算了吧)

        Dim vBCID As String = $"{drOB("BCID")}"
        Dim S2Parms As New Hashtable From {{"TPLANID", v_TPLANID}, {"RID", v_RID}, {"PLANID", v_PLANID}, {"APPSTAGE", v_APPSTAGE}, {"BCID", vBCID}}
        Dim dtPP2 As DataTable = TIMS.GET_CLASS2S_BIdt(objconn, S2Parms)
        If TIMS.dtNODATA(dtPP2) Then Return False '(沒資料就算了吧)
        Dim iPP1CNT As Integer = dtPP1.Rows.Count
        Dim iPP2CNT As Integer = dtPP2.Rows.Count
        If iPP1CNT <> iPP2CNT Then Return True '(數量不一致，異常)
        ' 1. 建立一個 DataView 物件，指向 dtPP1
        Dim dv1 As New DataView(dtPP1) With {.Sort = "SEQNO ASC"}
        ' 2. 設定排序條件：欄位排序
        Dim V_SEQNO1 As String = ""
        ' 3. 逐一取得排序後的 DataRowView
        For Each drv1 As DataRowView In dv1
            V_SEQNO1 &= $"{If(V_SEQNO1 <> "", ",", "")}{drv1("SEQNO")}"
        Next
        ' 1. 建立一個 DataView 物件，指向 dtPP1
        Dim dv2 As New DataView(dtPP2) With {.Sort = "SEQNO ASC"}
        ' 2. 設定排序條件：欄位排序
        Dim V_SEQNO2 As String = ""
        ' 3. 逐一取得排序後的 DataRowView
        For Each drv2 As DataRowView In dv2
            V_SEQNO2 &= $"{If(V_SEQNO2 <> "", ",", "")}{drv2("SEQNO")}"
        Next
        If V_SEQNO1 <> V_SEQNO2 Then Return True '(內容不一致，異常)
        Return False '(沒有發現問題)
    End Function
