...
Partial Class TC_11_002_54
    ...
    '(54:充電起飛計畫（在職）)切換程式
    Dim gfg_TPlanID54 As Boolean = True
    ...
    Private Sub cCreate1(ByVal iNum As Integer)
        ...
        'If sch_ddlAPPSTAGE.Visible Then
    ...
    Private Sub sSearch1()
        ...
        ElseIf v_rbAPPLIEDRESULT = "B" Then
            'NULL:申辦中尚未確認 'B:已送件
            sSql &= " AND a.APPLIEDRESULT IS NULL AND a.BISTATUS='B'" & vbCrLf
    ...
    Private Sub SHOW_Detail_BIDCASE(ByRef drRR As DataRow, ByVal vBCID As String, ByVal vCmdName As String)
        ...
        Dim pParms As New Hashtable
        pParms.Add("BCID", vBCID)
        pParms.Add("RID", vRID)
        pParms.Add("PLANID", vPLANID)
...
#Region "Public Shared"
    ...
    ''' <returns></returns>
    Public Shared Function GET_ORG_BIDCASEFL(ByRef oConn As SqlConnection, vBCID As String, vORGKINDGW As String, vTPLANID As String) As DataTable
        ...
        rsSql &= " WHERE a.BCID=@BCID AND kb.ORGKINDGW=@ORGKINDGW AND kb.TPLANID=@TPLANID" & vbCrLf
    ...
    Public Shared Sub ResponseZIPFileALL_BI(sm As SessionModel, ByRef MyPage As Page, ByRef oConn As SqlConnection, ByRef rPMS As Hashtable)
        Const cst_UtlSubName As String = "/*ResponseZIPFileALL_BI(sm As SessionModel, ByRef MyPage As Page, ByRef oConn As SqlConnection, ByRef rPMS As Hashtable)*/"
        'Hid_ORGKINDGW.Value = TIMS.ClearSQM(Hid_ORGKINDGW.Value)
        'Hid_RID.Value = TIMS.ClearSQM(Hid_RID.Value)
        'Hid_BCID.Value = TIMS.ClearSQM(Hid_BCID.Value)
        'Hid_BCASENO.Value = TIMS.ClearSQM(Hid_BCASENO.Value)
        ...
        Dim dtFL As DataTable = GET_ORG_BIDCASEFL(oConn, vBCID, vORGKINDGW, vTPLANID)
            ...
            Select Case String.Concat(vORGKINDGW, vKBID)
                    ...
                    If dtFLPI IsNot Nothing AndAlso dtFLPI.Rows.Count > 0 Then
                        ...
                        For Each drFLPI As DataRow In dtFLPI.Rows
                            ...
                            Try
                                ...
                                '年度申請階段_班級課程流水號_項目編號+項目名稱
                                t_FILENAME_PI = TIMS.GetValidFileName(String.Concat(vYEARS_ROC, "_", vORGNAME, "_", vKBNAME2, "_", vPSNO28, ".pdf"))
                    ...
                    If dtFLTT IsNot Nothing AndAlso dtFLTT.Rows.Count > 0 Then
                        For Each drFLTT As DataRow In dtFLTT.Rows
                            ...
                            Try
                                ...
                                s_FilePath1 = MyPage.Server.MapPath(Path.Combine(oUploadPath, oFILENAME1))
                                '年度申請階段_講師名稱_講師代碼_項目編號+項目名稱
                                t_FILENAME_TT = TIMS.GetValidFileName(String.Concat(vYEARS_ROC, "_", vORGNAME, "_", vKBNAME2, "_", vTEACHERID, "_", vTEACHCNAME, ".pdf"))
                    ...
                    If dtFLTT2 IsNot Nothing AndAlso dtFLTT2.Rows.Count > 0 Then
                        For Each drFLTT2 As DataRow In dtFLTT2.Rows
                            ...
                            Try
                                ...
                                s_FilePath1 = MyPage.Server.MapPath(Path.Combine(oUploadPath, oFILENAME1))
                                '年度申請階段_講師名稱_講師代碼_項目編號+項目名稱
                                t_FILENAME_TT2 = TIMS.GetValidFileName(String.Concat(vYEARS_ROC, "_", vORGNAME, "_", vKBNAME2, "_", vTEACHERID, "_", vTEACHCNAME, ".pdf"))
                    ...
                    If dtFLEV IsNot Nothing AndAlso dtFLEV.Rows.Count > 0 Then
                        ...
                        For Each drFLEV As DataRow In dtFLEV.Rows
                            ...
                            Try
                                ...
                                s_FilePath1 = MyPage.Server.MapPath(Path.Combine(oUploadPath, oFILENAME1))
                                '年度申請階段_班級課程流水號_項目編號+項目名稱
                                t_FILENAME_PI2 = TIMS.GetValidFileName(String.Concat(vYEARS_ROC, "_", vORGNAME, "_", vKBNAME2, "_", vPSNO28, ".pdf"))
                ...
                Case Else
                    ...
                    Try
                        ...
                        s_FilePath1 = MyPage.Server.MapPath(Path.Combine(oUploadPath, oFILENAME1))
                        '年度申請階段_單位名稱_項目編號+項目名稱
                        t_FILENAME = TIMS.GetValidFileName(String.Concat(vYEARS_ROC, "_", vORGNAME, "_", vKBNAME2, ".pdf"))
        ...
        Dim zipFileName As String = TIMS.GetValidFileName(String.Concat("p", vYEARS_ROC, "_", vDISTNAME3, "_", vORGNAME, "_", vBCID, "_", strNOW, ".zip"))
        ...
        With MyPage
            ...
            TIMS.SAVE_ADP_ZIPFILE(oConn, "-tc1100254bi", File)
    ...
    Public Shared Sub ResponseZIPFilePI(MyPage As Page, ByRef oConn As SqlConnection, ByRef rPMS As Hashtable, ByRef dtOBWP As DataTable)
        Const cst_UtlSubName As String = "/*ResponseZIPFilePI(MyPage As Page, ByRef oConn As SqlConnection, ByRef rPMS As Hashtable, ByRef dtOBWP As DataTable)*/"
        ...
        Dim strNOW As String = DateTime.Now.ToString("yyyyMMddHHmmss")
        Dim zipFileName As String = String.Concat("p", vYEARS_ROC, "_", vDISTNAME, "_", strNOW, ".zip")
        Dim filenames As String() = Directory.GetFiles(MyPage.Server.MapPath(String.Concat(Template_ZipPath1, "/")))
        Dim full_zipFileName As String = String.Concat(Template_ZipPath1, "/", zipFileName)
        Using zp As New ZipOutputStream(System.IO.File.Create(MyPage.Server.MapPath(full_zipFileName)))
            zp.SetLevel(6) ' 設定壓縮比
        ...
        With MyPage
            Dim File As New FileInfo(.Server.MapPath(full_zipFileName))
            TIMS.SAVE_ADP_ZIPFILE(oConn, "-tc1100254pi", File)
            ' Clear the content of the response
            .Response.ClearContent()
            ' LINE1 Add the file name And attachment, which will force the open/cance/save dialog To show, to the header
            .Response.AddHeader("Content-Disposition", String.Concat("attachment; filename=", File.Name))
...
#Region "Private1"
    ...
    Private Function CHK_ORG_BIDCASEFL_RTUREASON() As Boolean
        ...
        Dim vBCID As String = TIMS.ClearSQM(Hid_BCID.Value)
        Dim vORGKINDGW As String = TIMS.ClearSQM(Hid_ORGKINDGW.Value)
        Dim dtFL As DataTable = GET_ORG_BIDCASEFL(objconn, vBCID, vORGKINDGW, vTPLANID)
        For Each drFL As DataRow In dtFL.Rows
    ...
    Private Shared Function GET_ORG_BIDCASE_WAIVED_PI(oConn As SqlConnection, rPMS2 As Hashtable) As DataTable
        Dim vPLANID As String = TIMS.GetMyValue2(rPMS2, "PLANID")
        ...
        sSql &= " WHERE a.YEARS=@YEARS AND a.DISTID=@DISTID AND a.PLANID=@PLANID" & vbCrLf
...
#End Region
    ...
    Protected Sub BTN_PACKAGE_DOWNLOAD1_Click(sender As Object, e As EventArgs) Handles BTN_PACKAGE_DOWNLOAD1.Click
        ...
        ' "DOWNLOAD4" '下載
        Call ResponseZIPFileALL_BI(sm, Me, objconn, rPMS)
    End Sub
    ...
    Protected Sub BTN_TRAIN_PACKAGE_DOWNLOAD1_Click(sender As Object, e As EventArgs) Handles BTN_TRAIN_PACKAGE_DOWNLOAD1.Click
        ...
        If sm.UserInfo.LID = 0 Then vPLANID = TIMS.GetPlanID(sm.UserInfo.TPlanID, v_sch_ddlYEARS, vDISTID, objconn)

        If v_sch_ddlYEARS = "" Then
            Common.MessageBox(Me, "計畫年度 為必選!")
            Return
        ElseIf Hid_RID.Value = "" OrElse vDISTID = "" Then
            Common.MessageBox(Me, "訓練機構 為必選!")
            Return
        ElseIf vDISTID = "" Then
